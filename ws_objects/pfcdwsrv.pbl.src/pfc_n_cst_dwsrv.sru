$PBExportHeader$pfc_n_cst_dwsrv.sru
...
public function integer of_getobjects (ref string as_objlist[], string as_objtype, string as_band, boolean ab_visibleonly, boolean ab_isaddlabel)
end prototypes
...
string ls_visible//(Appeon)Toney 09.05.2013 - V141 ISG-CLX
...
Do While li_Tab > 0
	...
	ls_Visible = idw_Requestor.Describe(ls_ObjHolder + ".visible")//modify by gavins 20120621 apb bug//(Appeon)Toney 09.05.2013 - V141 ISG-CLX
	...
	If (idw_Requestor.Describe(ls_ObjHolder + ".type") = as_ObjType Or as_ObjType = "*") And &
		...
		(ls_visible = "1" Or ls_visible ="-1" Or Not ab_VisibleOnly) Then//(idw_Requestor.Describe(ls_ObjHolder + ".visible") = "1" Or Not ab_VisibleOnly) Then//(Appeon)Toney 09.05.2013 - V141 ISG-CLX
			li_Count ++
...
IF li_col_count > 0 THEN
	...
	FOR li_cnt = 1 to li_col_count
		...
		IF PosA (a_dwobject_attrib[li_cnt].is_column, "!", 1) > 0 THEN 
		...
		IF PosA (a_dwobject_attrib[li_cnt].is_datatype, "!", 1) > 0 THEN
		...
		IF PosA (a_dwobject_attrib[li_cnt].is_value, "!", 1) > 0 THEN
...
ELSE	
	...
	IF PosA (a_dwobject_attrib[1].is_datatype, "!", 1) > 0 THEN
	...
	IF PosA (a_dwobject_attrib[1].is_value, "!", 1) > 0 THEN
...
CHOOSE CASE Lower ( LeftA ( idw_Requestor.Describe ( as_column + ".ColType" ) , 5 ) )
...
CHOOSE CASE LeftA ( ls_coltype , 5 )
		...
		CASE "date"					//  DATE DATATYPE
			...
			if LenA (ls_string_format) > 0 then
...
End If
If LenA(Trim(as_column))=0 or IsNull(as_column) Then Return '!'
...
For ll_a = 1 to ll_args
	...
	If ll_pos > 0 Then
		as_argnames[UpperBound(as_argnames)+1] = LeftA ( ls_dwargswithtype[ll_a], ll_pos - 1 ) 
		as_argdatatypes[UpperBound(as_argdatatypes)+1] = MidA ( ls_dwargswithtype[ll_a], ll_pos + 1 ) 
	End If
Next
...
if (RegistryGet("HKEY_CURRENT_USER\Control Panel\International","sThousand",RegString!,ls_comma) <> 1) then
	ls_comma = ","
end if
...
CHOOSE CASE Lower ( LeftA ( idw_Requestor.Describe ( as_column + ".ColType" ) , 5 ) )
		...
		CASE "decim"		//  DECIMAL DATATYPE
			...
			ls_string_value = lnv_string.of_GlobalReplace (ls_string_value, ls_comma, "" ) 
			ls_string_value = lnv_string.of_GlobalReplace (ls_string_value, "(", "-")
			ls_string_value = lnv_string.of_GlobalReplace (ls_string_value, ")", "")
			if PosA (ls_string_value, "%") > 0 then
				ls_string_value = lnv_string.of_GlobalReplace (ls_string_value, "%", "")
...
//   ab_visibleonly: TRUE  - get only the visible objects,
//							 FALSE - get visible and non-visible objects
//   ab_isaddlabel   TRUE  - get column name add label name
//                   False - get column name
//
//	Returns:  		Integer
//   					The number of objects in the array
//
...
Do While li_Tab > 0
	...
	If (idw_Requestor.Describe(ls_ObjHolder + ".type") = as_ObjType Or as_ObjType = "*") And &
		...
		(ls_visible = "1" Or ls_visible ="-1" Or Not ab_VisibleOnly) Then//(idw_Requestor.Describe(ls_ObjHolder + ".visible") = "1" Or Not ab_VisibleOnly) Then//(Appeon)Toney 09.05.2013 - V141 ISG-CLX
         ...
         if ab_isaddlabel then
				ls_objlabel = idw_Requestor.Describe(ls_ObjHolder + "_t.text")
				if isnull(ls_objlabel) or trim(ls_objlabel) = '' or trim(ls_objlabel) = '!' then
						//
				else
			    		as_ObjList[li_Count] = ls_ObjHolder + " [" + ls_objlabel + "]"
				end if
			end if
	End if
