$PBExportHeader$pfc_n_cst_dwsrv_sort.sru
...
type variables
string 	is_sortorder1  //Add by Jack 07.19.2007
...
constant integer APPEONDRAGDROP = 4    //Add by Jack 06/21/2007
...
public function string of_build_click_sort_syntax (integer ai_xpos, integer ai_ypos, long al_row, dwobject adwo_obj)
end prototypes
...
string 	ls_ColType	//Add by Jervis 10-23-2008
...
li_headerlen = LenA(ls_headername)
li_suffixlen = LenA(is_defaultheadersuffix)

// Extract the columname from the header label 
// (by taking out the header suffix).
IF RightA(ls_headername, li_suffixlen) <> is_defaultheadersuffix THEN 
	// Cannot determine the column name from the header.	
	Return -1
END IF 	
ls_colname = LeftA (ls_headername, li_headerlen - li_suffixlen)

// Validate the column name.
If IsNull(ls_colname) or LenA(Trim(ls_colname))=0 Then 
	Return -1
End If
//---------Begin Modified by (Appeon)Toney 06.04.2013 for V141 ISG-CLX--------
    /*
		...
		ELSE
			//Start code by Jervis 10-23-2008------------------------------
			//PB Bug,Sort error when column type is datetime
			//ls_sortstring = is_sortcolumn + is_sortorder
			If not IsNull(idw_Requestor) and IsValid(idw_Requestor) Then 
				ls_ColType = idw_requestor.Describe(is_sortcolumn + ".Coltype")
			end if
			if ls_ColType = 'datetime' then
				//APB Bug,Unsupport if expression,Must user Date function
				ls_sortstring = "Date(if (isnull("+is_sortcolumn+"),date('1900-1-1')," + is_sortcolumn +"))"+ is_sortorder
			else
				ls_sortstring = is_sortcolumn + is_sortorder
			end if
			//End code by Jervis 10-23-2008---------------------------------
    ...
    */
//Modified By Ken.Guo 2009-12-21. Copy below script into of_build_click_sort_syntax()
ls_sortstring = of_build_click_sort_syntax(ai_xpos,ai_ypos,al_row,adwo_obj) 
If ls_sortstring = '-1' Then Return -1
//---------End Modfiied ------------------------------------------------------------------
...
li_rc = of_Sort()
If li_rc < 0 Then Return li_rc	
is_sortorder1 = is_sortorder  //Add by Jack 07.19.2007	
Return 1
...
CHOOSE CASE ii_style
	...
	//--------------------------- APPEON BEGIN ---------------------------
	//$<modify> 06.21.2007 By: Jack
	//$<reason> Contract Screen Painter Sort.
	/*
	CASE DRAGDROP, SIMPLE, DROPDOWNLISTBOX 
	*/
	CASE DRAGDROP, SIMPLE, DROPDOWNLISTBOX ,APPEONDRAGDROP	
	//---------------------------- APPEON END ----------------------------
		...
		*/
		ELSEIF ii_style = 3 THEN
			// 3 = DDLB style dialog.
			OpenWithParm (w_sortmulti, lnv_sortattrib) 					
		ELSE
			// 4 = Drag and Drop style dialog.
			OpenWithParm (w_sortdragdrop_contract, lnv_sortattrib) 			
		END IF
	//---------------------------- APPEON END ----------------------------
...
//--------------------------- APPEON BEGIN ---------------------------
//$<modify> 2005-09-07 By: Liang QingShi
//$<reason> 

//$<modify> 01.08.2008 by: Andy
//Return idw_Requestor.Describe ("DataWindow.Table.Sort")	//2006.6.27 By Jervis
String ls_Sort
Long   ll_Start = 1,ll_Pos

ls_Sort = idw_Requestor.Describe ("DataWindow.Table.Sort")
//Remove space after the comma(s),because the space of being uncertain.
DO WHILE (1=1)
	ll_Pos  = PosA(ls_Sort,",",ll_Start)
	if ll_Pos < 1 then exit
	
	ls_Sort = LeftA(ls_Sort,ll_Pos) + Trim(MidA(ls_Sort,ll_Pos + 1))
	ll_Start = ll_Pos + 1
LOOP
...
/*
string ls_Sort,ls_Sort1
long ll_Start,ll_Start1

ls_Sort = idw_Requestor.Describe ("DataWindow.Table.Sort")
If appeongetclienttype() <> 'PB' Then
	ll_Start = pos(ls_Sort,' A')
	Do While ll_Start > 0
		ls_Sort = Replace(ls_Sort, ll_Start, 2, ' A,')		
		ll_Start = Pos(ls_Sort, ' A', (ll_Start + 3))
	Loop

	ll_Start1 = pos(ls_Sort,' D')
	Do While ll_Start1 > 0
		ls_Sort = Replace(ls_Sort, ll_Start1, 2, ' D,')		
		ll_Start1 = Pos(ls_Sort, ' D', (ll_Start1 + 3))
	Loop
...
// Check parameters.
If IsNull(as_column) or LenA(Trim(as_column))=0 Then Return False
...
//Appeon Start Add By Jervis 06.21.2007 
int li_numsorts
string ls_orisorts[]
string ls_colname
if of_getStyle() = APPEONDRAGDROP and ib_visibleonly and anv_sortattrib.is_sort <> "?" and anv_sortattrib.is_sort <> "!" then
	lnv_string.of_parsetoarray( anv_sortattrib.is_sort, ",", ls_orisorts)
	li_numsorts = UpperBound(ls_orisorts)
	for li_i = 1 to li_numsorts
		ls_colname = lnv_string.of_GetToken (ls_orisorts[li_i], " ") 
		if idw_Requestor.Describe(ls_colname + ".visible") <> '1' and idw_Requestor.Describe(ls_colname + ".visible") <> '!' then
			li_numcols++
			ls_sortcolumns_all[li_numcols] = ls_colname
		end if
	next
end if
//Appeond End
...
FOR li_i = 1 To li_num_cols
	...
	IF PosA(Lower(ls_parse), Lower("LookUpDisplay(")) > 0 THEN
		...
		//--------Begin Added by Nova 03.24.2008------------------------
		//reason: fixed web error.
	ELSEIF PosA(Lower(ls_parse), Lower("LookUpDisplay (")) > 0 THEN
		ls_parse = lnv_string.of_GlobalReplace ( ls_parse, "LookUpDisplay (", "" )
		ls_parse = lnv_string.of_GlobalReplace ( ls_parse, " )", "" )
		ls_parse = Trim ( ls_parse )
		//--------End Added --------------------------------------------
...
CHOOSE CASE ai_style
	...
	*/
	CASE DEFAULT, DRAGDROP, SIMPLE, DROPDOWNLISTBOX,APPEONDRAGDROP	
	//---------------------------- APPEON END ----------------------------
		ii_style = ai_style

		if ii_style = DEFAULT then
			if isValid (idw_requestor) then
				if LenA (idw_requestor.dataobject) > 0 then
					//--------------------------- APPEON BEGIN ---------------------------
					//$<comment> 2005-08-30 By: Liang QingShi
					//$<reason> The DataWindow control 'help' is unsupported.
...
public function string of_build_click_sort_syntax (integer ai_xpos, integer ai_ypos, long al_row, dwobject adwo_obj);//////////////////////////////////////////////////////////////////////
// $<function>pfc_n_cst_dwsrv_sortof_build_click_sort_syntax()
// $<arguments>
//		value	integer 	ai_xpos 		
//		value	integer 	ai_ypos 		
//		value	long    	al_row  		
//		value	dwobject	adwo_obj		
// $<returns> string
// $<description>
// $<description> Copy below script from pfc_clicked
//////////////////////////////////////////////////////////////////////
// $<add> 2009-12-21 by Ken.Guo
//////////////////////////////////////////////////////////////////////
//$<Modify History>:
