$PBExportHeader$pfc_n_cst_dwsrv_dropdownsearch.sru
...
event pfc_itemchanged ( ref long al_row,  ref dwobject adwo_obj,  ref string as_data )
event pfc_setdata ( datawindow adw_requestor,  long al_row,  string as_column,  string as_data )
end type
...
li_searchtextlen = LenA (ls_searchtext)
...
If (li_searchtextlen < LenA(is_textprev)) or &
...
If inv_columns[ii_currentindex].s_editstyle = 'dddw' Then
	...
	//------------------Begin add code for ACF V1.0 @2003-07-23 ------------------
	//Commented
	//ls_dddw_displaycol = adwo_obj.dddw.displaycolumn
	//Added
	ls_dddw_displaycol = idw_requestor.Describe(ls_searchcolname + ".dddw.displaycolumn")
	//------------------End add code for ACF V1.0 @2003-07-23 ------------------
...
ElseIf inv_columns[ii_currentindex].s_editstyle = 'ddlb' Then
	...
	Do
		...
		lb_matchfound = ( Lower(ls_searchtext) = Lower( LeftA(ls_displaydata_value, LenA(ls_searchtext))) )
	...
	If lb_matchfound Then
		...
		ls_foundtext = LeftA (ls_displaydata_value, PosA(ls_displaydata_value,'~t') -1)			
	End If
End If
...
//------------------Begin commented for ACF V1.0 @2003-06-19------------------
//	// Determine what to highlight or where to move the cursor..
	if li_searchtextlen = LenA(ls_foundtext) THEN
		// Move the cursor to the end
		idw_requestor.SelectText (LenA (ls_foundtext)+1, 0)
	else
		// Hightlight the portion the user has not actually typed.
		idw_requestor.SelectText (li_searchtextlen + 1, LenA (ls_foundtext))
	end if
//------------------End commented for ACF V1.0 @2003-06-19------------------
end if
...
int		li_index
//------------------Begin add code for ACF V1.0 @2003-07-14------------------
String	ls_ColName
//------------------End add code for ACF V1.0 @2003-07-14------------------
...
ls_editstyle = idw_Requestor.Describe(ls_ColName+".Edit.Style")
//------------------End add code for ACF V1.0 @2003-07-14------------------
...
Else
	...
	ls_ddlb_displayvalue = LeftA(ls_ddlb_displaydatavalue, PosA(ls_ddlb_displaydatavalue, '~t') -1)
...
If lb_matchfound Then
	// Set the text.
	this.post event pfc_setData(idw_requestor,al_row,ls_searchcolname,ls_foundtext)
//------------------End commented for ACF V1.0 @2003-06-19------------------
end if
...
event pfc_setdata(datawindow adw_requestor, long al_row, string as_column, string as_data);
adw_requestor.setItem(al_row,as_column,as_data)
end event
...
// Check arguments
If IsNull(as_column) Or LenA(Trim(as_column))=0 Then Return -1
...
For li_i=1 to li_colcount
	...
	If ls_editstyle = 'dddw' Then
		...
		If PosA(ls_displayvaluecoltype, "char") >= 1 Then
...
// Check arguments
If IsNull(as_column) Or LenA(Trim(as_column))=0 Or &
...
// Check arguments
If IsNull(as_column) Or LenA(Trim(as_column))=0 Then 
	Return False
End If
