$PBExportHeader$pfc_n_cst_dwsrv_multitable.sru
$PBExportComments$PFC DataWindow Multiple Table service
forward
global type pfc_n_cst_dwsrv_multitable from n_cst_dwsrv
end type
end forward

global type pfc_n_cst_dwsrv_multitable from n_cst_dwsrv
end type
global pfc_n_cst_dwsrv_multitable pfc_n_cst_dwsrv_multitable

type variables
Protected:
//	7.0   Instance variable datatype changed from os_updcharacteristics to n_cst_dwsrv_multitableattrib type
//	7.0   Instance variable name changed from istr_newupdate to inv_newupdate
n_cst_dwsrv_multitableattrib inv_newupdate[]

// 7.0	This variable is obsolete.  Left in for migration purposes.
//			DO NOT USE THIS VARIABLE
n_cst_dwsrv_multitableattrib istr_newupdate[]
end variables
...
protected function integer of_storeupdatesettings (ref n_cst_dwsrv_multitableattrib anv_originalupdate)
...
//	Arguments:			ab_accepttext:	A boolean value specifying whether the DataWindow control
...
n_cst_dwsrv_multitableattrib	lnv_originalupdate
...
FOR li_tabidx = 1 to li_num_tables
	...
	FOR li_updcolidx = 1 to li_num_cols
		ls_column = idw_Requestor.Describe ( "#" + String(li_updcolidx) + ".Name" )
...
//   						ai_whereoption:	An integer that corresponds to the datawindow Update Where option
...
anv_originalupdate.s_updatetable = &
	idw_Requestor.Describe ( "DataWindow.Table.UpdateTable" )
	
// Store the Update Where statement.	
anv_originalupdate.i_whereoption = &
	Integer ( idw_Requestor.Describe ( "DataWindow.Table.UpdateWhere" ) )
	
// Store the Update Key In Place flag.	
IF Lower(idw_Requestor.Describe("DataWindow.Table.UpdateKeyInPlace")) = "yes" THEN 
...
li_numcols = Integer (idw_Requestor.Describe ( "DataWindow.Column.Count" )) 
FOR li_colidx = 1 to li_numcols
	...
	IF Lower(idw_Requestor.Describe (ls_column + ".Update" )) = "yes" THEN
	...
	IF Lower(idw_Requestor.Describe (ls_column + ".Key" )) = "yes" THEN
...
anv_infoattrib.is_description = &
	'Facilitate the updating of DataWindows that contain rows from more '+&
	'than one table.'
...
FOR li_i = 1 to li_numcols
	ls_column = idw_Requestor.Describe ( "#" + String(li_i) + ".Name" )
	IF ls_column = '!' Then Return -1

	// Get the table name for this column
	ls_table  = idw_Requestor.Describe (ls_column + ".dbName" ) 
	// Extract just the table portion of the column's database name
	ls_table  = LeftA ( ls_table, PosA ( ls_table, ".", 1) - 1 )

	//  Trim Owner name off passed table name and append to tablename
	ll_pos = PosA ( as_table, "." ) 
	If ll_pos > 0 Then ls_table = LeftA ( as_table, ll_pos ) + ls_table 
...
IF IsNull (as_table) OR LenA(Trim(as_table)) = 0 THEN Return -1
...
li_columncnt = Integer ( idw_Requestor.Describe ( "DataWindow.Column.Count" ) ) 
For li_cnt = 1 to li_columncnt
	ls_column = idw_Requestor.Describe ( "#" + String(li_cnt) + ".Name" )
	IF ls_column = '!' Then Return -1
...
For li_cnt = 1 To li_keycolumncnt
	...
	If PosA(ls_allcolumns, ls_findexp) = 0 Then
...
li_registeredcount = UpperBound ( inv_newupdate ) 
For li_cnt = 1 to li_registeredcount
	If LenA(Trim(inv_newupdate[li_cnt].s_updatetable)) > 0 Then
