...
li_pos1 = PosA ( ls_select, ls_dbname, 1 ) 
IF li_pos1 > 0 THEN 
	li_pos2 = lnv_string.of_lastpos ( ls_select, " ", li_pos1 ) 
	IF li_pos2 > 0 THEN 
		ls_owner = Trim (MidA ( ls_select, li_pos2, li_pos1 - li_pos2 )) 
	END IF
END IF 
...
FOR li_i = 1 to li_rcount
	...
	// Get the column type.
	ls_coltype = LeftA(inv_filterattrib.idw_dw.Describe ( ls_colname + ".ColType" ), 5)
	...
	Choose Case ls_coltype
		...
		//  DECIMAL DATATYPE
		Case "decim" 
			ls_expression = "Dec('" + ls_value + "')" 		//Added by Scofield on 2008-12-31			
		//---------End Added ------------------------------------------------------------------
...
this.Control[iCurrent+5]=this.mle_originalfilter
this.Control[iCurrent+6]=this.gb_originalfilter
this.Control[iCurrent+7]=this.cb_add
this.Control[iCurrent+8]=this.gb_newfilter
this.Control[iCurrent+9]=this.cb_ok
this.Control[iCurrent+10]=this.cb_dlghelp
end on
...
//$<reason> SetFocus (dw_filter) is unsupported. 
//SetFocus (dw_filter)
...
IF this.GetColumnName ( ) = "colvalue" THEN
	...
	Else
		// Clear all of the values.
		is_prevchildcolumn = ls_currentcolumn					//Added by Scofield on 2008-04-17
		idwc_values.Reset()
	END IF 
END IF 
...
End If
//--------------------------- APPEON BEGIN ---------------------------
//$<comment> 2005-09-12 By: Liang QingShi
//$<reason> SetFocus (dw_filter) is unsupported.
//SetFocus (dw_filter)
