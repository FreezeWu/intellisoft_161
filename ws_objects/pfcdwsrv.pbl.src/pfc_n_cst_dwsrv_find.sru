$PBExportHeader$pfc_n_cst_dwsrv_find.sru
...
//Check arguments.
If IsNull(as_find) or LenA(as_find)=0 or &
...
If IsNull(as_column) or LenA(Trim(as_column))=0 or &
	IsNull(as_find) or LenA(Trim(as_find))=0 or &
...
If ls_editstyle='dddw' or ls_editstyle='ddlb' Then
	If idw_requestor.Describe(as_colname+".ddlb.AllowEdit") = 'yes' or &
		...
		If ab_matchcase Then
			li_find_startpos = PosA (ls_find, as_find)
		Else			
			li_find_startpos = PosA (Lower(ls_find), Lower(as_find))
		End If
		li_find_selectedtextlen = LenA(as_find)			
...
//////////////////////////////////////////////////////////////////////////////
If LeftA(Lower(ls_coltype),4) = 'char' Then
...
If LeftA(Lower(ls_coltype),4) = 'char' Then
	If ls_editstyle="editmask" Then
		...
		li_maskedfind_textlen = LenA(ls_maskedfind)

		//Determine the "Masked Find" starting position and its length.
		li_count = 1
		For li_i = 1 to li_maskedfind_textlen
			If MidA(ls_find, li_count, 1) =  MidA(ls_maskedfind, li_i, 1) Then
...
If LeftA(ls_columnprotect, 1) = "'" or LeftA(ls_columnprotect, 1) = '"' Then
	ls_columnprotect = MidA(ls_columnprotect, 2)
	If RightA(ls_columnprotect, 1) = "'" or RightA(ls_columnprotect, 1) = '"' Then
		ls_columnprotect = MidA(ls_columnprotect, 1, LenA(ls_columnprotect)-1 )
	End If
End If
...
Else
	...
	ls_evaluate = 'Evaluate ("'+MidA(ls_columnprotect, 3)+'",'+string(al_row)+')'
...
Choose Case LeftA(Lower(ls_coltype),4)
	...
	Case 'char' 
		If of_Find(as_colname, inv_findattrib.is_find, al_row, al_row) = al_row Then
			...
			If (inv_findattrib.ib_matchcase) Then
				li_find_startpos = PosA(ls_find, inv_findattrib.is_find)
			Else
				li_find_startpos = PosA(Lower(ls_find),Lower(inv_findattrib.is_find))
			End If
			...
			If ls_editstyle='editmask' Then
				...
				ls_maskedfind = of_GetItem(al_row, as_colname)
				li_maskedfind_textlen = LenA(ls_maskedfind)
				li_maskedfind_startpos = PosA (ls_maskedfind, ls_selectedtext)
				li_maskedfind_selectedtextlen = LenA(ls_selectedtext)
				...
				For li_i = 1 to li_maskedfind_textlen
					If MidA(ls_find, li_count, 1) =  MidA(ls_maskedfind, li_i, 1) Then
						If li_i >= li_maskedfind_startpos And &
							...
							ls_maskedfind_selectedtext = ls_maskedfind_selectedtext + MidA(ls_find, li_count, 1)
			...
			If lb_replacechars Then
				...
				ls_find = ReplaceA(ls_find, li_find_startpos, LenA(inv_findattrib.is_find), &
