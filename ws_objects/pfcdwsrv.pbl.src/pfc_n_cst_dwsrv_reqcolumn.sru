$PBExportHeader$pfc_n_cst_dwsrv_reqcolumn.sru
...
IF PosA(Lower(is_skip), '~t'+Lower(as_colname)+'~t') > 0 Then Return 0
...
If PosA(Lower(ls_rc), 'yes') = 0 Then Return 0
...
For li_index = 1 to li_max
	If LenA (Trim(as_skipcolumn[li_index])) > 0 Then
...
For li_cnt = 1 to li_upper
	...
	If LenA(is_skipcolumn[li_cnt]) = 0 And li_slotavailable = 0 Then
...
li_upper = UpperBound(is_skipcolumn)
For li_cnt = 1 to li_upper
	If LenA(is_skipcolumn[li_cnt]) > 0 Then
