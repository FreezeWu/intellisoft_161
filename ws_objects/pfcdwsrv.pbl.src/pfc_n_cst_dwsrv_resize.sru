$PBExportHeader$pfc_n_cst_dwsrv_resize.sru
...
//modified detail height
boolean ib_setdetail = false
//$<add> 04.23.2008 by Andy
boolean ib_setheaderheight = false
//end add 04.23.2008
...
public function integer of_register (string as_method)
public subroutine of_setdetailheight (boolean ab_flag)
public subroutine of_setheaderheight (boolean ab_flag)
end prototypes
...
For li_cnt = 1 to li_upperbound
	...
	If LenA(inv_registered[li_cnt].s_control) > 0 Then
		...
		If LenA(ls_singlemodifyexp) > 0 Then
...
If LenA(ls_complexmodifyexp) > 0 Then
	ls_modifyret = idw_requestor.Modify(ls_complexmodifyexp)
	If IsNull(ls_modifyret) or LenA(ls_modifyret) > 0 Then	
		Return -1
	End If	
	//if ib_setdetail then modify detail.height
	if ib_setdetail and lr_ratioheight > 0 then
		idw_requestor.modify("datawindow.detail.height ='" + string(round(integer(idw_requestor.describe("datawindow.detail.height"))*lr_ratioheight,0)) + "'")
	end if
	//
	//$<add> 04.23.2008 by Andy
	//if ib_setheaderheight then modify Header.height
	if ib_setheaderheight and lr_ratioheight > 0 then
		
		idw_requestor.modify("datawindow.Header.height ='" + string(round(integer(idw_requestor.describe("datawindow.Header.height"))*lr_ratioheight,0)) + "'")
	end if
	//end add 04.23.2008
End If
...
//Check parameters
If IsNull(as_control) or LenA(Trim(as_control))=0 or IsNull(as_method) Then
	Return -1
End If
...
//Check parameters
If IsNull(as_control) or LenA(Trim(as_control))=0 or &
	IsNull(ab_scale) or &
...
For li_cnt = 1 to li_upperbound
	If IsNull(inv_registered[li_cnt].s_control) Or &
		LenA(Trim(inv_registered[li_cnt].s_control))=0 Then
...
//Check parameters
If IsNull(as_control) or LenA(Trim(as_control))=0 Then
	Return -1
End If
...
li_count = UpperBound(inv_registered)
For li_cnt = 1 to li_count
	If LenA(Trim(inv_registered[li_cnt].s_control)) > 0 Then
...
long ll_count,ll_i,ll_ret = 1
string ls_objects
string ls_name[]
n_cst_string lnv_string

ls_objects = idw_requestor.describe("datawindow.objects")
ll_count = lnv_string.of_parsetoarray( ls_objects , '~t', ls_name)

for ll_i = 1 to ll_count
	if of_Register(ls_name[ll_i], lb_scale, &
		li_movex, li_movey, li_scalewidth, li_scaleheight) <> 1 then
		ll_ret = -1
	end if
next
...
public subroutine of_setdetailheight (boolean ab_flag);//==============================APPEON BEGIN==========================
// Event: pfc_n_cst_dwsrv_resize.of_setdetailheight()
//--------------------------------------------------------------------
// Description: set ib_setdetail is true 
//--------------------------------------------------------------------
// Arguments:
...
ib_setdetail = ab_flag
end subroutine

public subroutine of_setheaderheight (boolean ab_flag);//==============================APPEON BEGIN==========================
// Event: pfc_n_cst_dwsrv_resize.of_setdetailheight()
//--------------------------------------------------------------------
// Description: set ib_setheaderheight is ab_flag 
//--------------------------------------------------------------------
// Arguments:
// 	value	boolean	ab_flag	
//--------------------------------------------------------------------
// Returns:  (none)
//--------------------------------------------------------------------
// Author:	Andy		Date: Apr 23,2008
//--------------------------------------------------------------------
// Modify History:
//	-------------------------------------------------------------------

ib_setheaderheight = ab_flag
