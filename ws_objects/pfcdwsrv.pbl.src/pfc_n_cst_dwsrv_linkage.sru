$PBExportHeader$pfc_n_cst_dwsrv_linkage.sru
...
For li_i = 1 to li_numlinks
	...
	If IsNull(ls_expression) or LenA(Trim(ls_expression))=0 Then Continue
...
If IsNull(as_event) or LenA(Trim(as_event))=0 Then Return -2
...
For li_i = 1 to li_numlinks
	...
	If IsNull(ls_coldata) or LenA(Trim(ls_coldata))=0 Then 
...
For li_i = 1 to li_upperbound
	...
	If Not LenA(Trim(ls_expression))=0 Then
		// Construct the Find expression.
		If (ls_find <> "") Then ls_find += " And "
		If PosA(Lower(ls_expression), 'null') > 0 Then
...
DO
	...
	//$<reason> It will Pop-up error in web.
	/*
		ll_row = idw_Requestor.Find (ls_find, ll_row, idw_Requestor.RowCount())
	*/
	If PosA(Lower(ls_find), 'null') > 0 Then
		ll_row = -1
	else
		ll_row = idw_Requestor.Find (ls_find, ll_row, idw_Requestor.RowCount())
	end if
...
// Validate arguments.
If IsNull(al_row) or al_row < 0 Then Return FAILURE
If IsNull(as_column) or LenA(Trim(as_column)) =0 Then Return FAILURE
...
// Verify passed arguments.
If IsNull(as_masterarg) or LenA(as_masterarg)=0 Then Return FAILURE
If IsNull(as_detailarg) or LenA(as_detailarg)=0 Then Return FAILURE
...
// Verify passed arguments.
If al_row<=0 or LenA(Trim(as_column))=0 or IsNull(as_column) Then Return ""
...
Choose Case Lower ( LeftA ( idw_master.Describe ( as_column + ".ColType" ) , 5 ) )
