$PBExportHeader$pfc_n_cst_dssrv_report.sru
$PBExportComments$PFC DataStore Reporting service
forward
global type pfc_n_cst_dssrv_report from n_cst_dssrv
end type
end forward

global type pfc_n_cst_dssrv_report from n_cst_dssrv
end type
global pfc_n_cst_dssrv_report pfc_n_cst_dssrv_report
...
For li_Cnt = 1 to li_NewLines
	If LenA(ls_Line[li_Cnt]) > li_LineWidth Then
		li_LineWidth = LenA(ls_Line[li_Cnt])
		li_Line = li_Cnt
	End if
Next
If LenA(ls_Text) < LenA(ls_Line[li_Line]) Then
...
For li_Count = 1 To li_NumObjects
	...
	If ids_requestor.Describe(ls_Objects[li_Count] + ".Band") = "header" And &
		RightA(ls_Objects[li_Count], 2) = is_defaultheadersuffix Then
...
// Print report
ids_requestor.Print(ab_CancelDialog)
...
If Abs(ai_YUnits) > 0 Then
	...
	Else	// Shift all bands
		li_CurrentHeight = Integer(ids_requestor.Describe("Datawindow.Header.Height"))
		...
		li_CurrentHeight = Integer(ids_requestor.Describe("Datawindow.Detail.Height"))
		...
		li_CurrentHeight = Integer(ids_requestor.Describe("Datawindow.Footer.Height"))
		...
		li_CurrentHeight = Integer(ids_requestor.Describe("Datawindow.Summary.Height"))
...
For li_Count = 1 To li_NumObjects
	...
	If ids_requestor.Describe(ls_Objects[li_Count] + ".type") = "line" Then
		// Lines have X1, Y1 and X2, Y2 attributes
		If abs(ai_XUnits) > 0 Then
			li_OriginalX = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".x1"))
			...
			li_OriginalX = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".x2"))
			li_X = li_OriginalX + ai_XUnits
		...
		End if
		If abs(ai_YUnits) > 0 Then
			li_OriginalY = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".y1"))
			...
			li_OriginalY = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".y2"))
			li_Y = li_OriginalY + ai_YUnits
	...
	Else	// all other objects have only one X and Y attribute
		If abs(ai_XUnits) > 0 Then
			li_OriginalX = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".x"))
			li_X = li_OriginalX + ai_XUnits
		...
		End if
		If abs(ai_YUnits) > 0 Then
			li_OriginalY = Integer(ids_requestor.Describe(ls_Objects[li_Count] + ".y"))
			li_Y = li_OriginalY + ai_YUnits
...
For li_Count = 1 To li_NumDWs
	...
	ids_requestor.DataObject = as_Datawindow[li_Count]
...
li_Return = ids_requestor.Create(ls_Modify)

//ids_requestor.SetRedraw(True)
...
Else
	ls_OldBorder = ids_requestor.Describe(as_Name + ".Border")
...
Else
	ls_Type = ids_requestor.Describe(as_Name + ".Type")
	ls_BackMode = ids_requestor.Describe(as_Name + ".Background.Mode")

	Choose Case ls_Type
		Case "line"
			ls_Color = ids_requestor.Describe(as_Name + ".Background.Color")
			If Not IsNull(al_BackColor) And Long ( ls_Color ) <> al_BackColor Then
				...
				End if
			End if
			ls_Color = ids_requestor.Describe(as_Name + ".Pen.Color")
		...
		Case "elipse", "rectangle", "roundrectangle"
			...
			If Not IsNull(al_BackColor) And Long ( ls_Color ) <> al_BackColor Then
				...
				End if
			End if
			ls_Color = ids_requestor.Describe(as_Name + ".Brush.Color")
			If Not IsNull(al_Color) And Long ( ls_Color ) <> al_Color Then
		...
		Case "column", "compute", "text"
			...
			If Not IsNull(al_BackColor) And Long ( ls_Color ) <> al_BackColor Then
				...
				End if
			End if
			ls_Color = ids_requestor.Describe(as_Name + ".Color")
			If Not IsNull(al_Color) And Long ( ls_Color ) <> al_Color Then
...
event destructor;call n_cst_dssrv::destructor;//////////////////////////////////////////////////////////////////////////////
//
//	Event:  Destructor
//
...
event constructor;call n_cst_dssrv::constructor;//////////////////////////////////////////////////////////////////////////////
//
//	Event:  Constructor
//
