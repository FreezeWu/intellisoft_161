$PBExportHeader$u_tabpg_dm_clause.sru
forward
global type u_tabpg_dm_clause from u_tabpg
end type
type pb_attrifilter from picturebutton within u_tabpg_dm_clause
end type
type cb_compare from commandbutton within u_tabpg_dm_clause
end type
type cb_go from commandbutton within u_tabpg_dm_clause
end type
type cb_collapseall from commandbutton within u_tabpg_dm_clause
end type
type cb_expandall from commandbutton within u_tabpg_dm_clause
end type
type cb_collapseitem from commandbutton within u_tabpg_dm_clause
end type
type cb_expanditem from commandbutton within u_tabpg_dm_clause
end type
type tv_1 from u_tvs within u_tabpg_dm_clause
end type
type dw_filter from u_dw_contract within u_tabpg_dm_clause
end type
type gb_1 from groupbox within u_tabpg_dm_clause
end type
type ds_category_unauthorized from n_ds within u_tabpg_dm_clause
end type
type ds_rely_temp from n_ds within u_tabpg_dm_clause
end type
type ds_rely_filtered from n_ds within u_tabpg_dm_clause
end type
type ds_clause_included from n_ds within u_tabpg_dm_clause
end type
type ds_rely from n_ds within u_tabpg_dm_clause
end type
type ds_folder_distinct from n_ds within u_tabpg_dm_clause
end type
type ds_folder_filtered from n_ds within u_tabpg_dm_clause
end type
type ds_folder from n_ds within u_tabpg_dm_clause
end type
type ds_clause from n_ds within u_tabpg_dm_clause
end type
end forward
...
global u_tabpg_dm_clause u_tabpg_dm_clause

type variables
w_dm_add_clause iw_atac
...
Long				il_from_template_id, il_ctx_id

DataStore		 ids_clause_rules_result  //added by gavins 20120723
end variables
...
public function integer of_get_rules_result (long al_clause_id)
end prototypes
...
ll_cnt2 =  ids_clause_tvi_map.Rowcount()
For i = 1 To ll_cnt
	ll_clause_id = iw_atac.dw_selected.GetItemNumber(i,'ctx_acp_clause_id')
	ll_find = ids_clause_tvi_map.Find('clause_id = ' + String(ll_clause_id),1,ll_cnt2) 
	If ll_find > 0 Then
		ll_tvi_handle = ids_clause_tvi_map.GetItemNumber(ll_find,'tvi_handle')
...
//<$Reason>Filter clauses based on dynamic adding rules
Long ll_from_template_id, ll_ctx_id
String ls_criteria, ls_clauseid[]
n_cst_dm_utils lnv_dm_utils //auto instance
n_cst_string_appeon lnv_string //auto instance
DataStore lds_clause_rules_result

IF iw_atac.Visible = TRUE THEN // Alfee 01.23.2008
//	lds_clause_rules_result = Create DataStore
//	lds_clause_rules_result.DataObject = 'd_dm_template_clause_rules_result'
	ids_clause_rules_result.Reset( )

	//Get template & contract id
	Select from_templete_id, ctx_id into :ll_from_template_id, :ll_ctx_id From ctx_am_document Where doc_id = :iw_atac.ii_doc_id And base_type = 1 ;
	il_from_template_id = ll_from_template_id //added by gavins 20120723
	il_ctx_id =  ll_ctx_id
	//Get Dynamic adding rules
...
//		lnv_dm_utils.of_get_clause_rule_reuslts(ll_ctx_id, ll_from_template_id, lds_clause_rules_result)
		lnv_dm_utils.of_get_clause_rule_reuslts(ll_ctx_id, ll_from_template_id, ids_clause_rules_result, false )//modified by gavins 20120726
		//Generate the filter based the rules' result//modified by gavins 20120723
		/*
		ll_cnt = lds_clause_rules_result.rowcount( )
		FOR i =1 to ll_cnt
		...
		IF Len(ls_criteria) > 0 THEN ls_filter += "and ctx_acp_clause_id not in (" + ls_criteria + ") "	
		*/
	END IF
END IF
...
//if ll_cnt < 1 then return -1
if ll_cnt < 1 then 
	if IsValid(inv_clause_comparison) then inv_clause_comparison.of_close_comparison_wizard('document') 
	return -1
end if
//--------End Modified------------------------------
...
//	inv_clause.of_create_clause_attribute( ll_clause_id, ll_cause_handle, tv_1) //Modified By Ken.Guo 2010-12-16  //Modified by gavins 20120723  move it to treeview's itempopulate
...
do while ll_find > 0
	...
	if ds_rely.getitemnumber(ll_find,"approval_status") <> il_clause_approved then 
		return false	
	end if		
...
inv_clause_comparison.of_start_compare_clauses( iw_atac, tv_1, 'document')
...
//
//====================================================================
string ls_updatecol[], ls_where
Long	ll_Find, ll_clause_id, li_result
...
lds_rules = gnv_data.ids_template_clause_rules

li_result = 1

If IsValid( ids_clause_rules_result ) And ids_clause_rules_result.RowCount( ) > 0 Then 

	ll_Find = ids_clause_rules_result.Find( "ctx_acp_template_id=" + String( il_from_template_id ) + " and ctx_id = " + string( il_ctx_id) + " and ctx_acp_clause_id = " +  string( al_clause_id ), 1, &
				ids_clause_rules_result.RowCount( ) )
	
	If ll_Find > 0 Then
//		gnv_data.of_get_data_from_cache('template_clause_rules', 'ctx_acp_template_id = ' + String(il_from_template_id) + ' and ctx_acp_clause_id = ' + String(al_clause_id), lds_rules) 
		lds_rules.SetFilter(  'ctx_acp_template_id = ' + String(il_from_template_id) + ' and ctx_acp_clause_id = ' + String(al_clause_id) )
		lds_rules.Filter( )
			
		li_result = f_ctx_find( il_ctx_id, lds_rules,ls_updatecol,'1',ls_where)
		lds_rules.SetFilter( "" )
		lds_rules.Filter( )
	End  If
...
this.Control[iCurrent+1]=this.pb_attrifilter
this.Control[iCurrent+2]=this.cb_compare
this.Control[iCurrent+3]=this.cb_go
this.Control[iCurrent+4]=this.cb_collapseall
this.Control[iCurrent+5]=this.cb_expandall
this.Control[iCurrent+6]=this.cb_collapseitem
this.Control[iCurrent+7]=this.cb_expanditem
this.Control[iCurrent+8]=this.tv_1
this.Control[iCurrent+9]=this.dw_filter
this.Control[iCurrent+10]=this.gb_1
end on

on u_tabpg_dm_clause.destroy
call super::destroy
...
ids_clause_rules_result = Create DataStore//added by gavins 20120723
ids_clause_rules_result.DataObject = 'd_dm_template_clause_rules_result'
...
//Get current selected folder or clause id
...
event itempopulate;call super::itempopulate;Long	ll_i, ll_Child
...
type ds_category_unauthorized from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_rely_temp from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_rely_filtered from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_clause_included from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_rely from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_folder_distinct from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_folder_filtered from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_folder from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
...
type ds_clause from n_ds within u_tabpg_dm_clause descriptor "pb_nvo" = "true" 
string dataobject = "d_dm_clause_all"
boolean ib_isupdateable = false
end type
