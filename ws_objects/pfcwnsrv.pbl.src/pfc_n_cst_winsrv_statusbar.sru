$PBExportHeader$pfc_n_cst_winsrv_statusbar.sru
...
public function string of_describe (string as_id, string as_attribute)
public subroutine of_set_statusbar_dw (datawindow adw_statusbar)
end prototypes
...
If ab_switch Then
	...
	li_rc = OpenWithParm(iw_statusbar, idw_Statusbar, iw_requestor)
	//---------End Modfiied ------------------------------------------------------
...
// Check arguments.
If IsNull(as_id) or LenA(trim(as_id))= 0 or  &
...
// Check arguments.
If IsNull(as_id) or LenA(trim(as_id))= 0 Then
	Return -1
End If
...
// Check arguments.
If IsNull(as_id) or LenA(trim(as_id))= 0 or IsNull(as_value) or &
...
// Check arguments.
If IsNull(as_id) or LenA(trim(as_id))= 0 or IsNull(as_value) Then
	Return -1
End If
...
// Check arguments.
If IsNull(as_modifyexp) or LenA(trim(as_modifyexp))= 0  Then
	Return -1
End If
...
//modify by gavins 20130510
If isvalid( idw_Statusbar ) then li_microhelpheight = idw_Statusbar.height
If li_microhelpheight <=0 Then li_microhelpheight = 58
//---------End Modfiied ------------------------------------------------------
...
For li_cnt = 1 to li_upper
	...
	If LenA(Trim(inv_dwobjects[li_cnt].s_id)) = 0 Then
		Continue
	End If
...
// Check arguments.
If IsNull(as_id) or Len(trim(as_id))= 0 or IsNull(as_attribute) Then
	Return ""
End If
...
// Find the ID and describe it.
For li_cnt = 1 to li_upper
	If as_id = inv_dwobjects[li_cnt].s_id Then
		If IsValid(iw_statusbar) Then
			ls_rc = iw_statusbar.of_describe( as_attribute )
		End If
		Return ls_rc
	End If
Next
...
public subroutine of_set_statusbar_dw (datawindow adw_statusbar);// (Appeon)Harry 01.21.2014 - V142 ISG-CLX Copy from CLX12.0
idw_Statusbar = adw_statusbar
end subroutine
