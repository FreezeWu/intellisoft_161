$PBExportHeader$pfc_n_cst_winsrv_preference.sru
...
Else
	...
	If IsNull(as_IniSection) Or LenA(Trim(as_IniSection)) = 0 Then
		Return -1
	End If
End If
...
For li_cnt = 1 To li_upperbound
	...
	//--------------------------- APPEON BEGIN ---------------------------
	//$<reason> The GetToolbar,GetToolbarPos method of window object(control) is unsupported.
	//$<reason> 
...
if LenA (am_source.ToolbarItemName) > 0 then
...
If LenA(ls_retvalue) > 0 Then
	...
	If ib_toolbaritemorder Then
		...
		If IsNumber(ls_retvalue) Then 
			...
			gnv_app.of_modify_menu_attr( am_source,'ToolbarItemOrder', Integer(ls_retvalue))
			//---------End Modfiied ------------------------------------------------------
		End If
	End If
	...
	If ib_toolbaritemspace Then
		...
		If IsNumber(ls_retvalue) Then 
			...
			gnv_app.of_modify_menu_attr( am_source,'ToolbarItemSpace', Integer(ls_retvalue))
			//---------End Modfiied ------------------------------------------------------
		End If
	End If
	...
	If ib_toolbaritemvisible Then
		...
		If Not IsNull(lb_visible) Then
			...
			gnv_app.of_modify_menu_attr( am_source,'ToolbarItemVisible', lb_visible)
			//---------End Modfiied ------------------------------------------------------
		End If
	End If
End if
...
For li_cnt = 1 to li_upperbound
	...
	//$<reason> The SetToolbar,SetToolbarPos method of window object(control) is unsupported.
