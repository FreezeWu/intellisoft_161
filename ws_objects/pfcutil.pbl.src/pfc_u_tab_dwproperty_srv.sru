$PBExportHeader$pfc_u_tab_dwproperty_srv.sru
...
If LenA(Trim(is_syntaxtappg)) > 0 Then
	...
	If li_upper > 0 Then
		...
		If LenA(iuo_tappage[li_upper].Text)=0 Then iuo_tappage[li_upper].Text = is_syntaxtext
	End If
End If
...
If IsNull(as_pages) or LenA(as_pages) = 0 Then
	Return -1
End If

// Loop and create the appropriate TabPages. --
Do While LenA(as_pages) > 0
	// Get a single page.
	ls_page = lnv_string.of_gettoken(as_pages, '~t')
	If LenA(ls_page) > 0 Then
		...
		If LenA(iuo_tappage[li_page].Text)=0 Then iuo_tappage[li_page].Text = 'UnAssigned'
...
If PosA(ls_text, 'syntax') > 0 Then
	...
	For li_cnt = 1 to li_upper
		...
		If ClassName(la_rc) = 'string' Then
			...
			If LenA(ls_syntax) > 0 And LenA(ls_tabpagesyntax) > 0 Then
				ls_syntax += '~r~n'
			End If
