$PBExportHeader$pfc_n_cst_sqlspy.sru
...
If LenA(is_logfile) = 0 or &
	LenA(as_newentry) = 0 Then
	Return 0
End If	
...
li_rc = GetFileSaveName  (TITLE, ls_path_filename, ls_filename, & 
				'SQL', 'SQL Files (*.SQL), *.SQL' )
gf_save_dir_path(ls_path_filename) //Added by Ken.Guo on 2009-03-10 
//---------End Modfiied ------------------------------------------------------------------
...
ll_startposition = LenA(al_newentry)

//Get the current length of the history string
ll_historylength = LenA(as_history)

If ll_historylength > TEST_LENGTH Then
	ll_pos = PosA(as_history, '/***', ll_startposition)
	If ll_pos > 0 Then
		as_history = MidA(as_history, ll_pos, ll_historylength)
	End If
End If
...
If PosA(Lower(ls_sqltype),Lower('Retrieve')) = 0  Then
