$PBExportHeader$pfc_u_tabpg_dwproperty_services.sru
...
For li_cnt = 1 To li_upper
	...
	//$<reason>  The system function 'Create' is unsupported.
	...
	If IsValid(lpo_object) Then
		...
		//---------------------------- APPEON END ----------------------------
			...
			If LenA(Trim(is_object[li_cnt])) = 0 Then is_object[li_cnt] = is_classname[li_cnt]
		...
		//--------------------------- APPEON BEGIN ---------------------------
		//$<comment> 2005-09-01 By: Liang QingShi
		//$<reason>  The system function 'Destroy' is unsupported.
		//Destroy lpo_object
...
li_upper = UpperBound(is_object)
For li_cnt = 1 to li_upper
	If LenA(is_object[li_cnt]) > 0 Then
...
If dw_services.Object.serviceenabled.Primary[ll_currentrow] = SERVICE_ENABLED Then
	...
	cb_property.Enabled = (LenA(ls_pages) > 0)
Else
	cb_property.Enabled = False
End If
...
If dwo.Name = 'serviceenabled' Then
	...
	If (data = SERVICE_ENABLED) Then
		...
		If IsNull(ls_servicename) or LenA(Trim(ls_servicename)) = 0 Then Return
