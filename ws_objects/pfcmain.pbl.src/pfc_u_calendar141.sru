$PBExportHeader$pfc_u_calendar141.sru
$PBExportComments$PFC Calendar class
forward
global type pfc_u_calendar141 from u_base
end type
type dw_cal from u_dw within pfc_u_calendar141
end type
end forward
...
global pfc_u_calendar141 pfc_u_calendar141
...
// Validate argument.
If IsNull(as_format) or LenA(Trim(as_format))=0 Then
	Return -1
End If
...
// Check arguments
If IsNull(as_dwcolumn) Or LenA(Trim(as_dwcolumn))=0 Then 
	Return False
End If
...
// Check arguments
If (IsNull(as_dwcolumn) Or LenA(Trim(as_dwcolumn))=0) Or &
...
If of_IsDateType(ls_coltype) Then
	...
	For li_cnt = 1 to li_upperbound
		If IsNull(is_dwcolumns[li_cnt]) or LenA(Trim(is_dwcolumns[li_cnt])) = 0 Then
...
For li_i=1 to li_colcount
	...
	End If
Next
If LenA(ls_modify) > 0 Then idw_requestor.modify(ls_modify)
...
For li_cnt = 1 to li_upper
	If as_dwcolumn = is_dwcolumns[li_cnt] Then 
		...
		If LenA(is_dwcolumnsexp[li_cnt]) > 0 Then
...
For li_i=1 To li_upper
	If LenA(is_dwcolumns[li_i]) > 0 Then
		li_cnt ++
...
li_upper = UpperBound(is_dwcolumns)
For li_cnt = 1 to li_upper
	If LenA(is_dwcolumns[li_cnt]) > 0 Then
...
if LenA(Trim(ls_date)) = 0 or IsNull(ls_date) or NOT IsDate( String ( Date ( ls_date ) ) ) or NOT IsDate( ls_date ) then
//---------------------------- APPEON END ----------------------------
...
If li_upperbound > 0 Then
	...
	For li_loop = 1 to li_upperbound
		...
		End If
	Next
	If LenA(Trim(ls_modifyexp)) > 0 Then
		dw_cal.Modify(ls_modifyexp)
	End If
End If
...
If ab_setrequestor Then
	...
	If IsValid(idw_requestor) Then
		...
		if isvalid(w_view_audit) then w_view_audit.Post wf_reset_st_run( ) //(Appeon)Harry 11.25.2013 - V141 for BugH110601 of History Issues
	ElseIf IsValid(iem_requestor) Then
...
If LenA(Trim(is_prevcell)) > 0 Then
...
//location 
ll_p1 = PosA( ls_syntax  , "column(" )
ll_p2 = PosA( ls_syntax  , "~n" , ll_p1 + 1 )
Do While ll_p1 > 0
	ls_temp = MidA( ls_syntax, ll_p1 , ll_p2 - ll_p1  )
	ll_location_first = ll_p1
	ll_location_last = ll_p2
	
	ll_pos = PosA( ls_temp  , "name=" + as_colname +" " )
	If ll_pos > 0 Then
		ls_colstring = ls_temp
		Exit
	End If
	
	ll_p1 = PosA( ls_syntax  , "column(" ,ll_p2 + 1)
	ll_p2 = PosA( ls_syntax  , "~n" , ll_p1 + 1 )
Loop

//convert 
ls_coldetail = ""
ls_coldetail = MidA(ls_colstring,8,LenA(ls_colstring))
ls_coldetail = MidA(ls_coldetail,1,LenA(ls_coldetail) - 2)

ls_coldetail = Lower(ls_coldetail)

//delete the edit style property
If PosA(ls_coldetail,'edit.') > 0 Then
	lnv_string.of_parsetoarray(ls_coldetail, ' ', ls_detail)
	For I = 1 To UpperBound(ls_detail)
		If PosA( ls_detail[I],"edit.") > 0 Then
			ls_detail[I] = ""
		End If
	Next
End If
...
If PosA(ls_coldetail,'editmask.') > 0 Then
	lnv_string.of_parsetoarray(ls_coldetail, ' ', ls_detail)
	For I = 1 To UpperBound(ls_detail)
		If PosA( ls_detail[I],"editmask.") > 0 Then
			ls_detail[I] = ""
		End If
	Next
End If
...
//change to ddlb
If PosA(ls_coldetail,'ddlb.') <= 0 Then
	ls_coldetail += ls_modify
End If

ls_colstring = "column(" + ls_coldetail + ")"
ls_colstring = lnv_string.of_globalreplace(ls_colstring,'"', "'")
...
If ls_dw_style = "1" Then
	ll_pos = PosA( ls_syntax  , "name=" + as_colname +"_t " )
	If ll_pos > 0 Then
		ll_pp2 = PosA(ls_syntax ,"~n" , ll_pos + 1)
		
		ll_pp1 = ll_pp2
		Do While ll_pp1 = ll_pp2
			ll_pp1 = PosA(ls_syntax ,"~n" , ll_pos - 1)
			ll_pos = ll_pos - 1
		Loop
		ls_temp = MidA(ls_syntax, ll_pp1 , ll_pp2 - ll_pp1)
