...
if AppeonGetClientType() = 'WEB' then
	this.DBMS = gs_CacheType
	this.DBParm = gs_CacheSet
end if
//---------------------------- APPEON END ----------------------------
...
if this.SqlErrText <> "" then
	li_pos1 = PosA (this.SqlErrText, "=") +2
	li_pos2 = (PosA (this.SqlErrText, "[") -2) - li_pos1
	ls_temp = MidA (this.SqlErrText, li_pos1,  li_pos2)
end if
...
li_keywordpos = PosA (ls_dbms, KEYWORD)
...
if ib_Trace then
	...
	this.dbms = MidA (this.dbms, li_keywordpos + LenA (KEYWORD), LenA (this.dbms))
end if
...
//$<reason> The Lock property of the Transaction object is unsupported by Appeon.
...
// Check arguments
if IsNull (as_registrykey) or LenA (Trim (as_registrykey)) =0 then	return -1
...
//$<reason> The fourth sql syntax is unsupported.
...
ls_sqlstatement = "SELECT DISTINCT Convert(char(255)," + as_column + ") FROM " + as_table
//---------------------------- APPEON END ----------------------------
...
Declare c_values_cursor dynamic cursor For sqlsa;
prepare sqlsa from :ls_sqlstatement using this;
Open dynamic c_values_cursor;
Fetch c_values_cursor Into :ls_value;
do while this.SQLCode = 0 
	as_values[UpperBound(as_values)+1] =  Trim(ls_value)
	Fetch c_values_cursor Into :ls_value;
	ll_rc = this.SQLCode
loop
close c_values_cursor;
...
if LenA (ls_connectstring) <= 0 then
	// No connectstring defined
	if LenA (this.dbparm) > 0 then
		ls_connectstring = ","
	end if
...
//--------------------------- APPEON BEGIN ---------------------------
//$<comment> 2005-08-30 By: Xie Hanwen
//$<reason> The SQLReturnData property  of the Transaction object is unsupported.
