...
If Lower(as_Attribute) = "label" Then
	...
	Else
		If PosA(ls_Source, ls_Target) > 0 Then Return True
	End If
...
if IsNull (ai_level) or ai_level <= 0 or &
	LenA (as_datawindow) = 0 or IsNull (as_datawindow) or &
...
if lower(LeftA(ls_rc,4)) <> "char" then
	inv_ds[ai_Level].s_DataWindow = ""
	Destroy inv_ds[ai_Level].ds_obj
	Return -4
End if
...
ls_Return = " ~t" + LeftA(ltvi_Item.Label,55) + "~t" + as_Picts
...
If li_Index = 0 Then Return -5 //Added By Ken.Guo 2009-10-29.
//---------End Added ------------------------------------------------------------------
...
For li_Cnt = (ai_Level - 1) To 1 Step - 1
	...
	For li_Cnt2 = 1 To li_Limit
		If li_RetrieveArgLevel[li_Cnt2] = li_Cnt Then
			...
			Choose Case LeftA(inv_ds[li_ArgIndex].ds_obj.Describe(ls_ArgCol + ".coltype"), 7)
				Case "integer", "long"
