$PBExportHeader$pfc_u_ddplb.sru
$PBExportComments$PFC DropDownPictureListBox class
forward
...
global pfc_u_ddplb pfc_u_ddplb
...
// Select all text
Return this.SelectText (1, LenA (this.text))
end event
...
/*
if LenA (this.SelectedText()) > 0 then
...
if Len (this.SelectedText()) > 0 then
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_copy,'enabled', true)
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_cut,'enabled', true)
else
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_copy,'enabled', false)
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_cut,'enabled', false)
end if

if Len (ClipBoard()) > 0 then
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_paste,'enabled', true)
else
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_paste,'enabled', false)
end if

if Len (this.text) > 0 then
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_selectall,'enabled', true)
else
	gnv_app.of_modify_menu_attr( lm_edit.m_edititem.m_selectall,'enabled', false)
end if
//---------End Modfiied ------------------------------------------------------
...
if li_searchindex > 0 then
	...
	if li_searchtextlen = LenA(ls_foundtext) THEN	
		...
		this.SelectText (LenA (ls_foundtext)+1, 0)	
	else
		// Highlight the portion the user has not actually typed.		
		this.SelectText (li_searchtextlen + 1, LenA (ls_foundtext))
	end if
end if
