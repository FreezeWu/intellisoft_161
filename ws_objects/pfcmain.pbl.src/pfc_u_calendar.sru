$PBExportHeader$pfc_u_calendar.sru
...
Integer	ii_MaxDay
String		is_MaxCell, is_MinCell
Date		id_Today, id_Pre_Today
...
public function integer of_convert_to_ddlb (u_dw aw_parm, string as_colname)
public function string of_register2 (string as_dwcolumn, integer ai_style)
public function integer of_reset_flag ()
end prototypes
...
If of_IsDateType(ls_coltype) Then
	...
	If ai_style = DDLB Or ai_style = DDLB_WITHARROW Then	
		...
		//Commented By Ken.Guo 12/16/2011
//		//--------------------------- APPEON BEGIN ---------------------------
//		//$<add> 01.16.2006 By: Luke
//		//$<reason> Conver column to ddlb in web.
//		If idw_requestor.dataobject = "" Then
//			if of_convert_to_ddlb(idw_requestor,as_dwcolumn) = -1 then
//				Messagebox("Error","Convert colunn style failed! ")
//			end if
//		End If
//		//---------------------------- APPEON END ----------------------------	
...
For li_i=1 to li_colcount
	...
	//--------------------------- APPEON BEGIN ---------------------------
	//$<modify> 01.16.2006 By: Luke
	//$<reason> The return value of function describe is difference between 
	//$<reason> PB and APB. If the column is empty "Describe ("#"+string(li_i)+".Edit.Style")"
	//$<reason> will return '?' in PB but return '!' in APB.
	...
	If ls_editstyle = 'ddlb' or ls_editstyle='edit' or ls_editstyle='editmask' Then
		If of_IsDateType(ls_coltype) Then
			...
			ls_modify = ls_modify + of_Register2(ls_colname, ai_style)
		End If
	End If
Next
If len(ls_modify) > 0 Then idw_requestor.modify(ls_modify)
...
if Len(Trim(ls_date)) = 0 or IsNull(ls_date) or NOT IsDate( String ( Date ( ls_date ) ) ) or NOT IsDate( ls_date ) then
//---------------------------- APPEON END ----------------------------
...
Else
	...
	// attribute.
	of_SetDate( id_Today, ib_initialvalue)
End If
dw_cal.Modify( "t_today.Text = 'Today: " + String( id_Today, 'mm/dd/yyyy' ) + "'" )
id_Pre_Today = id_Today
Return 1
end function
...
Integer	li_Max_Pre_Day,li_Today//added by gavins 20130102
String		ls_ModifyNonCurrent, ls_ModifyCurrent , ls_TodayCell
...
//--------------------------- APPEON BEGIN ---------------------------
//$<modify> 2005-09-12 By: Liang QingShi
//$<reason> SetRedraw(dw_cal,FALSE) is unsupported.
...
//Set the Title.
//ls_monthname = inv_datetime.of_MonthName(li_month)  //modified by gavins 20120102
dw_cal.Object.month[1] = li_Month
dw_cal.Object.year[1] =  li_year
...
is_MinCell = String( li_FirstDayNum )
li_Max_Pre_Day = Day( Relativedate( Date(li_year, li_month, 1 ), -1 ) )
li_Max_Pre_Day = li_Max_Pre_Day - li_FirstDayNum + 1
ls_ModifyNonCurrent = ''
//Blank cells prior to the first day of the month.
For li_loop = 1 to li_FirstDayNum - 1
//	dw_cal.SetItem(1,li_loop,"")//modified by gavins 20130102
	ls_ModifyNonCurrent = ls_ModifyNonCurrent + " cell" + string( li_loop ) + ".color=8421504"
	dw_cal.SetItem(1,li_loop, string( li_Max_Pre_Day + li_loop  )    )
Next
ls_ModifyCurrent = ''
...
For li_loop = 1 to li_daysinmonth
	...
	If li_loop = li_Today Then	ls_TodayCell = ' cell' + string( li_daycount )		
	ls_ModifyCurrent = ls_ModifyCurrent + " cell"+string(li_daycount)+".Color='"+string(il_fontcolor)+"' "
Next

ii_MaxDay = li_daysinmonth
is_MaxCell = string( li_daycount )
//Blank cells after the last day of the month.
For li_loop = li_daycount +1 to 42 
//	dw_cal.SetItem(1,li_loop,"") //modified by gavins 20130102
	ls_ModifyNonCurrent = ls_ModifyNonCurrent + " cell" + string( li_loop ) + ".color=8421504"
	dw_cal.SetItem(1,li_loop,  String( li_loop - li_daycount ))
Next

// Restore all cells back to default color and fontweight.
ls_modifyexp = ''
For li_loop = 1 to 42
	ls_modifyexp += "cell"+string(li_loop)+".Font.Weight='"+string(ii_normalfontweight)+"' "// "cell"+string(li_loop)+".Color='"+string(il_fontcolor)+"' " + &
Next
...
Next
dw_cal.Modify( ls_ModifyCurrent )
dw_cal.Modify(ls_modifyexp)
...
dw_cal.Modify( ls_ModifyNonCurrent )
If Year( id_Today ) = li_Year And Month( id_Today ) = li_Month Then//added by gavins 20120102
	dw_cal.Modify( "r_today.visible=1 r_today.x =" + string( Long( dw_cal.describe( ls_Todaycell + ".x" )) - 5 ) + " r_today.y=" + string( Long( dw_cal.describe( ls_Todaycell + ".y" )) -5 ) + " r_today.width=" + string( Long( dw_cal.describe( ls_Todaycell + ".width" )) + 8 ) +  " r_today.height=" + string( Long( dw_cal.describe( ls_Todaycell + ".height" )) + 8 ) ) 
Else
	dw_cal.Modify( "r_today.visible=0" ) 
End If
//Turn back redraw.
...
If ab_setrequestor Then
	...
	If IsValid(idw_requestor) Then
		...
		//idw_requestor.AcceptText() //Added by Ken.Guo on 2009-01-21  //Commented by (Appeon)Stephen 09.01.2014 - IC issue - date and saving in for mail log in
		if isvalid (w_view_audit) and idw_requestor.dataobject = "d_select_from_thru_date" then w_view_audit.Post wf_reset_st_run() //(Appeon)Harry 10.16.2014 - for BugH101301
	ElseIf IsValid(iem_requestor) Then
...
If (Year(ad_date) <> Year(id_prevdate) Or Month(ad_date) <> Month(id_prevdate)) Or &
	ib_alwaysredraw Or (ad_date = date(1900, 1, 1)) Or id_Pre_Today <> id_Today Then
...
//If dw_cal.Modify(ls_newcell + ".border=5") <> "" Then //modified by gavins 20130102     14898176
//	li_rc = -1
//End if
ls_Color =  dw_cal.describe( is_prevcell + ".color" )
If ls_Color = '16777215' Then ls_Color = '0'
If dw_cal.Modify(is_prevcell + ".background.color=16777215  "  + is_prevcell + ".color=" + ls_Color ) <> "" Then
	li_rc = -1
End if
If dw_cal.Modify(ls_newcell + ".background.color=11632216  "  + ls_newcell + ".color=16777215" ) <> "" Then
	li_rc = -1
End if
...
public function integer of_convert_to_ddlb (u_dw aw_parm, string as_colname);//Commented By Ken.Guo 12/16/2011. Error Function when the date format include time, e.g. mm/dd/yyyy hh:mm:ss
Return 1

////////////////////////////////////////////////////////////////////////
//// $<function>of_convert_to_ddlb()
//// $<arguments> aw_parm  datawindow need covert column
//// $<arguments> as_colname coloname that need convert. 
//// $<returns> integer  1 - success -1 - error
//// $<description> The convert column style is not supported by APB,
//// $<description> workaround by modified the syntax of datawindow.
////////////////////////////////////////////////////////////////////////
//// $<add> 01.16.2006 by Luke
////        01.16.2006 by Luke: add process of modified text of column  in grid
////////////////////////////////////////////////////////////////////////
//
//String ls_syntax,ls_temp,ls_colstring,ls_modify,ls_coldetail,ls_detail[]
//String ls_editstyle
//Long ll_pos,ll_location_first,ll_location_last ,I
//Long ll_p1 ,ll_p2
//n_cst_string lnv_string
//
//String ls_dw_style
//
//ls_dw_style = aw_parm.Describe ("DataWindow.Processing")
//ls_editstyle = aw_parm.Describe (as_colname+".Edit.Style")
//
////check column style
//If Lower(ls_editstyle) = 'ddlb' Then Return 1
//
////prepare 
//ls_modify = " ddlb.limit=0 ddlb.allowedit=yes ddlb.case=any ddlb.useasborder=no ddlb.imemode=0 "
//ls_syntax = aw_parm.Describe("datawindow.syntax")
//
////location 
//ll_p1 = Pos( ls_syntax  , "column(" )
//ll_p2 = Pos( ls_syntax  , "~n" , ll_p1 + 1 )
//Do While ll_p1 > 0
//	ls_temp = Mid( ls_syntax, ll_p1 , ll_p2 - ll_p1  )
//	ll_location_first = ll_p1
//	ll_location_last = ll_p2
//	
//	ll_pos = Pos( ls_temp  , "name=" + as_colname +" " )
//	If ll_pos > 0 Then
//		ls_colstring = ls_temp
//		Exit
//	End If
//	
//	ll_p1 = Pos( ls_syntax  , "column(" ,ll_p2 + 1)
//	ll_p2 = Pos( ls_syntax  , "~n" , ll_p1 + 1 )
//Loop
//
////convert 
//ls_coldetail = ""
//ls_coldetail = Mid(ls_colstring,8,Len(ls_colstring))
//ls_coldetail = Mid(ls_coldetail,1,Len(ls_coldetail) - 2)
//
//ls_coldetail = Lower(ls_coldetail)
//
////delete the edit style property
//If Pos(ls_coldetail,'edit.') > 0 Then
//	lnv_string.of_parsetoarray(ls_coldetail, ' ', ls_detail)
//	For I = 1 To UpperBound(ls_detail)
//		If Pos( ls_detail[I],"edit.") > 0 Then
//			ls_detail[I] = ""
//		End If
//	Next
//End If
//lnv_string.of_arraytostring(ls_detail,' ',False, ls_coldetail)
//
////delete the editmask style property
//If Pos(ls_coldetail,'editmask.') > 0 Then
//	lnv_string.of_parsetoarray(ls_coldetail, ' ', ls_detail)
//	For I = 1 To UpperBound(ls_detail)
//		If Pos( ls_detail[I],"editmask.") > 0 Then
//			ls_detail[I] = ""
//		End If
//	Next
//End If
...
//
////change to ddlb
//If Pos(ls_coldetail,'ddlb.') <= 0 Then
//	ls_coldetail += ls_modify
//End If
//
//ls_colstring = "column(" + ls_coldetail + ")"
////ls_colstring = lnv_string.of_globalreplace(ls_colstring,'"', "'") //Commented By Ken.Guo 12/12/2011. if property expression include single quote, then modify function execute failed.  
//
//ls_colstring = "destroy "+ as_colname + " create " + ls_colstring+ ""
//
////if the style of datawindow is grid, need rebuild text for it.Limited: the text columne
////must named by columne + "_t"
//long  ll_pp1,ll_pp2
//
//If ls_dw_style = "1" Then
//	ll_pos = Pos( ls_syntax  , "name=" + as_colname +"_t " )
//	If ll_pos > 0 Then
//		ll_pp2 = Pos(ls_syntax ,"~n" , ll_pos + 1)
//		
//		ll_pp1 = ll_pp2
//		Do While ll_pp1 = ll_pp2
//			ll_pp1 = Pos(ls_syntax ,"~n" , ll_pos - 1)
//			ll_pos = ll_pos - 1
//		Loop
//		ls_temp = Mid(ls_syntax, ll_pp1 , ll_pp2 - ll_pp1)
//
//		//prepare the return value
//		ls_colstring = ls_colstring + " destroy "+ as_colname + "_t create " + ls_temp + ""
//	End If
//End If  //end for change text of column
//
//
//
//String lr
//lr = aw_parm.Modify(ls_colstring)
//If Len(lr) > 0 Then
//	MessageBox("error","of_convert_to_ddlb error!" + ls_colstring)
//	Return -1
//End If
//Return 1
//
...
public function integer of_reset_flag ();//--------------------------- APPEON BEGIN ---------------------------
//$<add> 09.06.2006 By: LeiWei
//$<reason> Fix a defect.
//$<reason> Set the following variables to NULL when the datawindow re-created.

STRING LS_TMP[]
INTEGER LI_TMP[]

is_dwcolumns[] = LS_TMP[]
is_dwcolumnsexp[] = LS_TMP[]
ii_dwcolumnstyle[] = LI_TMP[]
...
//Modified By Ken.Guo 05/16/2012. does not want tigger the itemchanged event when click nect month
//Set the new date.
//of_SetDate (date(li_year, li_month, li_Day), True)
of_SetDate (date(li_year, li_month, li_Day), False)

Return 1
end event

event type integer pfc_prevmonth();Integer li_month
Integer li_year
Integer li_day
...
If dwo.Name = 'year' Then
	//---------Begin Modified by (Appeon)Stephen 09.02.2014 for BugS081901--------
	//dw_cal.Modify( "year.protect=0 year.background.color=16777215 year.EditMask.Spin=yes year.EditMask.AutoSkip=yes year.color=0" )
	If AppeonGetClientType() = 'PB' Then
		dw_cal.Modify( "year.protect=0 year.background.color=16777215 year.EditMask.Spin=yes year.EditMask.AutoSkip=yes year.color=0" )
	else
		dw_cal.Modify( "year.protect=0 year.background.color=16777215 year.EditMask.Spin=yes year.EditMask.AutoSkip=no year.color=0" )
	end if
	//---------End Modfiied ------------------------------------------------------	
Else
	dw_cal.Modify( "year.protect=1 year.background.color=11632216 year.EditMask.Spin=no year.color=16777215" )
End If
If dwo.Name = 'month' Then
	dw_cal.Modify( "month.protect=0 month.background.color=16777215 month.color=0" )
Else
	dw_cal.Modify( "month.protect=1 month.background.color=11632216 month.color=16777215" )
End If
If dwo.Name = 't_today' Or dwo.Name = 'r_1' Then
	of_SetDate (id_today, false )
End If
...
lb_SetDate = True
If Integer( is_MaxCell ) < Integer( Mid(ls_clickedcolumn, 5 ) ) Then//modified by gavins 20130102
	...
	End If
	lb_SetDate = False
ElseIf Integer( is_MinCell ) >  Integer( Mid(ls_clickedcolumn, 5 ) ) Then
	If li_Month - 1 < 1 Then
		li_year = li_year - 1
		li_Month = 12
	Else
		li_Month = li_Month - 1
...
//Set the new date. 
of_SetDate (date(li_year, li_month, li_Day), lb_SetDate)

If Not lb_SetDate Then Return
...
event itemchanged;call super::itemchanged;Integer	li_Year, li_Month , li_Day, li_daysinmonth
Integer	li_days[12]={31,28,31,30,31,30,31,31,30,31,30,31}



If dwo.Name = 'year' Or dwo.Name = 'month' Then
	If dwo.Name = 'month' Then
		li_Month = Integer( Data )
		li_Year = Year( id_date )
	Else
		li_Year = Integer( Data )
		li_Month = Month( id_date )
	End If
	li_Day = Day( id_Date )
	If li_Year < 1900 Or li_Year > 9999 Then Return 2
	li_daysinmonth = li_days[li_month]
	// Check for a leap year.
	If li_month=2 Then
		If ( (Mod(li_year,4) = 0 And Mod(li_year,100) <> 0) Or (Mod(li_year,400) = 0) ) Then
		...
		End If
	End If
	If li_Day > li_daysinmonth Then li_Day = li_daysinmonth
	of_SetDate (date(li_year, li_month, li_Day), False )	
	id_Date = Date( li_Year, li_Month , li_Day )
	dw_cal.Modify( "month.protect=1 month.background.color=11632216 month.color=16777215" )
End If
end event

event editchanged;call super::editchanged;If dwo.Name = 'year' and row > 0 Then
	If Len( data ) = 4 And IsNumber( data ) Then 	dw_cal.Event ItemChanged( row, dwo, data )	
End If
end event
