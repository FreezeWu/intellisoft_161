$PBExportHeader$n_cst_string_appeon.sru
$PBExportComments$PFC String service
forward
global type n_cst_string_appeon from nonvisualobject
end type
end forward

global type n_cst_string_appeon from nonvisualobject autoinstantiate
end type
...
public function long of_longarraytostring (long al_source[], string as_delimiter, boolean ab_processempty, ref string as_ref_string)
public function long of_datearraytostring (date ad_source[], string as_delimiter, boolean ab_processempty, ref string as_ref_string)
public function long of_parsetolangarray (string as_source, string as_delimiter, ref long al_array[])
public function long of_parsetodatearray (string as_source, string as_delimiter, ref date ad_array[])
public function integer of_parsefilename (string as_pathname, ref string as_filename, ref string as_filepath)
public function string of_parse_incriteria (string as_criteria, string as_type)
public function string of_parse_quote (string as_criteria, string as_type)
public function string of_format_phonenumber (string as_phonenumber)
public function integer of_arraytostring (string as_source[], string as_mark, ref string as_delimiter, ref string as_ref_string)
public function integer of_replace_env_var (ref string as_pathname)
public subroutine of_insert_to_array (ref string as_array[], long ai_into_index, string as_value, boolean ab_usefind)
public subroutine of_sortarray (ref datetime adt_value[], string as_sorttype)
public subroutine of_sortarray (ref decimal adec_value[], string as_sorttype)
public subroutine of_sortarray (ref long al_value[], string as_sorttype)
public subroutine of_sortarray (ref string as_value[], string as_sorttype)
end prototypes
...
/////////////////////////////////////////////////////////////////////////////////
ll_pos = PosA(as_source, as_separator)	
...
if ll_pos = 0 then
	...
	ls_ret = MidA(as_source, 1, ll_pos - 1)
	as_source = RightA(as_source, LenA(as_source) - (ll_pos+LenA(as_separator)-1) )
end if
...
ls_return = space(al_length - LenA(as_Source)) + as_source
...
ls_return = as_source + space(al_length - LenA(as_Source))
...
do while ll_count<ll_length
	...
	li_ascii = AscA (lc_char[ll_count])
...
ll_Len = LenA(as_Target)
ll_Count = 0

ll_Pos = PosA(as_source, as_Target)

Do While ll_Pos > 0
	ll_Count ++
	ll_Pos = PosA(as_source, as_Target, (ll_Pos + ll_Len))
Loop
...
ElseIf ab_remove_nonprint Then
	// Remove nonprintable characters from the beginning of a string.
	do while LenA (as_source) > 0 and not lb_printable_char
		lc_char = as_source
...
do while not lb_done
	li_keyword = PosA (Lower(as_source), Lower(as_keyword))
	if li_keyword > 0 then
		...
		li_equal = li_keyword - LenA(as_separator)
		If li_equal > 0 Then
			// not the start so see if this is a compound occurance separated by the separator string
			ls_exact = MidA(ls_source, li_equal, LenA(as_separator))  
		...
		if LeftA(as_source, 1) = "=" then
			li_separator = PosA (as_source, as_separator, 2)
			if li_separator > 0 then
				ls_keyvalue = MidA(as_source, 2, li_separator - 2)
			else
...
do 
	li_keyword = PosA (Lower(as_source), Lower(as_keyword), li_keyword + 1)
	if li_keyword > 0 then
		ls_temp = LeftTrim (RightA (as_source, LenA(as_source) - (li_keyword + LenA(as_keyword) - 1)))
		if LeftA (ls_temp, 1) = "=" then
			li_equal = PosA (as_source, "=", li_keyword + 1)
			li_separator = PosA (as_source, as_separator, li_equal + 1)
			if li_separator > 0 then
				as_source = LeftA(as_source, li_equal) + as_keyvalue + as_separator + RightA(as_source, LenA(as_source) - li_separator)
			else
				as_source = LeftA(as_source, li_equal) + as_keyvalue
			end if
			li_found = 1
		end if
	end if
...
FOR ll_loop = 1 TO ll_source_len
	...
	if of_IsPrintable(lch_char) then
		...
		ls_source = ReplaceA(ls_source, ll_pos, 1, "")
	end if 
NEXT
...
if IsNull(as_source) or LenA(as_source)=0 then
	//String is empty
	Return True
end if
...
If Not ab_processempty Then
	For ll_Count = 1 to ll_ArrayUpBound
		// Do not include any entries that match an empty string 
		If string(al_source[ll_Count]) <> '' Then
			If LenA(as_ref_string) = 0 Then
				//Initialize string
...
Do While ll_Pos > 0
	...
	// Update array and counter
	ll_Count ++
	al_Array[ll_Count] = long(ls_holder)
...
Do While ll_Pos > 0
	...
	// Update array and counter
	ll_Count ++
	ad_Array[ll_Count] = date(ls_holder)
...
ll_pos = PosA(as_pathname , '\')
If ll_pos > 0 Then
	Do While ll_pos > 0
		ls_filepath	= ls_filepath + LeftA(as_pathname , ll_pos)
		as_pathname = MidA(as_pathname , ll_pos + 1)
		ll_pos = PosA(as_pathname , '\')
	Loop
	If LenA(as_pathname) > 0 Then as_filename = as_pathname
...
public function string of_parse_incriteria (string as_criteria, string as_type);//Parse the criteria for 'IN' operation - Alfee 07.25.2008
Long ll_pos
String ls_criteria

IF IsNUll(as_criteria) OR LenA(Trim(as_criteria)) < 1 THEN RETURN ""
ls_criteria = as_criteria

//Check and Add Brackets if needed
IF LeftA(Trim(ls_criteria), 1) <> "(" THEN ls_criteria = "(" + ls_criteria 
IF RightA(Trim(ls_criteria), 1) <> ")" THEN ls_criteria += ")"

//Check and Add Quotes if needed
CHOOSE CASE Upper(as_type)
	CASE 'C', 'D' //for char, datetime type
		IF PosA(ls_criteria, "'") > 0 THEN 
			//Return if any single qutoes already included 
			RETURN ls_criteria
		ELSE
			//Replace duoble quote to single quote
			ll_pos = PosA(ls_criteria, '"')
			DO WHILE ll_pos > 0 
				ls_criteria = ReplaceA(ls_criteria, ll_pos, 1, "'")
				ll_pos = PosA(ls_criteria, '"')
			LOOP

			//Return if any single qutoes already included - Alfee 09.17.2008
			IF PosA(ls_criteria, "'") > 0 THEN RETURN ls_criteria		

			//Add quotes at the begining and end of the string
			ls_criteria = Trim(ls_criteria)
			ll_pos = PosA(ls_criteria, "(")
			IF ll_pos > 0 THEN ls_criteria = ReplaceA(ls_criteria, ll_pos, 1, "('")
			ll_pos = LastPos(ls_criteria, ")")						
			IF ll_pos > 0 THEN ls_criteria = ReplaceA(ls_criteria, ll_pos, 1, "')")
			//Add quotes for each value
			ll_pos = PosA(ls_criteria, ",")			
			DO WHILE ll_pos > 0
				ls_criteria = ReplaceA(ls_criteria, ll_pos, 1, "','")
				ls_criteria = LeftA(ls_criteria, ll_Pos + 2) + Trim(MidA(ls_criteria, ll_Pos + 3)) 
				ll_pos = PosA(ls_criteria, ",", ll_Pos + 3)
			LOOP
		END IF		
	CASE ELSE
		//
...
public function string of_parse_quote (string as_criteria, string as_type);//Parse the criteria to add single quotes - Alfee 09.18.2008
Long ll_pos
String ls_criteria, ls_leftchar, ls_rightchar

CHOOSE CASE Upper(as_type)
	//---------Begin Modified by (Appeon)Toney 06.07.2013 for V141 ISG-CLX--------	
	  /*
		CASE 'C', 'D' //for char, datetime type only	
	  */
	CASE 'C','S','D'
	//---------End Modfiied ------------------------------------------------------------------
	
		
		//Check and Add singe quotes if needed		
		IF IsNUll(as_criteria) THEN as_criteria = ""
		IF LenA(Trim(as_criteria)) <= 1 THEN RETURN "'" + as_criteria + "'"
		
		ls_criteria = Trim(as_criteria)	
		IF LeftA(ls_criteria, 1) = "'" AND RightA(ls_criteria, 1) = "'" THEN 	  //sigle quotes
			//Do nothing
		ELSEIF LeftA(ls_criteria, 1) = '"' AND RightA(ls_criteria, 1) = '"' THEN //double quotes
			IF LenA(ls_criteria) = 2 THEN ls_criteria = "''"
			IF LenA(ls_criteria) > 2 THEN ls_criteria = "'" + MidA(ls_criteria, 2, LenA(ls_criteria) - 2) + "'"
		ELSE 																						  //No quotes
			ls_criteria = "'" + as_criteria + "'"
		END IF	
...
public function string of_format_phonenumber (string as_phonenumber);//Format a phone number(+@@@(@@@)@@@-@@@@") - alfee 04.02.2010

Integer i, li_len
String  ls_char, ls_ret, ls_tmp

if Isnull(as_phonenumber) or LenA(Trim(as_phonenumber)) < 1 then return ''

//Strip to numbers
for i = 1 to LenA(as_phonenumber)
	ls_char = MidA(as_phonenumber, i, 1) 
	if isnumber(ls_char) then ls_ret += ls_char
next

//format the number
li_len = LenA(ls_ret)
if li_len = 10 then
	ls_ret = "(" + MidA(ls_ret,1,3) + ") " + MidA(ls_ret,4,3) + "-" + MidA(ls_ret,7) 
elseif li_len = 7 then
	ls_ret = MidA(ls_ret,1,3) + "-" + MidA(ls_ret,4) 
elseif li_len <= 13 and li_len > 10 then
	ls_tmp = RightA(ls_ret, 10)
	ls_ret = "+" + MidA(ls_ret,1,li_len -10) + "(" + MidA(ls_tmp,1,3) + ") " + MidA(ls_tmp,4,3) + "-" + MidA(ls_tmp,7) 
else
	//do nothing
end if
...
public function integer of_arraytostring (string as_source[], string as_mark, ref string as_delimiter, ref string as_ref_string);//////////////////////////////////////////////////////////////////////
// $<function>n_cst_string_appeon::of_arraytostring()
// $<arguments>
//		value    	string	as_source[]  		
//		value    	string	as_mark      		
...
For ll_Count = 1 to ll_ArrayUpBound
	...
	If ll_Count = 1 Then
		//Initialize string
		as_ref_string = as_mark + as_source[ll_Count] + as_mark
	else
		//Concatenate to string
		as_ref_string = as_ref_string + as_delimiter + as_mark + as_source[ll_Count] + as_mark
	End If
Next 
...
public function integer of_replace_env_var (ref string as_pathname);//////////////////////////////////////////////////////////////////////
// Function: of_replace_env_var()
// Arguments:
// 	reference    string    as_pathname
//--------------------------------------------------------------------
// Return:  integer
//--------------------------------------------------------------------
// Author:	 Ken.Guo 2011-06-24.
//--------------------------------------------------------------------
// Description: Replace the ENV var with physic value
//--------------------------------------------------------------------
//$<Modify History>:
//(Appeon)Toney 06.07.2013 - V141 ISG-CLX
//////////////////////////////////////////////////////////////////////
String ls_env_var,ls_env_var_value,ls_newvalue
Long ll_pos_start,ll_pos_end
n_cst_systeminfo lnv_systeminfo

ll_pos_start = Pos(as_pathname, '%')
If ll_pos_start = 0 Then Return 0

ll_pos_end = Pos(as_pathname, '%', ll_pos_start + 1 ) 
If ll_pos_end = 0 Then Return -1

If ll_pos_end = ll_pos_start + 1 Then Return 0

ls_env_var = Mid(as_pathname, ll_pos_start + 1, ll_pos_end - ll_pos_start - 1)

lnv_systeminfo = Create n_cst_systeminfo
ls_env_var_value = lnv_systeminfo.of_get_envvar( ls_env_var)
If ls_env_var_value <> '' Then
	as_pathname = Replace(as_pathname,ll_pos_start,ll_pos_end - ll_pos_start + 1, ls_env_var_value )	
End If
...
public subroutine of_insert_to_array (ref string as_array[], long ai_into_index, string as_value, boolean ab_usefind);//////////////////////////////////////////////////////////////////////
// Function: of_insert_to_array()
// Arguments:
// 	reference    string     as_array[]
// 	value        long       ai_into_index
// 	value        string     as_value
// 	value        boolean    ab_usefind
//--------------------------------------------------------------------
// Return:  (None)
//--------------------------------------------------------------------
// Author:	 Ken.Guo 2011-05-05.
//--------------------------------------------------------------------
// Description: Insert a string value to array.
//--------------------------------------------------------------------
//$<Modify History>:
//(Appeon)Toney 06.07.2013 - V141 ISG-CLX
//////////////////////////////////////////////////////////////////////
Long i,j,ll_cnt
String ls_arr_temp[],ls_source,ls_targer

ll_cnt = UpperBound(as_array[])

//Find in exists array
If ab_usefind Then
	For i = 1 To ll_cnt
		If as_value = as_array[i] Then
			j = i
			exit
		End If
	Next
...
If j > 0 Then
	//Move if exists
	ls_arr_temp[] = as_array[]
	as_array[ai_into_index] = as_array[j]
	For i = ai_into_index + 1 To j
		as_array[i] = ls_arr_temp[i - 1]
	Next
Else
	//Insert 
	ls_arr_temp[] = as_array[]
	as_array[ai_into_index] = as_value	
	For i = ai_into_index To ll_cnt
		as_array[i + 1] = ls_arr_temp[i]
	Next
...
public subroutine of_sortarray (ref datetime adt_value[], string as_sorttype);//////////////////////////////////////////////////////////////////////
// $<function>n_cst_string_appeonof_sortarray()
// $<arguments>
//		reference	datetime  	adt_value[] 		
//		value    	string	as_sorttype	 Values: 'A' or 'D'	
// $<returns> (none)
// $<description> Sort array element. 
// $<description>
//////////////////////////////////////////////////////////////////////
...
lds_temp.DataObject = 'd_array_for_sort'

ll_cnt = UpperBound(adt_value[])
If UpperBound(adt_value[]) <= 1 Then Return 

For i = 1 To ll_cnt
	lds_temp.InsertRow(0)
Next

lds_temp.object.d_col[1,ll_cnt] = adt_value[]	

If Upper(as_sorttype) = 'A' Then
	lds_temp.SetSort('d_col A')
Else
	lds_temp.SetSort('d_col D')	
End If

lds_temp.Sort()
adt_value[] = lds_temp.object.d_col[1,ll_cnt] 
If isvalid(lds_temp) Then Destroy lds_temp
Return 
end subroutine

public subroutine of_sortarray (ref decimal adec_value[], string as_sorttype);//////////////////////////////////////////////////////////////////////
// $<function>n_cst_string_appeon::of_sortarray()
// $<arguments>
//		reference	decimal  	adec_value[] 		
//		value    	string	as_sorttype	 Values: 'A' or 'D'	
// $<returns> (none)
// $<description> Sort array element. 
// $<description>
...
ll_cnt = UpperBound(adec_value[])
If UpperBound(adec_value[]) <= 1 Then Return 

For i = 1 To ll_cnt
	lds_temp.InsertRow(0)
Next

lds_temp.object.dec_col[1,ll_cnt] = adec_value[]	

If Upper(as_sorttype) = 'A' Then
	lds_temp.SetSort('dec_col A')
Else
	lds_temp.SetSort('dec_col D')	
End If

lds_temp.Sort()
adec_value[] = lds_temp.object.dec_col[1,ll_cnt] 
If isvalid(lds_temp) Then Destroy lds_temp
Return 
end subroutine
...
ll_cnt = UpperBound(al_value[])
If UpperBound(al_value[]) <= 1 Then Return 

For i = 1 To ll_cnt
	lds_temp.InsertRow(0)
Next

lds_temp.object.n_col[1,ll_cnt] = al_value[]	

If Upper(as_sorttype) = 'A' Then
	lds_temp.SetSort('n_col A')
Else
	lds_temp.SetSort('n_col D')	
End If

lds_temp.Sort()
al_value[] = lds_temp.object.n_col[1,ll_cnt] 
If isvalid(lds_temp) Then Destroy lds_temp
Return 
end subroutine
...
ll_cnt = UpperBound(as_value[])
If UpperBound(as_value[]) <= 1 Then Return 

For i = 1 To ll_cnt
	lds_temp.InsertRow(0)
Next

lds_temp.object.s_col[1,ll_cnt] = as_value[]	

If Upper(as_sorttype) = 'A' Then
	lds_temp.SetSort('s_col A')
Else
	lds_temp.SetSort('s_col D')	
End If

lds_temp.Sort()
as_value[] = lds_temp.object.s_col[1,ll_cnt] 
If isvalid(lds_temp) Then Destroy lds_temp

Return 
end subroutine

on n_cst_string_appeon.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cst_string_appeon.destroy
