$PBExportHeader$w_batreplace_eng_new.srw
forward
global type w_batreplace_eng_new from window
end type
type cb_4 from commandbutton within w_batreplace_eng_new
end type
type st_2 from statictext within w_batreplace_eng_new
end type
type plb_1 from picturelistbox within w_batreplace_eng_new
end type
type cb_3 from commandbutton within w_batreplace_eng_new
end type
type st_6 from statictext within w_batreplace_eng_new
end type
type st_5 from statictext within w_batreplace_eng_new
end type
type sle_2 from singlelineedit within w_batreplace_eng_new
end type
type sle_1 from singlelineedit within w_batreplace_eng_new
end type
type rb_2 from radiobutton within w_batreplace_eng_new
end type
type rb_1 from radiobutton within w_batreplace_eng_new
end type
type cbx_3 from checkbox within w_batreplace_eng_new
end type
type cbx_2 from checkbox within w_batreplace_eng_new
end type
type cbx_1 from checkbox within w_batreplace_eng_new
end type
type st_4 from statictext within w_batreplace_eng_new
end type
type ddlb_1 from dropdownlistbox within w_batreplace_eng_new
end type
type cb_2 from commandbutton within w_batreplace_eng_new
end type
type st_3 from statictext within w_batreplace_eng_new
end type
type st_1 from statictext within w_batreplace_eng_new
end type
type cb_1 from commandbutton within w_batreplace_eng_new
end type
type gb_1 from groupbox within w_batreplace_eng_new
end type
type gb_2 from groupbox within w_batreplace_eng_new
end type
type st_7 from statictext within w_batreplace_eng_new
end type
type st_8 from statictext within w_batreplace_eng_new
end type
type ln_1 from line within w_batreplace_eng_new
end type
type ln_2 from line within w_batreplace_eng_new
end type
type st_9 from statictext within w_batreplace_eng_new
end type
end forward
...
global w_batreplace_eng_new w_batreplace_eng_new

type prototypes
//FUNCTION INT LibraryEntryImportEx(STRING lpszLibraryList[], INT iNumberOfLibs, STRING lpszDestLibName, STRING lpszEntryName, INT iEntryType, STRING lpszEntrySyntax, STRING lpszAppName) LIBRARY "pborc115.dll"

FUNCTION Ulong PBORCA_SessionOpen()  LIBRARY "pborc115.dll"
FUNCTION Ulong PBORCA_SessionClose(ulong handle)  LIBRARY "pborc115.dll"

Function   Int PBORCA_SessionSetLibraryList(Ulong   hSession,   String   pLIbnames[],   &   
int   nNoOfLibs)   Library   'pborc115.dll'  

Function Int PBORCA_SessionSetCurrentAppl(Ulong   hSession,   String   lpszApplLibName,&   
String   lpszApplName)   Library   'pborc115.dll'   
...
String   pUserData)   Library   "pborc115.dll"  
end prototypes
...
public function integer libraryentryimportex (string lpszlibrarylist[], integer inumberoflibs, string lpszdestlibname, string lpszentryname, integer ientrytype, string lpszentrysyntax, string lpszappname)
end prototypes
...
lc_app = GetApplication().ClassDefinition
is_application_pbl = lc_app.LibraryName
//==========================================================================
...
public function integer libraryentryimportex (string lpszlibrarylist[], integer inumberoflibs, string lpszdestlibname, string lpszentryname, integer ientrytype, string lpszentrysyntax, string lpszappname);//====================================================================
//$<Function>: libraryentryimportex
//$<Arguments>:
// 	value    string     lpszlibrarylist[]
// 	value    integer    inumberoflibs
// 	value    string     lpszdestlibname
// 	value    string     lpszentryname
// 	value    integer    ientrytype
// 	value    string     lpszentrysyntax
// 	value    string     lpszappname
//$<Return>:  integer
//$<Description>: Import entry to pb library. (Use PB ORCA)
//$<Author>: (Appeon) long.zhang 01.28.2016
//--------------------------------------------------------------------
//$<Modify History>:
//====================================================================

uLong ll_lBufferSize
Int li_rtn
Int li_nNoOfLibs
String ls_pLIbnames[], ls_lpszApplLibName, ls_lpszApplName, ls_lpszComments
String ls_pCompErrorProc,ls_pUserData

SetNull(ls_pCompErrorProc)
SetNull(ls_pUserData)

If il_hSession = 0 Then
	il_hSession = PBORCA_SessionOpen()
	If il_hSession <= 0 Then 
		Messagebox('PBORCA_SessionOpen Error', String(il_hSession))
		Return il_hSession
	End If
	
	ls_pLIbnames = lpszlibrarylist[]
	li_nNoOfLibs = inumberoflibs
	
	//Librarylist
	li_rtn = PBORCA_SessionSetLibraryList( il_hSession,   ls_pLIbnames[],   li_nNoOfLibs)
	If li_rtn < 0 Then	
		PBORCA_SessionClose(il_hSession)
		il_hSession = 0
		Messagebox('PBORCA_SessionSetLibraryList Error', String(li_rtn))
		Return li_rtn
	...
	//App
	ls_lpszApplLibName =  is_application_pbl
	
	ls_lpszApplName = lpszappname
	li_rtn = PBORCA_SessionSetCurrentAppl( il_hSession,  ls_lpszApplLibName, ls_lpszApplName)
	If li_rtn < 0 Then
		PBORCA_SessionClose(il_hSession)
		il_hSession = 0
		Messagebox('PBORCA_SessionSetCurrentAppl Error', String(li_rtn))
		Return li_rtn
	//	Return code Description
	//	0 PBORCA_OK Operation successful
	//	-1 PBORCA_INVALIDPARMS Invalid parameter list
...
//Import
ls_lpszComments = 'Modified: '+String(now()) 
ll_lBufferSize = Len(lpszentrysyntax) * 2

li_rtn = PBORCA_CompileEntryImport( il_hSession,   lpszdestlibname,   &   
	lpszentryname,   ientrytype,   &   
	ls_lpszComments,  lpszentrysyntax,   &   
	ll_lBufferSize,   ls_pCompErrorProc,   &   
	ls_pUserData) 
If li_rtn < 0 Then
	PBORCA_SessionClose(il_hSession)
	il_hSession = 0
	Messagebox('PBORCA_CompileEntryImport Error', String(li_rtn))
	Return li_rtn
	...
	//object could not be saved in the library
...
on w_batreplace_eng_new.create
...
on w_batreplace_eng_new.destroy
destroy(this.cb_4)
destroy(this.st_2)
destroy(this.plb_1)
destroy(this.cb_3)
...
event close;//Close Session
If il_hSession >0 Then PBORCA_SessionClose(il_hSession)
end event
...
string facename = "Segoe Keycaps"
long textcolor = 33554432
long backcolor = 1090519039
boolean allowedit = true
boolean sorted = false
boolean vscrollbar = true
string item[] = {"@Fixedsys","@system","@Teminal","Arial","Arial Black","Arial CE","Arial CYR","Arial Greek","Arial Narrow","Arial TUR","AvantGarde","Basemic","Courier","Courier New","Courier New CE","Courier New CYR","Fixedsys","Georgia","Terminal","Segoe UI"}
borderstyle borderstyle = stylelowered!
end type
