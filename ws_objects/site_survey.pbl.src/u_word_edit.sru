$PBExportHeader$u_word_edit.sru
$PBExportComments$(Appeon)Eugene 06.25.2013 - V141 ISG-CLX
forward
global type u_word_edit from olecontrol
end type
end forward
...
string binarykey = "u_word_edit.udo"
...
global u_word_edit u_word_edit
...
public function integer of_openword (blob ab_word);Long			ll_RE
oleobject		lole_Window

ll_RE = This.Clear( )
If ll_RE < 0 Then MessageBox( 'Tips' ,  "Failed to clear word." + string( ll_RE ) )

ll_RE = This.Insertclass( "Word.Document" )
If ll_RE < 0 Then MessageBox( 'Tips' , "Failed to insert word." + string( ll_RE ) )

If Not IsNull( ab_word ) Then
	Try
		This.ObjectData = ab_word
	Catch( Exception ex )
		MessageBox( 'Tips', 'Failed to open Word content !' )
		this.Clear( )
		Return -40
	End Try 
End If
This.DisplayType = DisplayAsActiveXDocument!
ll_RE = This.Activate( inplace!)
If ll_RE < 0 Then MessageBox( 'Tips' , "Failed to Activate word." + string( ll_RE ) )
...
lole_object = This.object.Application.ActiveDocument
...
this.object.Application.ActiveDocument.Application.NormalTemplate.Saved = TRUE //Added By Ken.Guo 2010-01-07.

this.object.Application.ActiveDocument.SaveAs(ls_FileName, 0,false,'',false) //Word 2003 format
...
this.object.Application.ActiveDocument.SaveAs(ls_FileName, 10,false,'',false)
...
// Insert file first
lole_Selection = this.object.Application.ActiveDocument.ActiveWindow.Selection
...
If  Not IsValid( This.Object.Application.ActiveDocument ) THEN RETURN -1
If Not FileExists(as_filename) THEN RETURN -1

TRY

This.Object.Application.ActiveDocument.Content.Select()
lole_Selection = This.Object.Application.ActiveDocument.Activewindow.Selection
...
This.Object.Application.ActiveDocument.Range(0,0).InsertBefore(as_text + '~r~n')
...
//		this.object.OpenLocalFile(ls_FileName, true) 
		this.InsertFile( ls_FileName )
		this.Activate( inplace! )
	else
		SetNull( lblb_Data )
		of_openword( lblb_Data )		
		//----------End Added ----------------------------------------						
	end if
end if
...
// Get window object
lole_Window = this.object.Application.ActiveDocument.ActiveWindow
...
IF this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[2].ID = 1732 AND &
	this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[2].Visible THEN
	this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[2].Visible = FALSE
END IF
IF this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[1].ID = 5757 AND &
	this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[1].Visible THEN
	this.object.Application.ActiveDocument.CommandBars.Item("Formatting").Controls.Item[1].Visible = FALSE
END IF
...
on u_word_edit.create
end on

on u_word_edit.destroy
end on
...
//ll_R = InsertClass( "word.document" ) 
//If ll_R <> 0 Then
//	MessageBox( 'tips', 'Failed to Insert Class of word ' + string(  ll_R ) )
//End If
end event
