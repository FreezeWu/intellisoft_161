...
global type n_cst_easymail_imap4 from nonvisualobject autoinstantiate
end type
...
//				lb_receive = True
//			End If
			If  Lower( ids_new_msg_id.GetItemString( i,  'fromintellisoft' ) ) = 'yes' Then  //(Appeon)Harry 05.28.2014 - V142 ISG-CLX modify  'fromcontractlogix' to 'fromintellisoft'
				lb_receive = True
			End If
...
public function integer of_receive_emails (string as_type);Integer			li_count1, j
string				ls_boxname
...
//Get Custom Fields
If iole_IMAP4Mail.Message.GetHeaderField('X-FromIntelliSoft',0) = 'Yes' Then  //(Appeon)Harry 05.28.2014 - V142 ISG-CLX modify 'X-FromContractLogix' to 'X-FromIntelliSoft'
...
For i = 1 To ids_new_msg_id.Rowcount( )
	...
	ids_new_msg_id.SetItem( i, 'fromintellisoft', iole_IMAP4Mail.Message.GetHeaderField('X-FromIntelliSoft',0) ) //(Appeon)Harry 05.28.2014 - V142 ISG-CLX modify  'fromcontractlogix' to 'fromintellisoft'
...
If ll_result <> 0 Then
	If IsValid( iole_IMAP4SSL ) And ai_pop3ssl = 1 Then
		If ab_show_msg Then MessageBox( 'Email' , 'SSL Error( Code:' + String( iole_IMAP4SSL.Error.Code)  + ',Description:' + string( iole_IMAP4SSL.Error.Description ) + ')' )
		return false
	End If
		
	If ab_show_msg Then messagebox( 'Email', 'Could not connect to mailserver.Error:' + string(iole_IMAP4Mail.GetLastErrorString() ))
	return false
end if

ll_Result = iole_IMAP4Mail.Login(  as_account , as_pwd  )
If ll_result <> 0 Then
	If ab_show_msg Then messagebox( 'Email', 'Failed. '+ string(iole_IMAP4Mail.GetLastErrorString() ))
	return false
end if
