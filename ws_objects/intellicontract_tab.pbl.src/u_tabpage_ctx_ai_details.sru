$PBExportHeader$u_tabpage_ctx_ai_details.sru
$PBExportComments$[intellicontract_tab]  Contract Contact Tab Page
forward
global type u_tabpage_ctx_ai_details from u_tabpg
end type
type dw_details from u_dw_contract within u_tabpage_ctx_ai_details
end type
type dw_1 from u_dw_contract within u_tabpage_ctx_ai_details
end type
type cb_1 from commandbutton within u_tabpage_ctx_ai_details
end type
end forward
...
global u_tabpage_ctx_ai_details u_tabpage_ctx_ai_details
...
ls_search_string = 'arguments=(("al_ctx_id", number))'
...
astr_sql[1].s_where = "ctx_basic_info.ctx_id = ctx_action_items.ctx_id" 
...
inv_cst_sql.OF_parse( dw_details.Describe("DataWindow.Table.Select"), astr_sql)
astr_sql[1].s_where  = "(ctx_basic_info.ctx_id = ctx_action_items.ctx_id and ctx_basic_info.is_template = 0)"
...
ls_sql_smnt  = "ctx_basic_info.status = ( SELECT code_lookup.lookup_code FROM code_lookup WHERE ( code_lookup.lookup_name = 'Contract Status' ) AND ( code = 'active' ) )" 

//--------------------------- APPEON BEGIN ---------------------------
//$<modify> 10.30.2006 By: Davis
//$<reason> Should display all action items from individual contracts.
/*
choose CASE (as_series)
	CASE 'Needs Processing'
		astr_sql[1].s_where = astr_sql[1].s_where + " AND ctx_action_items.print_flag = 0 AND ctx_action_items.action_status is null and letter_flag = 'Y' AND " + ls_sql_smnt
	CASE 'Awaiting Response'
		astr_sql[1].s_where = astr_sql[1].s_where + " AND ctx_action_items.print_flag = 1 AND ctx_action_items.action_status is null and letter_flag = 'Y' AND " + ls_sql_smnt
	CASE ELSE
end choose 
*/
...
if ll_found_row > 0 then 
	astr_sql[1].s_where =  astr_sql[1].s_where + ' AND ctx_action_items.action_type = '  +STRING(contract_action_type.getItemNumber(ll_found_row,'lookup_code'))
end if
/*
...
public function string of_set_filter (string as_data);/******************************************************************************************************************
**  [PUBLIC]   : of_set_filter( )
**==================================================================================================================
**  Purpose   	: Sets the filter in dw_1
**==================================================================================================================
**  Arguments 	: [string]  as_data The data value from the intm changed event
**==================================================================================================================
**  Returns   	: [none]   
**==================================================================================================================
**  Notes     	: 	   
**==================================================================================================================
**  Created By	: Michael B. Skinner 14 November 2005  ? Intellisoftgroup, Inc
**==================================================================================================================
**  Modification Log
**   Changed By             Change Date                                               Reason
** ------------------------------------------------------------------------------------------------------------------
...
//$<reason> Change the filter Condition.
long ll_Type,ll_Status,ll_step,ll_active_status
...
If IsNumber(as_data) Or Not f_validstr(as_data) Then
	//ls_filter = ls_filter
	ls_filter = as_data
Else
		choose  case as_data
				case '(NONE)'
					ls_filter = ls_filter
				//Satrt Change By Jervis at 01.22.2007
				/*
				case 'Needs Processing'
					ls_filter =  ls_filter +  " and isNull( action_status )  and print_flag = 0"
				case 'Awaiting Response' 
					ls_filter =  ls_filter +  " and isNull( action_status )  and print_flag = 1"
				*/
		Case 'Needs Processing'
			ls_filter =  ls_filter +  " and isNull( action_status )  and print_flag >= 0"
		Case 'Awaiting Response'
			...
			//End Change
		Case Else
			MessageBox('Developer Message', ' Case not found in u_tabpage_ctx_ai_details::of_set_filter() ')
		End Choose
END IF

ldw.setfilter(ls_filter)
if ldw.filter() <> 1 then
	messagebox('Developer Message', 'Please check the filter in of_setfiter')
end if
...
// Initial
dw_1.AcceptText()
dw_1.SetColumn( "action_type" )
//ls_Filter = "1=1"			//Comment by Scofield on 2008-12-31

// Get Filter condition
ll_act_type = dw_1.GetitemNumber(1, "action_type")
ls_act_user = dw_1.GetitemString(1, "action_user")
ll_act_status = dw_1.GetitemNumber(1, "action_status")
ldt_act_date = dw_1.GetitemDatetime(1, "action_date")
ldt_act_date1 = dw_1.GetitemDatetime(1, "action_date_1")
ldt_due_date = dw_1.GetitemDatetime(1, "due_date")
ldt_due_date1 = dw_1.GetitemDatetime(1, "due_date_1")
ll_active_status = dw_1.GetItemNumber(1,"active_status")

// Set Filter condition
IF Not IsNull( ll_act_type ) AND ll_act_type > 0 THEN
	ls_Filter = ls_Filter + " AND action_type = " + String( ll_act_type )
END IF

IF Not IsNull( ls_act_user ) AND LEN(ls_act_user) > 0 THEN
	ls_Filter = ls_Filter + " AND action_user = '" + ls_act_user + "'"
END IF

IF Not IsNull( ll_act_status ) THEN
	ls_Filter = ls_Filter + " AND action_status = " + String( ll_act_status )
END IF

IF Not IsNull( ldt_act_date ) THEN
	ls_Filter = ls_Filter + " AND action_date >= Date('" + String( Date( ldt_act_date ), "yyyy-mm-dd" ) + "')"
END IF

IF Not IsNull( ldt_act_date1 ) THEN
	ls_Filter = ls_Filter + " AND action_date <= Date('" + String( Date( ldt_act_date1 ), "yyyy-mm-dd" ) + "')"
END IF

IF Not IsNull( ldt_due_date ) THEN
	ls_Filter = ls_Filter + " AND due_date >= Date('" + String( Date( ldt_due_date ), "yyyy-mm-dd" ) + "')"
END IF

IF Not IsNull( ldt_due_date1 ) THEN
	ls_Filter = ls_Filter + " AND due_date <= Date('" + String( Date( ldt_due_date1 ), "yyyy-mm-dd" ) + "')"
END IF

//--------------------------- APPEON BEGIN ---------------------------
//$<add> 12.12.2006 By: Jervis
//$<reason> 
//ls_Filter = ls_Filter + " and (active_status >= 0 or isnull(active_status))"
if Not IsNull(ll_active_status) then
	if ll_active_status = -1 then
		ls_Filter = ls_Filter + " AND (active_status < 0)"
	elseif ll_active_status = 1 then
		ls_Filter = ls_Filter + " AND (active_status >= 0 or isnull(active_status))"
	end if
end if
//---------------------------- APPEON END ----------------------------
...
ls_filter = Mid(ls_filter,Pos(Upper(ls_filter),Upper("AND")) + Len("AND "))		//Added by Scofield on 2008-12-31

Return ls_filter

end function

on u_tabpage_ctx_ai_details.create
int iCurrent
call super::create
...
on u_tabpage_ctx_ai_details.destroy
call super::destroy
...
IF RowCount = 1 AND Upper(AppeonGetClientType())="WEB" THEN THIS.ScrollToRow(1)//(Appeon)Toney 10.12.2013 - V141 ISG-CLX Fix Reintegration BugT101002 force refresh UI of datawindow in web
//---------End Added ------------------------------------------------------
...
event pfc_preupdate;call super::pfc_preupdate;long ll_i,ll_seq,ll_ori,ll_cur
...
dw_1.dataobject = 'd_ext_ctx_id'
*/
dw_1.dataobject = 'd_contract_action_items_filter'
...
ldw_child.Setfilter("user_id = '" + gs_user_id + "'")
ldw_child.Filter()
*/
//---------------------------- APPEON END ----------------------------
/* Commented by Allen
...
IF dwo.name='b_1' then	//Clear Filter
		This.Reset()
		This.InsertRow( 0 )
		ls_Filter=""
