$PBExportHeader$w_recruitment_data.srw
forward
global type w_recruitment_data from w_response
end type
type st_2 from statictext within w_recruitment_data
end type
type cb_upload from commandbutton within w_recruitment_data
end type
type cb_email from commandbutton within w_recruitment_data
end type
type st_1 from statictext within w_recruitment_data
end type
type ddlb_filter from dropdownlistbox within w_recruitment_data
end type
type cb_filter from commandbutton within w_recruitment_data
end type
type cb_sort from commandbutton within w_recruitment_data
end type
type cb_convert from commandbutton within w_recruitment_data
end type
type cb_delete from commandbutton within w_recruitment_data
end type
type cb_view from commandbutton within w_recruitment_data
end type
type cb_save from commandbutton within w_recruitment_data
end type
type cb_add from commandbutton within w_recruitment_data
end type
type cb_close from commandbutton within w_recruitment_data
end type
type dw_browse from u_dw within w_recruitment_data
end type
type dw_detail from u_dw within w_recruitment_data
end type
end forward
...
global w_recruitment_data w_recruitment_data
...
this.Control[iCurrent+5]=this.ddlb_filter
this.Control[iCurrent+6]=this.cb_filter
this.Control[iCurrent+7]=this.cb_sort
this.Control[iCurrent+8]=this.cb_convert
...
if dw_detail.modifiedcount( ) > 0 then
	if MessageBox("Filter","Do you want to save changes?",Question!,YesNO!,1) = 1 then
		if of_save() < 0 then
			return
		end if
	end if	
...
if dw_browse.event pfc_filterdlg() = 1 then
	of_triggerRowFocusChanged(ll_row)
end if
end event
...
//<$>Commented:long.zhang 10.12.2013
...
dw_browse.event pfc_SortDlg()//Added by Appeon long.zhang 10.12.2013 (V14.1 Recruitment Functionality) BugS101108
end event
