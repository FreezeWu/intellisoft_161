$PBExportHeader$n_cst_scheduler.sru
forward
global type n_cst_scheduler from nonvisualobject
end type
end forward

global type n_cst_scheduler from nonvisualobject autoinstantiate
end type
...
long il_report_storage_id //(Appeon)Stephen 05.23.2017 - V15.4-WebView Reporting Functionality
...
integer ii_max_retries = 1 //maha 11.17.2015 //Set default value 1, Added by Appeon long.zhang 10.13.2016 (Auto scheduler shut down not working Bug_id#5364)
...
DateTime ldt_null //Added by Appeon long.zhang 10.13.2016
boolean lb_IsTimerStop = false //Evan 12.29.2011 //Note: lb_IsTimerStop is ONLY USED FOR SCHEDULER AUTO SHUTDOWN now, Appeon long.zhang 10.13.2016
Boolean lb_timer_stopped_flag = False//Timer(0) flag,its different from lb_IsTimerStop, Added by Appeon long.zhang 10.12.2016 (Auto scheduler shut down not working Bug_id#5364)

SetNull(ldt_null) //Added by Appeon long.zhang 10.13.2016
...
FOR r = 1 To li_rc	
	...
	if ids_schedule.GetItemNumber( r, "active_status") = 1 and ids_schedule.GetItemNumber(r, "retry_fail") = 1 THEN  //Start Code Change ----10.12.2016 #V152 maha added check for retry setting 
		li_max_attempts = ids_schedule.GetItemNumber(r, "fail_max_att") 
		
		if  li_max_attempts = 0 or isnull(li_max_attempts) then li_max_attempts = 1//Added by Appeon long.zhang 10.12.2016 (Auto scheduler shut down not working Bug_id#5364	)
...
FOR r = 1 To li_rc	
	...
	il_report_storage_id = ids_schedule.GetItemNumber( r, "report_storage_id") //(Appeon)Stephen 05.23.2017 - V15.4-WebView Reporting Functionality
	...
	lb_timer_stopped_flag = True//Added by Appeon long.zhang 10.12.2016 (Auto scheduler shut down not working Bug_id#5364)
	...
	ELSE  //using rerun on failure
		if is_comp_flag = 'Y' then
			...
			//<$>reason:If succeed, set next_time no mater which num of attempts. For Auto scheduler shut down not working Bug_id#5364
...
//			end choose	
			ids_schedule.SetItem(r,"next_start",ldt_next_time)
			ids_schedule.SetItem(r,"org_run_time",ldt_null)
			li_fail_cnt = 0
			//------------------- APPEON END -------------------
			...
			choose case li_fail_cnt //attempt
				...
				case li_max_attempts //
					//------------------- APPEON BEGIN -------------------
					//<$>added:long.zhang 10.13.2016
					//<$>reason:If failed in the last attempt, always set next_time. For Auto scheduler shut down not working Bug_id#5364
					//				Don't use li_reset_fail and retry_fail together anymore.  Reset org_run_time.
...
//End Code Change ----04.30.2015
	...
	//immediate garbage collection for C/S, Added by Appeon long.zhang 11.18.2016 (Scheduler error)
	GarbageCollect ( )
NEXT
...
if ii_retry_time = ii_max_retries then 
	ii_retry_time = 0   
	lb_IsTimerStop = true
end if
...
//------------------- APPEON BEGIN -------------------
//<$>added:long.zhang 10.12.2016
//<$>reason: Restore Timer when it was stopped by Timer(0) , For Auto scheduler shut down not working Bug_id#5364 
//if lb_IsTimerStop then Timer(60, w_auto_schedule) //Added by Evan 12.29.2011 --- for fix pb exception
If lb_timer_stopped_flag  then Timer(60, w_auto_schedule) 
//------------------- APPEON END -------------------
...
datetime ldt_null
if fileexists(ls_path) and il_report_storage_id>0 then
	f_report_storage(il_report_storage_id, ls_path, gl_export_rows_count, ldt_null, ldt_null, "", 'S')
end if
//---------End Added ------------------------------------------------------
...
//---------Begin Added by (Appeon)Stephen 05.23.2017 for V15.4-WebView Reporting Functionality--------
datetime ldt_null
if fileexists(as_save_path+ls_extend) and il_report_storage_id>0 then
	f_report_storage(il_report_storage_id, as_save_path+ls_extend, al_proc_cnt, ldt_null,ldt_null, "", 'S')
end if
//---------End Added ------------------------------------------------------
...
ll_report_row = w_custom_report.tab_1.tabpage_run.dw_run.rowcount()//(Appeon)Stephen 05.31.2017 - V15.4-WebView Reporting Functionality
...
datetime ldt_null
if lower(right(ls_filepath, 4)) <> ".pdf" then
	ls_filepath = ls_filepath + ".pdf"
end if
if fileexists(ls_filepath) and il_report_storage_id>0 then
	f_report_storage(il_report_storage_id, ls_filepath,ll_report_row, ldt_null, ldt_null, "", 'S')
end if
//---------End Added ------------------------------------------------------
...
on n_cst_scheduler.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cst_scheduler.destroy
