$PBExportHeader$pfc_ia_cst_nv_net_dev_new_temp.sru
forward
global type pfc_ia_cst_nv_net_dev_new_temp from pfc_cst_nv_net_dev_prac_folder
end type
type dw_appoint_stat from u_dw within tabpage_ai
end type
type dw_tracking from u_dw within tabpage_ai
end type
type cb_calc from commandbutton within tabpage_ai
end type
...
type gb_filter_1 from groupbox within pfc_ia_cst_nv_net_dev_new_temp
end type
type cb_ma from commandbutton within pfc_ia_cst_nv_net_dev_new_temp
end type
type dw_payors from u_dw within pfc_ia_cst_nv_net_dev_new_temp
end type
type st_as_msg from statictext within pfc_ia_cst_nv_net_dev_new_temp
end type
type dw_facility_add from u_dw within pfc_ia_cst_nv_net_dev_new_temp
end type
type cb_batch from commandbutton within pfc_ia_cst_nv_net_dev_new_temp
end type
type p_1 from picture within pfc_ia_cst_nv_net_dev_new_temp
end type
type cb_1 from commandbutton within pfc_ia_cst_nv_net_dev_new_temp
end type
type cbx_compl from checkbox within pfc_ia_cst_nv_net_dev_new_temp
end type
type cb_change from commandbutton within pfc_ia_cst_nv_net_dev_new_temp
end type
type st_filter from statictext within pfc_ia_cst_nv_net_dev_new_temp
end type
type dw_qf from datawindow within pfc_ia_cst_nv_net_dev_new_temp
end type
type st_group_by from statictext within pfc_ia_cst_nv_net_dev_new_temp
end type
type ddlb_group_by from dropdownlistbox within pfc_ia_cst_nv_net_dev_new_temp
end type
type st_zoom from statictext within pfc_ia_cst_nv_net_dev_new_temp
end type
type ddlb_zoom from dropdownlistbox within pfc_ia_cst_nv_net_dev_new_temp
end type
type cb_track_recalc from commandbutton within pfc_ia_cst_nv_net_dev_new_temp
end type
end forward
...
st_zoom st_zoom
ddlb_zoom ddlb_zoom
cb_track_recalc cb_track_recalc
end type
global pfc_ia_cst_nv_net_dev_new_temp pfc_ia_cst_nv_net_dev_new_temp

type variables
Boolean ib_payors_just_retrieved = True
Boolean ib_save = False
boolean ib_show_ids_addr = false  //maha 06.12.2013
boolean ib_insertrow = true  //maha 07.30.2015 - to allow supppression in Action items
...
long il_facility_aif   //maha 08.10.2015
end variables
...
IF il_app_id > 0 THEN
	...
	//$<reason> Use app_hdr_app_id column to filter.
...
//Start Code Change ----08.10.2015 #V15 maha - added action items option
IF lb_from_prac_folder THEN //from Practitioner Appl. Status
	...
	IF il_facility_aif > 0 THEN ls_sql += " AND ( net_dev_action_items.facility_id = " + String( il_facility_aif ) + ")"
End if
...
IF li_rc = 0 THEN
	if ib_insertrow = true then   //Start Code Change ----07.30.2015 #V15 maha - to allow supression in  action items screen
		tab_1.tabpage_ai.dw_net_dev.InsertRow( 0 )
	end if
	//------------------- APPEON BEGIN -------------------
	//$<add> Michael 06.23.2011
...
//tab_1.tabpage_ai.dw_net_dev.Sort( ) //Start Code Change ----05.02.2014 #V14.2 maha - sorting is peculiar in this dw callinf sort doe not work properly
...
st_zoom.x = ddlb_group_by.x + ddlb_group_by.width + 22
...
public function integer of_set_facility_id (integer ai_facility_id);//Start Code Change ----08.10.2015 maha 
il_facility_aif = ai_facility_id  
ii_facility_id = ai_facility_id

return 1
end function

on pfc_ia_cst_nv_net_dev_new_temp.create
int iCurrent
call super::create
...
this.cb_track_recalc=create cb_track_recalc
...
this.Control[iCurrent+9]=this.cbx_compl
this.Control[iCurrent+10]=this.cb_change
this.Control[iCurrent+11]=this.st_filter
this.Control[iCurrent+12]=this.dw_qf
this.Control[iCurrent+13]=this.st_group_by
this.Control[iCurrent+14]=this.ddlb_group_by
this.Control[iCurrent+15]=this.st_zoom
this.Control[iCurrent+16]=this.ddlb_zoom
this.Control[iCurrent+17]=this.cb_track_recalc
end on

on pfc_ia_cst_nv_net_dev_new_temp.destroy
call super::destroy
...
type tab_1 from pfc_cst_nv_net_dev_prac_folder`tab_1 within pfc_ia_cst_nv_net_dev_new_temp
integer x = 997
integer y = 36
integer width = 3438
integer height = 1796
...
//	tab_1.tabpage_ai.gb_filter_1.Visible = True
	...
	cb_track_recalc.visible = true  //Start Code Change ----05.08.2014 #V14.2 maha
//changed to REPORTS
...
this.Control[iCurrent+3]=this.cb_calc
end on
...
type dw_orgs_facilities from pfc_cst_nv_net_dev_prac_folder`dw_orgs_facilities within pfc_ia_cst_nv_net_dev_new_temp
...
event resize;call super::resize;of_resize_manual()//Added by Appeon long.zhang 10.17.2014
end event
...
event clicked;//Start Code Change ----10.01.2013 #V14 maha
long ll_step
integer li_final
integer i
integer li_find
...
integer li_steps
integer li_temp
integer li_null  //maha 10.15.2013

if dw_tracking.rowcount() = 0 then return

li_final = integer(gnv_data.of_getitem("app_steps","last step",""))

setnull(ldt_null)
setnull(li_null) //maha 10.15.2013
//for each step
for i = 1 to li_final
	choose case i
		case 1
			...
			if li_find > 0 then
				dw_tracking.setitem(1,"step_1",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else //Start Code Change ----03.30.2011 #V11 maha - for each step, if the action does not exist clear the date
				dw_tracking.setitem(1,"step_1",ldt_null)
			end if
		case 2
		//	ll_step = dw_tracking.getitemnumber(1,"step_2_type")
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then 
				dw_tracking.setitem(1,"step_2",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_2",ldt_null)
			end if
		case 3
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then 
				dw_tracking.setitem(1,"step_3",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_3",ldt_null)
			end if
		case 4
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then
				dw_tracking.setitem(1,"step_4",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_4",ldt_null)
			end if
		case 5
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then
				dw_tracking.setitem(1,"step_5",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_5",ldt_null)
			end if
		case 6
		//	ll_step = dw_tracking.getitemnumber(1,"step_6_type")
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then
				dw_tracking.setitem(1,"step_6",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_6",ldt_null)
			end if
		case 7
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then
				dw_tracking.setitem(1,"step_7",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_7",ldt_null)
			end if
		case 8
		//	ll_step = dw_tracking.getitemnumber(1,"step_8_type")
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then 
				dw_tracking.setitem(1,"step_8",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_8",ldt_null)
			end if
		case 9
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then 
				dw_tracking.setitem(1,"step_9",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_9",ldt_null)
			end if
		case 10
			...
			li_find = dw_net_dev.find( ls_find, 1, dw_net_dev.rowcount())
			if li_find > 0 then 
				dw_tracking.setitem(1,"step_10",dw_net_dev.getitemdatetime(li_find,"action_date"))
			else 
				dw_tracking.setitem(1,"step_10",ldt_null)
			end if
	end choose
	...
	choose case i
		...
		case 2
			...
			if not isnull(ld_d1) and not isnull(ld_d2) then
				...
				dw_tracking.setitem(1,"days_1_2", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_1_2", li_null) 
			end if
		case 3
			...
			if not isnull(ld_d2) and not isnull(ld_d3) then
				...
				dw_tracking.setitem(1,"days_2_3", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_2_3", li_null) 
			end if
		case 4
			...
			if not isnull(ld_d3) and not isnull(ld_d4) then
				...
				dw_tracking.setitem(1,"days_3_4", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_3_4", li_null) 
			end if
		case 5
			...
			if not isnull(ld_d4) and not isnull(ld_d5) then
				...
				dw_tracking.setitem(1,"days_4_5", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_4_5", li_null) 
			end if
		case 6
			...
			if not isnull(ld_d5) and not isnull(ld_d6) then
				...
				dw_tracking.setitem(1,"days_5_6", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_5_6", li_null) 
			end if
		case 7
			...
			if not isnull(ld_d6) and not isnull(ld_d7) then
				...
				dw_tracking.setitem(1,"days_6_7", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_6_7", li_null) 
			end if
		case 8
			...
			if not isnull(ld_d7) and not isnull(ld_d8) then
				...
				dw_tracking.setitem(1,"days_7_8", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_7_8", li_null) 
			end if
		case 9
			...
			if not isnull(ld_d8) and not isnull(ld_d9) then
				...
				dw_tracking.setitem(1,"days_8_9", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_8_9", li_null) 
			end if
		case 10
			...
			if not isnull(ld_d9) and not isnull(ld_d10) then
				...
				dw_tracking.setitem(1,"days_9_10", li_temp)
			else //Start Code Change ----10.15.2013 #V14 maha
				dw_tracking.setitem(1,"days_9_10", li_null) 
			end if
				
	end choose
next

//Start Code Change ----10.15.2013 #V14 maha - rewrote calc total adding null values
//calc total
choose case li_final
	...
	case 9
		if not isnull(ld_d1) and not isnull(ld_d9) then
			...
			dw_tracking.setitem(1,"days_total", li_temp)
		end if
	case 10
...
string picturename = "US-dollar.jpg"
...
event clicked;of_open_tracking()
end event
...
long backcolor = 67108864
boolean enabled = false
string text = "Include Completed Actions"
...
fontfamily fontfamily = swiss!
string facename = "Segoe UI"
string text = "Recalculate Tracking"
end type
...
//------------------- APPEON BEGIN -------------------
//<$>added:long.zhang 10.28.2014
//<$>reason: BugS091802 of V14.2 Issues of New Features by ISG

//dw_tracking.update()  //Start Code Change ----05.22.2014 #V14.2 maha - added save
of_save()
