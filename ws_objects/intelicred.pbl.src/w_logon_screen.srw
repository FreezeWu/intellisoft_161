...
type st_welcome from statictext within w_logon_screen
end type
type p_gear from picture within w_logon_screen
end type
type st_isg from statictext within w_logon_screen
end type
type cb_cancel_button from commandbutton within w_logon_screen
end type
type cb_1 from commandbutton within w_logon_screen
end type
type pb_5 from picturebutton within w_logon_screen
end type
type pb_9 from picturebutton within w_logon_screen
end type
type pb_3 from picturebutton within w_logon_screen
end type
type pb_8 from picturebutton within w_logon_screen
end type
type pb_2 from picturebutton within w_logon_screen
end type
type pb_6 from picturebutton within w_logon_screen
end type
type pb_7 from picturebutton within w_logon_screen
end type
type pb_4 from picturebutton within w_logon_screen
end type
type pb_1 from picturebutton within w_logon_screen
end type
type p_2 from picture within w_logon_screen
end type
...
type r_1 from rectangle within w_logon_screen
end type
type st_iss from statictext within w_logon_screen
end type
type p_logo from picture within w_logon_screen
end type
type cb_support from commandbutton within w_logon_screen
end type
type st_forgot_pw from statictext within w_logon_screen
end type
type dw_image from datawindow within w_logon_screen
end type
type st_id from statictext within w_logon_screen
end type
type sle_user_id from singlelineedit within w_logon_screen
end type
type st_pw from statictext within w_logon_screen
end type
type sle_password from singlelineedit within w_logon_screen
end type
type cb_ok from u_cb within w_logon_screen
end type
type cb_close from u_cb within w_logon_screen
end type
type st_program from statictext within w_logon_screen
end type
...
//-------------------------------- RP 1.26.2016 -------------------------------------
constant long COLOR_BACKGROUND =    rgb(135, 117, 182  )  //Start Code Change ----10.23.2017 #V16 maha
constant long COLOR_TEXT =  rgb(255, 255, 255)    //Start Code Change ----10.23.2017 #V16 maha
//constant long COLOR_BACKGROUND =    rgb(34, 13, 70  ) //new purple		//RGB(41 ,38, 109)        // rgb(25, 48, 72) slate blue
//constant long COLOR_TEXT =  rgb(177, 175, 182)   //rgb(172, 172, 172)
//constant long INTELLI_PURPLE = RGB(80 ,0, 170) v92
//constant long INTELLI_PURPLE = RGB(35 ,0, 105)
//constant long INTELLI_PURPLE = rgb(101, 100, 106)		//RGB(41 ,38, 109)
...
//constant long INTELLI_GREY = rgb(101, 100, 106)
...
Boolean ib_blocking = False //Added by Appeon long.zhang 02.17.2017 (Login bug# 5502)
...
public function integer of_get_user_global_infor (string as_user_id)
public subroutine of_set_text_colors ()
end prototypes
...
integer li_cvo  //maha 02.07.2017
...
//***NOTE: WE NEED TO CONSIDER AD LOGIN about executing following these sql again when Processing AD User. Added by Appeon long.zhang 03.23.2017
//get user info 
...
select opt_2 into :li_opt2 from security_settings;
 //Start Code Change ----02.07.2017 #V153 maha - cvo session
 gnv_appeondb.of_commitqueue() //alfee 04.08.2013

if gb_cvo_combo = true then 
	select  isg_cvo into :li_cvo  from security_users where upper(user_id) = upper(:ls_user);
	if li_cvo > 0 then gb_cvo_user = true
end if
...
if upper(ls_user) = "MASTER" then
	...
	If ib_ad_autologin Then
		...
		gnv_app.of_setuserid(gs_user_id) //Added by Appeon long.zhang 03.14.2017 (V15.3 Bug # 5539 - Documents tab: Can't upload documents - getting SQL error No rows effected)
		return 1 
	End If
	ls_ad_error = ''
	gb_ad_login = False
	If gnv_user_option.of_get_option_value('allow_ad_function') = '1'  Then
		...
		If sqlca.sqlcode = 0 and li_ad_status = 1  and (Not(li_active_status = 0) ) Then
			If inv_ad.of_connect_ad( ls_ad_pc_domainname, ls_ad_user, ls_passorg, ref ls_ad_error, True, True) = 0 Then
				...
				of_get_user_global_infor(ls_userid) //Added by Appeon long.zhang 03.27.2017 (to keep the gi_role_id, gi_role_id logic same with Maha's script below)
		...
		Else
			If  sqlca.sqlcode = 0 Then
				If li_ad_status = 0 or isnull(li_ad_status) Then
					...
					End If
				End If
				If  li_active_status = 0  Then //Modified from 1 to 0, by Appeon long.zhang 02.15.2017 (BugS011702)
...
public function integer of_get_user_global_infor (string as_user_id);//====================================================================
//$<Function>: of_get_user_global_infor
//$<Arguments>:
// 	value    string    as_user_id
//$<Return>:  integer
//$<Description>: Get Global User Information. E.g.: gi_role_id, gb_cvo_user
//$<Author>: (Appeon) long.zhang 03.27.2017 (v15.3 AD Authentication)
//--------------------------------------------------------------------
//$<Modify History>:
//====================================================================
Int li_role, li_cvo

//Get gi_role_id by the real user_id, Added by Appeon long.zhang 03.23.2017 (to keep the gi_role_id logic same with Maha's script)
select role_id into :li_role from security_users where upper(user_id) = upper(:as_user_id);
gi_role_id = li_role

//cvo session, Copied from above for AD, Added by Appeon long.zhang 03.23.2017 (to keep the gi_role_id logic same with  Maha's gb_cvo_user script)
if gb_cvo_combo = true then 
	select  isg_cvo into :li_cvo  from security_users where upper(user_id) = upper(:as_user_id);
	if li_cvo > 0 then gb_cvo_user = true
end if
...
public subroutine of_set_text_colors (); //Start Code Change ----03.30.2017 #V153 maha - added removed from construct event of each object - call from open

st_program.backcolor = color_background
st_id.backcolor = color_background
st_pw.backcolor = color_background
st_forgot_pw.backcolor = color_background
st_version.backcolor = color_background
st_program.textcolor = color_text
st_id.textcolor = color_text
st_pw.textcolor = color_text
st_forgot_pw.textcolor = color_text
...
select cvo_combo into :li_cvo from icred_settings; 
gnv_appeondb.of_commitqueue()
//---------------------------- APPEON END ----------------------------
...
gb_cvo_user = false   //Start Code Change ----02.07.2017 #V153 maha - set default in case of relogin 
...
st_version.text+=  " " + CharA ( 169 ) + "ISG Group LLC 1998-2017" //maha 04.02.2010 - added //modified to 2017, by Appeon long.zhang 04.18.2017 (BugL041802)
...
if li_banner = 1 and not gb_IsAutoExpImp and not gb_AutoSchedule then //web based scheduler - alfee 01.27.2011
	//------------------- APPEON BEGIN -------------------
	//<$>added:long.zhang 02.17.2017
	//<$>reason:Stop execute pfc_postopen() when blocking, walkaround for Login bug# 5502
	//open(w_security_banner)
	ib_blocking = True
	open(w_security_banner)
	ib_blocking = False
	This.Post Event pfc_postopen() //Re-execute pfc_postopen()
	//------------------- APPEON END -------------------	
...
//End Code Change---04.01.2009
 ...
 //Start Code Change ----02.07.2017 #V153 maha - for cvo session bypass
if li_cvo = 1 then
	gb_cvo_combo = true
end if
  //End Code Change ----02.07.2017
...
If Not gb_ad_command_off and Not lb_ReLogin and gs_verifycode = '' and (gnv_user_option.of_get_option_value('allow_ad_function') = '1'  )Then
	...
	If ls_ad_pc_domainname <> '' Then		
		...
		If sqlca.sqlcode = 0 and li_ad_status = 1 and (li_active_status_user = 1 or isnull(li_active_status_user))  Then
			...
			If st_forgot_pw.visible Then st_forgot_pw.visible = False //Hide forgot pw when using AD, Added by Appeon long.zhang 02.15.2017
...
if gs_idpname<>"" and gs_verifycode<>"" and AppeonGetClientType() = 'WEB' then
	...
	of_get_user_global_infor(ls_mapping_user) //Added by Appeon long.zhang 03.27.2017 (to keep the gi_role_id, gi_role_id logic same with Maha's script)
...
If ib_blocking Then Return //Added by Appeon long.zhang 02.17.2017 (For Login bug# 5502)
...
integer li_cvo  //maha 03.21.2017
integer li_zoom
date ld_password
datetime ldt_lock //Start Code Change ---- 08.02.2007 #V7 maha
time lt_time
...
ElseIf gb_autologin = True Then
	...
	gs_user_id = sle_user_id.text
	li_res = 1
	 //Start Code Change ----03.21.2017 #V153 maha - cvo session
	if gb_cvo_combo = true then 
		select  isg_cvo into :li_cvo  from security_users where upper(user_id) = upper(:gs_user_id);
		if li_cvo > 0 then gb_cvo_user = true
	end if
...
//Start Code Change ----08.01.2017 #V154 maha
select font_zoom into :li_zoom from security_users where user_id = :gs_user_id;
if isnull(li_zoom) or li_zoom < 100 then 
	gi_zoom = 100
else
	gi_zoom = li_zoom
end if
//Start Code Change ----08.01.2017 
