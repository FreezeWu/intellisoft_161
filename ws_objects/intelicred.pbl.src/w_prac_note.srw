...
type dw_detail from datawindow within w_prac_note
end type
type dw_prac_notes from datawindow within w_prac_note
end type
type rb_both from radiobutton within w_prac_note
end type
type rb_calls from radiobutton within w_prac_note
end type
type rb_notes from radiobutton within w_prac_note
end type
type cb_filter from commandbutton within w_prac_note
end type
type cb_sort from commandbutton within w_prac_note
end type
type dw_print from datawindow within w_prac_note
end type
type gb_1 from groupbox within w_prac_note
end type
type p_1 from picture within w_prac_note
end type
...
type dw_facil from datawindow within w_prac_note
end type
type gb_view from groupbox within w_prac_note
end type
end forward
...
i = dw_prac_notes.retrieve(il_prac_id,gs_user_id )
...
idw_current = dw_prac_notes	//alfee 11.18.2016	
...
event clicked;//Added by Appeon long.zhang 12.10.2015 (Allegheny Unable to print notes from provider folder Case#60592 Bug_id#4885)
If idw_current <> This Then
	idw_current = This
End If
end event
...
If row < 1 Then Return //Added by Appeon long.zhang 02.22.2016 (BugL022201)
...
//Added by Appeon long.zhang 12.10.2015 (Allegheny Unable to print notes from provider folder Case#60592 Bug_id#4885)
If idw_current <> This Then
	idw_current = This
End If
end event
...
if appeongetclienttype() <> "PB" then	//Web version
	...
	If idw_current = dw_prac_notes Then //modified by Appeon long.zhang 12.10.2015 (Allegheny Unable to print notes from provider folder Case#60592 Bug_id#4885)
...
//if tab_1.selectedtab = 1 then
If idw_current= dw_prac_notes Then //Uncomment and modified by Appeon long.zhang 12.10.2015 (Allegheny Unable to print notes from provider folder Case#60592 Bug_id#4885)
	dw_print.dataobject = "d_pd_notes_browse_print"
	dw_print.settransobject(sqlca)
...
dw_detail.setitem(1,"pd_notes_prac_id",il_prac_id)
//tab_1.tabpage_detail.dw_detail.setitem(1,"pd_notes_ent_date",today())
dw_detail.setitem(1,"pd_notes_ent_date",datetime(today(),now())) //for a web bug - alfee 05.07.2009
dw_detail.setitem(1,"pd_notes_user_id",gs_user_id)
dw_detail.setitem(1,"pd_notes_facility_id",ii_facid)
dw_detail.setitem(1,"note_type", 1)  //Start Code Change ----07.28.2014 #V14.2 maha - set the type value
dw_detail.setfocus() //Start Code Change ----10.14.2008 #V85 maha
...
dw_detail.accepttext( ) //Added by Appeon Harry 08.04.2016 (Usability id 5272 - Saving Provider Notes after appending)
if dw_detail.rowcount() < 1 then return
If dw_detail.Modifiedcount( ) <1 Then Return //Added by Appeon long.zhang 12.10.2015 ( Allegheny Unable to print notes from provider folder Case#60592 Bug_id#4885)
...
dw_prac_notes.retrieve(il_prac_id,gs_user_id) 
