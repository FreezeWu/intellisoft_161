$PBExportHeader$u_cst_gadget_notes_new.sru
forward
global type u_cst_gadget_notes_new from u_cst_gadget
end type
end forward
...
global u_cst_gadget_notes_new u_cst_gadget_notes_new
...
public subroutine of_highlight_row (integer al_row)
...
this.setredraw(false)  //(Appeon)Stephen 12.21.2015 - V14.2 Applause Cycle 2 Cosmetic Bug #4900 - Dashboard-NOTE- on clicking add for note, extra add button shows
li_row = Dw_content.InsertRow(1)
dw_content.SetRow(li_row)
dw_content.ScrollToRow(li_row)
//---------Begin Added by (Appeon)Stephen 12.21.2015 for V14.2 Applause Cycle 2 Cosmetic Bug #4900 - Dashboard-NOTE- on clicking add for note, extra add button shows--------
dw_content.selectrow(0, false)
dw_content.selectrow(li_row, true)
//---------End Added ------------------------------------------------------
...
this.setredraw(true)  //(Appeon)Stephen 12.21.2015 - V14.2 Applause Cycle 2 Cosmetic Bug #4900 - Dashboard-NOTE- on clicking add for note, extra add button shows
...
If len(ls_note) > 10 Then
	ls_note_mess = Mid(ls_note,1,10)+'...'
Else
...
If dw_content.update() = 1 Then
	...
	dw_content.Modify("b_edit.Enabled=no")  //(Appeon)Harry 02.03.2016 - V15.1 Applause Cycle 3 Bug # 4967 - Dashboard: issues with notes - 2
	lw_note = of_find_note_win(al_row)
	If isvalid(lw_note) Then
...
If dw_content.Getrow() > 0 Then
	...
	Else
		...
		dw_content.Modify("b_edit.Enabled=yes")   //(Appeon)Harry 02.03.2016 - V15.1 Applause Cycle 3 Bug # 4967 - Dashboard: issues with notes - 2
	End If
End If
...
OpenwithParm(w_dashboard_gadget_notes_config,This)
...
on u_cst_gadget_notes_new.create
call super::create
end on

on u_cst_gadget_notes_new.destroy
call super::destroy
end on
...
type dw_content from u_cst_gadget`dw_content within u_cst_gadget_notes_new
...
type ln_split from u_cst_gadget`ln_split within u_cst_gadget_notes_new
end type

type st_1 from u_cst_gadget`st_1 within u_cst_gadget_notes_new
end type

type st_title from u_cst_gadget`st_title within u_cst_gadget_notes_new
end type

type pb_close from u_cst_gadget`pb_close within u_cst_gadget_notes_new
end type
...
type pb_max from u_cst_gadget`pb_max within u_cst_gadget_notes_new
end type

type pb_edit from u_cst_gadget`pb_edit within u_cst_gadget_notes_new
end type
