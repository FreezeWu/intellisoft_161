$PBExportHeader$w_npdb_report.srw
$PBExportComments$Used to display the data in the datastord
forward
global type w_npdb_report from w_sheet
end type
type cb_close from commandbutton within w_npdb_report
end type
type st_2 from u_st_splitbar within w_npdb_report
end type
type st_1 from statictext within w_npdb_report
end type
type dw_1 from u_dw within w_npdb_report
end type
type cb_7 from commandbutton within w_npdb_report
end type
type cb_submission from commandbutton within w_npdb_report
end type
type cb_response from commandbutton within w_npdb_report
end type
type cb_1 from commandbutton within w_npdb_report
end type
type cb_4 from commandbutton within w_npdb_report
end type
type cb_6 from commandbutton within w_npdb_report
end type
type tv_1 from u_tv within w_npdb_report
end type
end forward
...
global w_npdb_report w_npdb_report
...
if upperbound(invo.ids_report_dws) > 0  then
	...
	if  not f_validstr(invo.ids_report_dws[1].object.compute_9[1]) then 
		...
		ll_lev1 = tv_1.InsertItemLast(0,  'No Reports Found',1)
	end if
	...
	messagebox(title, "No report file has been been processed for this provider verification.")
	close(this)
end if
...
if  gi_email_type = 1 then //Start Code Change ----12.11.2012 #V12 maha
	Super::EVENT ue_mail_bak('NPDB Reports for : ' + wordcap(lower(trim(parent.dw_1.object.compute_9[1])))  ,'' )
else
