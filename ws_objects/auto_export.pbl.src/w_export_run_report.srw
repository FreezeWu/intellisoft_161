$PBExportHeader$w_export_run_report.srw
forward
global type w_export_run_report from window
end type
type cb_1 from commandbutton within w_export_run_report
end type
type st_1 from statictext within w_export_run_report
end type
type sle_1 from singlelineedit within w_export_run_report
end type
type dw_export_file from datawindow within w_export_run_report
end type
type cb_quit from u_cb within w_export_run_report
end type
type cb_test from commandbutton within w_export_run_report
end type
type dw_select from datawindow within w_export_run_report
end type
type gb_1 from groupbox within w_export_run_report
end type
end forward
...
global w_export_run_report w_export_run_report

type variables
long il_export_id
end variables

on w_export_run_report.create
...
on w_export_run_report.destroy
destroy(this.cb_1)
destroy(this.st_1)
destroy(this.sle_1)
...
//---------Begin Modified by (Appeon)Toney 06.20.2013 for V141 ISG-CLX--------
  /*
	value = GetFileSaveName("Select Save location and name for file", docname, named )
...
long ic
long ll_pracid
long ll_log //maha 07.18.2016
integer li_savedata = 0
integer li_type1
integer res //maha 02042011
String  ls_SQL = ''
Long    ll_pracidold
Long    ll_Num
...
choose case ls_prompt1
	case "C","D","N","F"   //Start Code Change ----02.04.2011 #V11 maha - added F
...
IF isnull(ls_sny_retrieve) or ls_sny_retrieve = "" then
	...
	*/
	...
	//<add> 07/17/2007 by: Andy
	li_type1 = 1
	//ls_SQL = ls_sny_retrieve - commented and moved below by alfee 10.31.2008
	//end of add
	...
	ls_SQL = ls_sny_retrieve //alfee 10.31.2008
	...
	select export_name into : ls_name from export_header where export_id = :il_export_id;  //Start Code Change ----07.26.2016 #V152 maha - name parameter
	ll_log = of_log_sys_report("E","MAN","", il_export_id , ls_name)  //Start Code Change ----06.06.2016 #V152 maha	
	ll_prac_row = lds_export_retrieve.retrieve()
	if ll_prac_row < 1 then 
...
if li_type = 3 or li_type = 4 or li_type = 6 then //(Appeon)long.zhang 05.17.2014 (v14.2 Provider-Contract direct linking)
	//---------Begin Modified by (Appeon)Toney 06.20.2013 for V141 ISG-CLX--------	
	  ...
	  */
	 res = n_export.of_export_data_with_text_ctx( il_export_id, prac_ids,ls_path,li_save_data, "", "", "",li_type1,ls_sql )
	//---------End Modfiied ------------------------------------------------------------------	
else
	res = n_export.of_export_data_with_text( il_export_id, prac_ids,ls_path,li_save_data,"","","",li_type1,ls_sql )
end if

of_log_sys_report_end( ll_log, gl_export_rows_count,"")  //Start Code Change ----07.18.2016 #V152 maha

//i = run('"' + ls_path + '"')
//if i < 1 then
//	messagebox("Run failed",ls_path)
//end if
//---------------------------- APPEON END ----------------------------
destroy n_export;
...
integer taborder = 60
boolean bringtotop = true
string dataobject = "d_export_hdr_report_list"
...
cnt = dw_export_file.Retrieve( il_export_id )
...
//$<reason> Filter unmatched exports
dwchild.settransobject(sqlca)
dwchild.retrieve()
If gb_contract_version Then
	dwchild.setfilter("export_type in (3,4)")
	dwchild.filter()
Else
	dwchild.setfilter('')
	dwchild.filter()
