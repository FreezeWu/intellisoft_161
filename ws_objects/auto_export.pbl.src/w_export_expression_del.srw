$PBExportHeader$w_export_expression_del.srw
$PBExportComments$(Appeon)Toney 06.20.2013 - V141 ISG-CLX replace with the same name object that is from CLX
forward
global type w_export_expression_del from w_response
end type
type lb_functions from listbox within w_export_expression_del
end type
type gb_3 from groupbox within w_export_expression_del
end type
type dw_2 from u_dw within w_export_expression_del
end type
type cb_9 from commandbutton within w_export_expression_del
end type
type cb_8 from commandbutton within w_export_expression_del
end type
type cb_7 from commandbutton within w_export_expression_del
end type
type cb_6 from commandbutton within w_export_expression_del
end type
type cb_5 from commandbutton within w_export_expression_del
end type
type cb_4 from commandbutton within w_export_expression_del
end type
type dw_1 from u_dw within w_export_expression_del
end type
type cb_3 from commandbutton within w_export_expression_del
end type
type cb_2 from commandbutton within w_export_expression_del
end type
type cb_1 from commandbutton within w_export_expression_del
end type
type mle_1 from multilineedit within w_export_expression_del
end type
type gb_1 from groupbox within w_export_expression_del
end type
type gb_2 from groupbox within w_export_expression_del
end type
end forward
...
global w_export_expression_del w_export_expression_del
...
if ls_Err <> '' then
	messagebox("Expression error","Expression is not valid.  " + ls_source)
	mle_1.setfocus( )
...
if  not isnumber(ls_Err) then 
	//not OK
	if ls_Err = '!' then
		messagebox("Evaluate Error","Expression is not valid." + ls_source)
		mle_1.setfocus( )
		if IsValid(n_export) then Destroy n_export
		return -1
	else
...
if Not IsValid(istr_pass) then 
	messagebox("string is not valid","expression is not valid.")
	mle_1.setfocus( )
	return -1
end if

if Not IsValid(istr_pass.s_u_dw) or upperbound(istr_pass.s_string_array) = 0 then
	messagebox("Paramaters not valid","expression is not valid.")
	mle_1.setfocus( )
	return -1
end if
...
if ls_Err <> '' then
	messagebox("Syntax error","Expression is not valid." + ls_systax)
	mle_1.setfocus( )
...
on w_export_expression_del.create
int iCurrent
call super::create
...
on w_export_expression_del.destroy
call super::destroy
...
//end of add
pfc_cst_u_band ldw_band
w_contract_data_design lw_parent
String ls_ModifySyn
