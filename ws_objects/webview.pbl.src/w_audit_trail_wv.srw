$PBExportHeader$w_audit_trail_wv.srw
forward
global type w_audit_trail_wv from pfc_w_response
end type
type st_3 from statictext within w_audit_trail_wv
end type
type dw_select from datawindow within w_audit_trail_wv
end type
type dw_prac_facil from datawindow within w_audit_trail_wv
end type
type cb_close from commandbutton within w_audit_trail_wv
end type
type dw_link_list from datawindow within w_audit_trail_wv
end type
type dw_addresses from u_dw within w_audit_trail_wv
end type
type tab_1 from tab within w_audit_trail_wv
end type
...
type tabpage_email from pfc_cst_u_tabpg_wv_audit_email within tab_1
end type
type tabpage_email from pfc_cst_u_tabpg_wv_audit_email within tab_1
end type
type tab_1 from tab within w_audit_trail_wv
...
type dw_detail from u_dw within w_audit_trail_wv
end type
end forward
...
global w_audit_trail_wv w_audit_trail_wv
...
n_cst_wv_audit_email inv_wv_audit_email //Added by Appeon long.zhang 06.29.2017 (v15.4 WebView Email Notifications)
...
public function integer of_create_audit_email (integer ai_tab, u_dw adw_detail, datetime adt_create)
protected function integer of_create_dynamic_dw (integer ai_tab, long al_row, u_dw adw_detail, string as_type)
end prototypes
...
string ls_field_type, ls_value_new
integer  li_pos
Int li_rtn //Added by Appeon long.zhang 06.30.2017 (v15.4 WebView Email Notifications)
datetime ldt_value_date
string  ls_field, ls_screen_name
string  ls_rec_id_sql, ls_audit_type_next
String ls_date //Added by Appeon long.zhang 07.05.2017 (v15.4 WebView Email Notifications)
long  ll_field_id, ll_prac_id
...
if adw_detail.rowcount() < 1 then return -1
ldt_date = datetime( today(),now())
ls_date = string(ldt_date, 'yyyy-mm-dd hh:mm:ss:fff') //Added by Appeon long.zhang 07.05.2017 (v15.4 WebView Email Notifications)
...
//				end if
			...
			elseif  ls_audit_type = 'EQW' then	
				...
				ls_sql = "update sys_audit set audit_pending = 0, auditor ='" + gs_user_id + "', audit_date = '" + string(ldt_date) + "' where table_id = " + string(ll_table_id) +&
							...
							*/
				ls_sql = "update sys_audit set audit_pending = 0, auditor ='" + gs_user_id + "', audit_date = '" + ls_date + "' where table_id = " + string(ll_table_id) +&
						   " and seq_no  = " + string(ll_seq_no) + " and rec_id in(" + ls_rec_id_sql +")"
				//---------End Modfiied ------------------------------------------------------
...
//					end if
			...
			elseif  ls_audit_type = 'EQW' then
				...
				ls_sql = "update sys_audit set audit_pending = 0, auditor ='" + gs_user_id + "', audit_date = '" + string(ldt_date) + "' where table_id = " + string(ll_table_id) +&
						   " and seq_no  = " + string(ll_seq_no) + " and rec_id in(" + ls_rec_id_sql +")"
							*/
				ls_sql = "update sys_audit set audit_pending = 0, auditor ='" + gs_user_id + &
							"', audit_date = '" + ls_date + &
							"', audit_rejected = 1 "  + &
							" where table_id = " + string(ll_table_id) +&
						   " and seq_no  = " + string(ll_seq_no) + " and rec_id in(" + ls_rec_id_sql +")" //add audit_rejected = 1, Added by Appeon long.zhang 07.05.2017 (v15.4 WebView Email Notifications)
				//---------End Modfiied ------------------------------------------------------
				...
				update sys_audit
				  ...
				  where rec_id   = :ll_rec_id
					 and seq_no  = :ll_seq_no
					 and field_id  = :ll_field_id; //add audit_rejected = 1, Added by Appeon long.zhang 07.05.2017 (v15.4 WebView Email Notifications)
			end if
...
//Insert wv_audit email if needed, Added by Appeon long.zhang 06.30.2017 (v15.4 WebView Email Notifications)
li_rtn = of_create_audit_email(ai_tab, adw_detail, ldt_date) 
If li_rtn < 0 Then
	messagebox("Prompt", "Failed to create WebView Audit Email record. Error Code:" + String(li_rtn) +'.')
end if
...
string ls_filter
string ls_pracs
String ls_prac_id_column_name//Added by Appeon long.zhang 07.01.2017 (v15.4 WebView Email Notifications)
integer res
integer r
long pc
long ll_prac
...
//------------------- APPEON BEGIN -------------------
//<$>added:long.zhang 07.01.2017
//<$>reason:v15.4 WebView Email Notifications
If adw_list.dataobject = 'd_wv_audit_email_list' Then //Emails tabpage
	ls_prac_id_column_name = "prac_id"
Else
	ls_prac_id_column_name = "v_full_name_prac_id"
End If
//------------------- APPEON END -------------------
...
for r = 1 to adw_list.rowcount()
	...
	ll_pracs[r] = adw_list.getitemnumber(r, ls_prac_id_column_name)
next
...
ls_filter = ls_prac_id_column_name + " in (" + ls_pracs + ")" //modified by Appeon long.zhang 07.01.2017 (v15.4 WebView Email Notifications)
...
public function integer of_create_audit_email (integer ai_tab, u_dw adw_detail, datetime adt_create);//====================================================================
//$<Function>: of_create_audit_email
//$<Arguments>:
// 	value    integer       ai_tab
// 	value    u_dw    adw_detail
// 	value    datetime      adt_create
//$<Return>:  integer  0:don't need to create email/ -x:insert failed/ 1: succeed
//$<Description>: Insert records to wv_audit_email, these records will be used to send mails later.
...
Long ll_cnt
Long ll_prac_id
Boolean lb_skip_flag = False
Boolean lb_accept_flag = False
Boolean lb_reject_flag = False
Boolean lb_edit_flag = False
String ls_opt_type
String ls_create_user
...
If lb_accept_flag and not (lb_skip_flag or lb_reject_flag or lb_edit_flag) Then //Alway accept
	ls_accept_reject = 'A'
ElseIf lb_reject_flag Then //If exist any Reject Action
	ls_accept_reject = 'R'
ElseIf lb_edit_flag and adw_detail.rowcount() = 1 Then //Accept when only has one Edit Action, Added by Appeon long.zhang 07.24.2017 (v15.4 WebView Email Notifications)
	ls_accept_reject = 'A'
Else //Only generate email
	Return 0
End If

//Aalways Set Facility to 0 when created, it will be updated after sending email.
ll_facility_id = 0

ll_prac_id = adw_detail.GetItemNumber(1, 'sys_audit_prac_id')
ls_create_user = gs_user_id

//Insert wv_audit email if needed.
li_rtn = inv_wv_audit_email.of_add_email_record(ll_prac_id, ll_facility_id, ls_create_user, ls_accept_reject, adt_create)
If li_rtn < 0 Then
	Return li_rtn
End If
...
long      ll_row, ll_prac_id, ll_seq, ll_pos
integer  li_screen_id,  li_facility
Int li_rtn //Added by Appeon long.zhang 07.24.2017 (v15.4 WebView Email Notifications)
string    ls_audit_type, ls_fieldname
long      ll_rec_id, ll_field_id, ll_table_id
datetime ldt_date
string ls_msg
...
if as_type = 'edit' then 
	...
	//Insert wv_audit email if needed, Added by Appeon long.zhang 07.24.2017 (v15.4 WebView Email Notifications)
	If adw_detail.RowCount() = 1 Then
		li_rtn = of_create_audit_email(ai_tab, adw_detail, ldt_date) 
		If li_rtn < 0 Then
			messagebox("Prompt", "Failed to create WebView Audit Email record. Error Code:" + String(li_rtn) +'.')
		end if
...
inv_wv_audit_email = Create n_cst_wv_audit_email//Added by Appeon long.zhang 07.03.2017 (v15.4 WebView Email Notifications)
...
Destroy inv_wv_audit_email //Added by Appeon long.zhang 07.03.2017 (v15.4 WebView Email Notifications)
...
choose case tab_1.selectedtab
	...
	case 4
		return 1
	Case 5  //Always Set facility id instance variable for tabpage_email, Added by Appeon long.zhang 07.01.2017 (v15.4 WebView Email Notifications)
		tab_1.tabpage_email.Event ue_facility_filter_changed(Long(data)) 
		return 0
end choose

//When changing facility id in other tabs, always Set facility id for tabpage_email, Added by Appeon long.zhang 07.01.2017 (v15.4 WebView Email Notifications)
tab_1.tabpage_email.of_set_facility_id( il_facility)
...
//Added by Appeon long.zhang 07.31.2017 (BugS072403)
...
//------------------- APPEON BEGIN -------------------
//<$>added:long.zhang 06.29.2017
//<$>reason:speical logic for emails (index = 5), v15.4 WebView Email Notifications
//of_retrieve_dw(newindex)
If NewIndex = 5 Then
	tab_1.tabpage_email.Event ue_selectionchanged()
	Return
Else
	of_retrieve_dw(newindex)
End If 
