$PBExportHeader$pfc_n_cst_filesrv.sru
...
public function integer of_deltree (string as_directoryname, boolean ab_delete_current_dir)
end prototypes
...
If RightA(ls_Path, 1) <> is_Separator Then	ls_Path = ls_Path + is_Separator
...
//Begin - Added By Mark Lee 05/02/2013	 Rewrite it, because if the user open the PDF file use the OLE Control and then read the PDF file again, it will cannot obtain any data.
long			ll_FileLen
n_cst_dm_utils lnv_dm_utils

If lnv_dm_utils.of_readblob( as_FileName, ablb_Data) < 0 Then 
	Return -1
Else
	ll_FileLen = FileLength(as_FileName)
	Return ll_FileLen
End IF 
//End - Added By Mark Lee 05/02/2013
//---------End Modfiied ------------------------------------------------------
end function
...
For li_Cnt = 1 To li_Writes
	ls_Text = MidA(as_Text, ll_CurrentPos, 32765)
	ll_CurrentPos += 32765
...
//FILE_ATTRIBUTE_NOT_CONTENT_INDEXED=8192 added by nova 12.22.2009
 If (lb_ReadWrite And (aul_FileAttrib = 0 + 8192 Or &
							 	aul_FileAttrib = 1 + 8192 Or &
								aul_FileAttrib = 32 + 8192 Or &
								aul_FileAttrib = 33 + 8192 Or &
								aul_FileAttrib = 128 + 8192 )) Then Return True								
...
// PowerBuilder Foundation Classes (PFC) source code by other than Sybase, Inc. and its subsidiaries is prohibited.
//////////////////////////////////////////////////////////////////////////////
//---------Begin Modified by (Appeon)Toney 06.03.2013 for V141 ISG-CLX--------
    /*
      	integer				li_RC
		...
		If RightA(as_DirectoryName, 1) <> is_Separator Then
		...
		For ll_Cnt = 1 To ll_Entries
			If lnv_DirList[ll_Cnt].ib_SubDirectory Then
				...
				If lnv_DirList[ll_Cnt].is_FileName <> "[..]" Then
					...
					ls_SubDirectory = MidA(lnv_DirList[ll_Cnt].is_FileName, 2, &
						(LenA(lnv_DirList[ll_Cnt].is_FileName) - 2))
    ...
    */
//Modified By Ken.Guo 04/25/2012. Overload this functionl
Return  This.of_deltree(as_directoryname, True)
//---------End Modfiied ------------------------------------------------------
...
If IsNull(as_path) or LenA(Trim(as_path))=0 Then	Return -1
...
If li_Pos = 0 Then
	as_Drive = ""
Else
	If MidA(as_Path, (li_Pos + 1), 1) = is_Separator Then
		li_Pos ++
	End if

	as_Drive = LeftA(as_Path, li_Pos)
	as_Path = RightA(as_Path, (LenA(as_Path) - li_Pos))
End if

// Get the file name and extension
li_Pos = lnv_string.of_LastPos(as_Path, is_Separator, 0)
ls_File = RightA(as_Path, (LenA(as_Path) - li_Pos))
as_Path = LeftA(as_Path, li_Pos)
...
Else
	...
	If li_Pos > 0 Then
		as_FileName = LeftA(ls_File, (li_Pos - 1))
		as_Ext = RightA(ls_File, (LenA(ls_File) - li_Pos))
	Else
		as_FileName = ls_File
		as_Ext = ""
	End if
...
for ll_cnt = 1 to ll_limit
	...
	If anv_DirList[ll_Cnt].ib_Subdirectory Then
		ls_sortfilename = MidA(lnv_direntry.is_FileName, 2, (LenA(lnv_direntry.is_FileName) - 2))
		of_DirAttribToDS(lds_files, lnv_direntry, ls_sortfilename, 0)
	ElseIf anv_DirList[ll_Cnt].ib_Drive Then
		ls_sortfilename = MidA(lnv_direntry.is_FileName, 3, 1)
...
ll_Pos = PosA(as_sortfilename, ".")
If ll_Pos = 0 Or as_sortfilename = ".." Then
	ls_Name = as_sortfilename
	ls_Ext = ""
Else
	ls_Name = LeftA(as_sortfilename, (ll_Pos - 1))
	ls_Ext = RightA(as_sortfilename, (LenA(as_sortfilename) - ll_Pos))
End If
...
// Function: of_deltree
//--------------------------------------------------------------------
// Description: Overload this function. add argument.
//--------------------------------------------------------------------
// Arguments:
//                as_DirectoryName
//                ab_delete_current_dir: Whether delete current dir.
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// Modify History: Pasted Script From of_deltree(as_DirectoryName)
//(Appeon)Toney 06.03.2013 - V141 ISG-CLX
//====================================================================
...
If ab_delete_current_dir Then //Added By Ken.Guo 04/25/2012. Add if statement.
	Return of_RemoveDirectory(as_DirectoryName)
Else
	Return 1
End If
