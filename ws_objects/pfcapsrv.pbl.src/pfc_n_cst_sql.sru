...
// be separated
li_PosU = PosA(Lower(as_SQL), "union") 
li_Pos = li_PosU
lb_isUnion = FALSE 
lb_isUnionAll = FALSE 
Do Until li_Pos <= 0
	...
	If li_Pos > 0 Then
		...
		if lb_isUnion = TRUE then
			...
			li_PosAll = li_pos
			ls_right = MidA(Trim(as_SQL), li_PosAll,3)
			...
			If lb_isUnionAll = TRUE Then
				as_SQL = ReplaceA(as_SQL, li_PosAll, 3, LI_ALLREPLACE)
			Else
				li_Pos = li_PosAll
			End If
			ls_right = MidA(Trim(as_SQL), li_Pos, 6)
			If Upper(ls_right) = "SELECT" Then
				as_SQL = ReplaceA(as_SQL, li_PosU, 5, LI_UNIONREPLACE)
			End If															
		End If
		li_PosU = PosA(Lower(as_SQL), "union", li_Pos+5) 
		li_Pos = li_PosU
	End If
Loop
