...
For li_cnt = 1 to li_upper
	If LenA(inv_cachelist[li_cnt].s_id) > 0 Then 
...
Choose Case as_method
	Case RETRIEVE, DATAOBJECTDATA, POWEROBJECT, IMPORTFILE
		If IsNull(as_dwobjectname) or LenA(as_dwobjectname)= 0 Then
			Return -1
		End If		
End Choose
...
IF NOT Isvalid(inv_cachelist[li_newentry].ds_obj) THEN
	inv_cachelist[li_newentry].ds_obj = Create n_ds
END IF
//---------------------------- APPEON END ----------------------------
...
IF inv_cachelist[li_newentry].ds_obj.DataObject <> as_dwobjectname THEN
	inv_cachelist[li_newentry].ds_obj.DataObject = as_dwobjectname
END IF
//---------------------------- APPEON END ----------------------------
...
li_upper = UpperBound (inv_cachelist)
For li_cnt = 1 to li_upper
	If IsValid(inv_cachelist[li_cnt]) Then
		If IsValid (inv_cachelist[li_cnt].ds_obj) Then
			Destroy inv_cachelist[li_cnt].ds_obj
		End If
	End If
