...
//$<reason> SetFocus(mle_userinput) is unsupported.
//SetFocus(mle_userinput)

mle_userinput.SetFocus()
...
For li_msgline = 1 to li_msglinecount
	...
	li_startpoint += LenA(ls_textline) + 1

	// Determine the number of occurrences of the Tab Character.
	ll_counttabs = 0
	ll_Pos = PosA(ls_textline, TAB)
	Do While ll_Pos > 0
		ll_counttabs ++
		ll_Pos = PosA(ls_textline, TAB, (ll_Pos + li_tabcharlen))
	Loop
...
this.Control[iCurrent+2]=this.gb_userinput
this.Control[iCurrent+3]=this.mle_userinput
this.Control[iCurrent+4]=this.st_userinput
this.Control[iCurrent+5]=this.cb_1
this.Control[iCurrent+6]=this.cb_2
this.Control[iCurrent+7]=this.cb_3
this.Control[iCurrent+8]=this.cb_print
this.Control[iCurrent+9]=this.cb_userinput
this.Control[iCurrent+10]=this.mle_message
this.Control[iCurrent+11]=this.lv_bmp
end on
...
If li_bmp > 0 Then
	...
	Choose Case li_bmp
		Case 1
			p_1.picturename = 'info.bmp' 
		Case 2
			p_1.picturename = 'stop.bmp' 
		Case 3
			p_1.picturename = 'excl.bmp' 
		Case 4
			p_1.picturename = 'quest.bmp' 
	End Choose
...
ii_buttonclicked = Integer(MidA(This.ClassName(), 4, 1))
