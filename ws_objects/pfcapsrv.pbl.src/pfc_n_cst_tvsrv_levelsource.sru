$PBExportHeader$pfc_n_cst_tvsrv_levelsource.sru
...
//																		1, inv_attrib[li_ArgIndex].ids_obj.RowCount())
			...
			Choose Case LeftA(inv_attrib[li_ArgIndex].ids_obj.Describe(ls_ArgCol + ".coltype"), 7)
				Case "integer", "long"
...
If LenA(adw_control.DataObject) = 0 Then Return -1
...
If LenA(ads_control.DataObject) = 0 Then Return -1
...
//	Check Arguments
If IsNull(as_dwobjectname) or LenA(Trim(as_dwobjectname))= 0 Then Return -1
...
// check sql statement
If IsNull(as_sql) or ( LenA(as_sql) = 0 ) Then Return -1
...
If LenA(ls_sqlerr) > 0 Then Return -1
...
if IsNull (ai_level) or ai_level <= 0 or &
	...
	LenA (as_DWobjectname) = 0 or IsNull (as_DWobjectname) or &
	LenA (as_method) = 0 or IsNull (as_method) then
	return -1
end if
...
CHOOSE CASE as_method
	...
	CASE inv_cache.IMPORTFILE
		...
		If LenA(as_importfile) <> 0 and Not IsNull(as_importfile) Then
			If inv_cache.of_IsRegistered(ls_cacheid) > 0 Then
				// already registered so unregister and re-register
				li_rc = inv_cache.of_unregister(ls_cacheid)
			End If				
	...
	CASE inv_cache.RETRIEVE
		// register a Retrieve
		If LenA(as_dataobject) <> 0 and Not IsNull(as_dataobject) and &
	...
	CASE inv_cache.SQL
		...
		If LenA(as_sql) <> 0 and Not IsNull(as_sql) and &
			IsValid(atr_obj) and Not IsNull(atr_obj) Then
				If inv_cache.of_IsRegistered(ls_cacheid) > 0 Then
					// already registered so unregister and re-register
					li_rc = inv_cache.of_unregister(ls_cacheid)
	...
	CASE inv_cache.DATAOBJECTDATA
		...
		If LenA(as_dataobject) <> 0 and Not IsNull(as_dataobject) Then
			If inv_cache.of_IsRegistered(ls_cacheid) > 0 Then
				// already registered so unregister and re-register
				li_rc = inv_cache.of_unregister(ls_cacheid)
			End If				
...
If Trim(ls_PictureColumn) <> "" Then
	...
	ElseIf LeftA(ads_source.Describe(ls_PictureColumn + ".coltype"), 4) = "char" Then
...
If Trim(ls_StateColumn) <> "" Then
	...
	ElseIf LeftA(ads_source.Describe(ls_StateColumn + ".coltype"), 4) = "char" Then
		// The column contains a bmp name, so add the picture
...
For li_Cnt = 1 To li_Args
	If ls_Parm[li_Cnt] <> "" Then
		li_P1 = PosA(ls_Parm[li_Cnt], ".", 1)
		li_P2 = PosA(ls_Parm[li_Cnt], ".", (li_P1 + 1))
		ls_Type = LeftA(ls_Parm[li_Cnt], (li_P1 - 1))
		ls_Level = MidA(ls_Parm[li_Cnt], (li_P1 + 1), (li_P2 - (li_P1 + 1)))
		ls_ColName = RightA(ls_Parm[li_Cnt], (LenA(ls_Parm[li_Cnt]) - li_P2))
...
// Validate arguments
If IsNull(as_decodekey) or (LenA(trim(as_decodekey)) = 0) Then Return "!"
...
For li_Cnt = 1 To li_Args
	If ls_Parm[li_Cnt] <> "" Then
		li_P1 = PosA(ls_Parm[li_Cnt], ".", 1)
		ls_Oper = LeftA(ls_Parm[li_Cnt], (li_P1 - 1))
		ls_Colname = MidA(ls_Parm[li_Cnt], (li_P1 + 1) )
...
Else
	...
	for li_cnt = 1 to upperbound(ls_column)
		...
		If PosA(ls_operators[li_cnt], "N") > 0 Then
			ls_expression = " NOT (" + ls_expression + ") "
		End If
		If PosA(ls_operators[li_cnt], "A") > 0 Then
			ls_expression = " AND " + ls_expression
		End If
		If PosA(ls_operators[li_cnt], "O") > 0 Then
