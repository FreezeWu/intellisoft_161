$PBExportHeader$pfc_n_cst_inifile.sru
...
do while li_rc >= 0
	...
	if li_rc = -1 then
		return li_rc
	elseif li_rc > 0 then
		ll_first = PosA (ls_line, "[")
		ll_last = PosA (ls_line, "]")
		if ll_first > 0 and ll_last > 0 then
			ls_line = lnv_string.of_LeftTrim (ls_line, true, true)
			if LeftA (ls_line, 1) = "[" then
				ll_pos = PosA (ls_line, "]")
				ls_section = MidA (ls_line, 2, ll_pos - 2)
				li_section++
...
if lb_sectionfound then
	...
	do while li_rc >= 0 and not lb_sectionfound
		...
		if li_rc > 0 then
			...
			if CharA (ls_comment) = ";" then Continue
			
			ll_equal = PosA (ls_line, "=")
			if ll_equal > 0 then
				ls_key = lnv_string.of_Trim (LeftA (ls_line, ll_equal - 1), true, true)
...
do while li_rc >= 0
	...
	if li_rc >= 1 then
		...
		if ll_first >0 and ll_last >0 then
			...
			if LeftA (ls_temp, 1) = "[" then
				ll_pos = PosA (ls_temp, "]")
				ls_section = MidA (ls_temp, 2, ll_pos - 2)
				// Determine if this is the section being searched for.				
				if Lower (ls_section) = Lower (as_section) then
					// The search for section has been found.
					lb_sectionfound = true
	...
	if li_rc >= 0 and not lb_skipline then
		lblb_newfile = lblb_newfile + Blob (ls_line,EncodingAnsi!) //Encoding – Nova 11.16.2010
	end if
loop 
		...
		if li_rc > 0 then
			...
			if ll_first > 0 and ll_last > 0 then
				...
				if lb_sectionfound then
					...
					if Lower (LeftA (ls_temp, li_keylength)) = Lower (as_key) then
						// Yes, the key has been found.						
						ls_temp = lnv_string.of_LeftTrim (MidA (ls_temp, li_keylength + 1), true, true)
						if CharA (ls_temp) = "=" then
							// Start skipping.
							lb_skipline = true
