...
if Trim(inv_Attrib.is_PictureColumn) <> "" then
	...
	elseif LeftA(ads_source.Describe(inv_Attrib.is_PictureColumn + ".coltype"), 4) = "char" then
...
if Trim(inv_Attrib.is_StateColumn) <> "" then
	...
	elseif LeftA(ads_source.Describe(inv_Attrib.is_StateColumn + ".coltype"), 4) = "char" then
		// The column contains a bmp name, so add the picture
...
//--------------------------- APPEON BEGIN ---------------------------
//$<comment> 2005-09-02 By: Liang QingShi
//$<reason> The OverlayPictureIndex property of the ListViewItem object is unsupported.
...
//$<reason> The ItemX property of the ListViewItem object is unsupported.
...
//--------------------------- APPEON BEGIN ---------------------------
//$<comment> 2005-09-01 By: Liang QingShi
//$<reason> The ItemY property of the ListViewItem object is unsupported.
...
// Check arguments
if LenA (as_labelcolumn) = 0 or IsNull (as_labelcolumn) or LenA(as_DWobjectname) = 0 &
or IsNull (as_DWobjectname) or LenA(as_method) = 0 or IsNull (as_method) then return -1
...
choose case LeftA(as_ColType, 7)
	case "integer", "long"
...
//--------------------------- APPEON BEGIN ---------------------------
//$<modify> 2005-09-01 By: Liang QingShi
//$<reason> 

//li_totalitems = ilv_requestor.TotalSelected()
li_totalitems = ilv_requestor.SelectedIndex()
...
If IsNull(alvi_new.Data) = False Then
	alvi_new.Data = String(alvi_new.Data) + ls_key
End If
//---------------------------- APPEON END ----------------------------
...
Choose Case Lower(as_position)
	Case "first"
		...
		//--------------------------- APPEON BEGIN ---------------------------
		//$<comment> 2005-09-01 By: Liang QingShi
		//$<reason> The ItemX,ItemY property of the ListViewItem object is unsupported.
