$PBExportHeader$pfc_n_cst_appmanager.sru
...
//$<reason> The DwMessageTitle property of the Application object is unsupported.
...
if of_IsRegistryAvailable() then
	if LenA (is_userkey) > 0 then
		RegistryGet (is_userkey + "\logon", "userid", ls_userid)		
	end if
else
	if LenA (is_userinifile) > 0 then
