$PBExportHeader$pfc_n_cst_luw.sru
...
For li_i = 1 To li_max
	...
	If LenA (is_typetoprocess) > 0 Then
		If PosA (is_typetoprocess, of_GetType(lpo_tocheck)) = 0 Then
	...
	//$<reason> The ClassDefinition property of the PowerObject object is unsupported.
...
For li_i = 1 to li_max
	...
	If TypeOf (lpo_tocheck) = DataWindow! or TypeOf (lpo_tocheck) = DataStore! Then
		...
		//$<reason> To modify the unsupport features
...
//--------------------------- APPEON BEGIN ---------------------------
//$<add> 2005-09-05 By: Liang QingShi
//$<reason> 
...
For li_i = 1 to li_max
	...
	If lb_defined Then
		//--------------------------- APPEON BEGIN ---------------------------
		//$<modify> 2005-09-05 By: Liang QingShi
		//$<reason> Dynamic calling for overloaded functions is unsupported.
	  ...
	  Choose Case Typeof ( lpo_tocheck )
		...
		Case DataStore!
			...
			Continue
		Case Userobject!
			lbase_update = lpo_tocheck
			li_rc = lbase_update.of_Update (ab_accepttext,  ab_resetflag, lpo_updaterequestor)
			If li_rc < 0 Then Return -1
			Continue
		Case Tab!
			ltab_update = lpo_tocheck
			li_rc = ltab_update.of_Update (ab_accepttext,  ab_resetflag, lpo_updaterequestor)
			If li_rc < 0 Then Return -1
			Continue
		End choose
	   //---------------------------- APPEON END ----------------------------
...
For li_i = 1 to li_max
	...
	If LenA(ls_classnames) > 0 Then ls_classnames += ', '
...
If apo_control.TriggerEvent("pfc_descendant") = 1 Then
	lb_defined = True
Else
	lb_defined = False
End If
...
For li_idx = 1 to li_maxperformed
	If LenA(ls_msgperformed) > 0 Then ls_msgperformed += ', ~r~n'
	ls_msgperformed += atr_performed[li_idx].ClassName()+ &
		'('+atr_performed[li_idx].of_GetName()+')'
Next
If LenA(ls_msgperformed) > 0 Then ls_msgperformed += '~r~n~r~n'

// Loop around the secondar transactions.
For li_idx = 1 to li_maxsecondary
	If LenA(ls_msgsecondary) > 0 Then ls_msgsecondary += ', ~r~n'
...
For li_i = 1 to li_max
	If atr_control[li_i].of_Begin() < 0 Then
		...
		If ib_assist Then
			...
			If LenA(ls_name) = 0 Then ls_name = atr_control[li_i].Classname()
