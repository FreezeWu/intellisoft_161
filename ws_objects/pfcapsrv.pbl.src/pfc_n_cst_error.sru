$PBExportHeader$pfc_n_cst_error.sru
...
// Check arguments.
If IsNull(as_file) or LenA(Trim(as_file))=0 Then
	Return -1
End If
...
//$<add> Evan 06.13.2012
//$<reason> V12.2-Email Logging
lblb_MailContent = Blob(ls_notetext, EncodingANSI!)
lnv_EmailLog.of_SaveSentItem(gs_user_id,ls_notesubject,"",ls_MailTo,"","","",2,2,lblb_MailContent)
//---------- APPEON END ------------
...
If IsNull(is_logfile) or LenA(Trim(is_logfile))=0 Then
	Return -1
End If
...
Else
	...
	Choose Case ii_style
		Case 	0
			...
			If isnull(Trim(inv_errorpass.is_text)) = False And Trim(inv_errorpass.is_text) <> '' Then
				inv_errorpass.ii_buttonclicked = of_MessageBox('pfc_errorsrv_msg', &
					inv_errorpass.is_title, &
					inv_errorpass.is_text, inv_errorpass.ie_icon, &
					inv_errorpass.ie_buttonstyle, inv_errorpass.ii_default)
...
If LenA(inv_errorpass.is_user) > 0 Then
	// Optional user value.
	as_text += 'User: '+ inv_errorpass.is_user + NEWLINE
End If

If LenA(inv_errorpass.is_usertext) >0 Then
	// Optional comments.
...
If LenA(Trim(as_msgid)) >0 Then
	...
	Else
		...
		If ll_row > 0 Then 
			...
			If LenA(Trim(as_overridetitle))=0 Then
...
If IsNull(as_user) or LenA(Trim(as_user))=0 Then
	Return -1
End If
