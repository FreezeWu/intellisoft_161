$PBExportHeader$pfc_n_cst_security.sru
...
Else
	...
	If li_numset > 0 Then 
		...
		If Not am_item.visible Then 
			...
			gnv_app.of_modify_menu_attr( am_item,'toolbaritemvisible', False)
			//---------End Modfiied ------------------------------------------------------
		End If
	End If
End If
...
// check arguments
If LenA(as_object) < 0 or isnull(as_object) then Return -1
...
if LenA(ls_dwname) = 0 then return false
...
If il_numberofrows > 0 Then 
	For li_idx = 1 To il_numberofrows
		...
		li_posi = PosA(ls_column,".")
		Do While li_posi > 0
			ls_column = MidA(ls_column, li_posi + 1)
			li_posi = PosA(ls_column,".")
		Loop  
...
do while LenA(ls_query) > 0  // strip apart the tab seperated string
	li_posi = PosA(ls_query,"~t")
	if li_posi = 0 then // no more tabs so take what's left of the string
		li_posi = LenA(ls_query)
	else
		li_posi = li_posi - 1
	end if
	ls_token = LeftA(ls_query,li_posi) // grab off the token
	ls_query = MidA(ls_query,li_posi+2) // skip past the tab 
	if LenA(ls_token) > 0 then 
		ls_ans =ids_info.describe(ls_token+".type ")
		ls_ans = lower(ls_ans)
		if PosA(ls_ans,"column") > 0 then // is it a column?
			ls_name = ls_dw_name + "."+ls_token
			if not isnull(ls_name) and LenA(trim(ls_name)) > 0 then 
...
if LenA(aw_window.menuname) > 0 then
	li_menu_idx = 1
...
if ii_numgroups = 0  then 
	...
	if LenA(as_defaultgroup) > 0 then
...
//$<reason> The ClassDefinition object is unsupported.
...
// check for menu items
If LenA(aw_obj.menuname) > 0 Then
...
//$<reason> The RichTextEdit control is unsupported, in cluding its functions, events and methods.
//richtextedit	l_rte
...
//$<reason> The Graph control is unsupported.
//$ graph				l_graph
