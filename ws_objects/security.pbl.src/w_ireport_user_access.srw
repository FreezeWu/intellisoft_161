$PBExportHeader$w_ireport_user_access.srw
forward
global type w_ireport_user_access from window
end type
type cbx_update from checkbox within w_ireport_user_access
end type
type cb_2 from commandbutton within w_ireport_user_access
end type
type cbx_2 from checkbox within w_ireport_user_access
end type
type cbx_1 from checkbox within w_ireport_user_access
end type
type st_1 from statictext within w_ireport_user_access
end type
type dw_roles from datawindow within w_ireport_user_access
end type
type cbx_access from checkbox within w_ireport_user_access
end type
type dw_users from datawindow within w_ireport_user_access
end type
type cb_cancel from commandbutton within w_ireport_user_access
end type
type cb_save from commandbutton within w_ireport_user_access
end type
type cb_1 from commandbutton within w_ireport_user_access
end type
type dw_access from datawindow within w_ireport_user_access
end type
type dw_report from datawindow within w_ireport_user_access
end type
type gb_1 from groupbox within w_ireport_user_access
end type
type gb_2 from groupbox within w_ireport_user_access
end type
end forward
...
string title = "IntelliReport User Security"
...
global w_ireport_user_access w_ireport_user_access
...
on w_ireport_user_access.create
...
on w_ireport_user_access.destroy
...
Choose case is_type
	case "I"
		dw_report.dataobject = "d_ireport_list"
	case "E" //export - not coded
		//dw_report.dataobject = ""
	case "S"  //standard reports - not coded
		//dw_report.dataobject = ""
end choose
...
choose case il_runtype
	case 1,2		
		for ll_u = 1 to dw_users.rowcount()
			...
			for ll_r = 1 to dw_report.rowcount()
				ll_report = dw_report.getitemnumber(ll_r,"view_id")
				ls_name = dw_report.getitemstring(ll_r,"view_name")
				dw_report.selectrow(0,false)
	...
	case 3
		for ll_r = 1 to dw_report.rowcount()
			...
			dw_report.selectrow(ll_r,true)
			dw_report.scrolltorow(ll_r)
			dw_access.setfilter("report_id = " + string(ll_report))
			dw_access.filter()
			ll_access = dw_access.rowcount()
			for ll_u = 1 to dw_users.rowcount()
				ls_user = dw_users.getitemstring(ll_u,"user_id")
...
integer taborder = 20
string title = "none"
string dataobject = "d_user_report_access"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
integer taborder = 10
string title = "none"
string dataobject = "d_ireport_list_security"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
ll_report = this.getitemnumber(r,"view_id")
