$PBExportHeader$w_qa_peer_profile_screen.srw
$PBExportComments$The Reappointment Measurements search window
forward
global type w_qa_peer_profile_screen from w_sheet
end type
type cb_1 from commandbutton within w_qa_peer_profile_screen
end type
type tab_qa from pfc_cst_qa_peer_profile within w_qa_peer_profile_screen
end type
type tab_qa from pfc_cst_qa_peer_profile within w_qa_peer_profile_screen
end type
end forward
...
global w_qa_peer_profile_screen w_qa_peer_profile_screen
...
public subroutine of_retrieve_measuremts (long al_facility_id, long al_prac)
end prototypes
...
IF tab_qa.SelectedTab = 2 THEN
	//this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_action_items.Event pfc_cst_letter( )
...
// ------------------------------------------------------------------------------------------------------------------
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////Start Code Change ----08.01.2010 #V10 maha - rewrote
integer t

t = this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.selectedtab
//messagebox("window","of_new")
choose case t
	case 1 //data
			this.tab_qa.tabpage_quality_profile.uo_qa.of_add_new( 1)
	case 2,3,5
			this.tab_qa.tabpage_quality_profile.uo_qa.of_add_new( t)
	case else
end choose
...
public subroutine of_delete ();//Start Code Change ----10.05.2010 #V10 maha - rewrote 
datawindow ldw_active
integer li_tab

if tab_qa.selectedtab = 2 then
	li_tab = tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.selectedtab
	tab_qa.tabpage_quality_profile.uo_qa.of_delete( li_tab )
...
if isvalid(w_prac_data_1) or isvalid(w_prac_data_intelliapp ) or isvalid(w_prac_data_2) or isvalid(w_prac_data_intelliapp_2 )  then  //Start Code Change ----06.18.2013 #V14 maha - added _2 windows
		...
		if lw_prac_window.il_prac_id <> 0 and not isnull(lw_prac_window.il_prac_id) then 
			if messagebox(title,'Do you want to retrieve the current practitioner in the practitioner window?',Question!,yesno!,1) = 1 then
				...
				setpointer(hourglass!)
					...
					//---------Begin Modified by (Appeon)Stephen 07.01.2014 for Bug 4096: QA Profiles screen does not open the selected practitioner--------
					//this.tab_qa.tabpage_search.uo_search.ddlb_search_type.SelectItem ('I.D.',1 )
					this.tab_qa.tabpage_search.uo_search.ddlb_search_type.SelectItem ('Prac ID',1 )			
					//---------End Modfiied ------------------------------------------------------
...
integer li_col, li_cnt, li_i
string   ls_colnam

//------------------- APPEON BEGIN -------------------
//$<add> Stephen 04.27.2011
//$<reason> Invalid DataWindow row/column 
if tab_qa.tabpage_search.uo_search.dw_search.getrow() < 1 then return  
li_col = long(tab_qa.tabpage_search.uo_search.dw_search.object.datawindow.column.count)
li_i = 0
for li_cnt = 1 to li_col
	ls_colnam =  tab_qa.tabpage_search.uo_search.dw_search.describe("#"+string(li_cnt)+".name")
	if ls_colnam = "parent_facility_id" or ls_colnam = "prac_id" then li_i = li_i + 1
	if li_i = 2 then exit
next

if li_i < 2 then return
//------------------- APPEON END ---------------------
...
this.tab_qa.of_retrieve_measurements(al_facility_id,al_prac)

//this.tab_qa.tabpage_quality_profile.uo_1.tab_measurements.selecttab( 1)
end subroutine

on w_qa_peer_profile_screen.create
int iCurrent
call super::create
...
on w_qa_peer_profile_screen.destroy
call super::destroy
destroy(this.cb_1)
destroy(this.tab_qa)
end on
...
//<$>reason:FIxed BugL111401
FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_data.control)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_data.CONTrol[LL_I]  , ls_scale )
next

FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_browse.control)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_browse.CONTrol[LL_I]  , ls_scale )
next

FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_detail.control)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_detail.CONTrol[LL_I]  , ls_scale )
next

FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_report.control)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_report.CONTrol[LL_I]  , ls_scale )
next

FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_action_items.control)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_action_items.CONTrol[LL_I]  , ls_scale )
next
//-------------appeon End------------------------
...
FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_oppe.uo_oppe.tab_1.tabpage_actions.CONTrol)//added by long.zhang 11.20.2013 BugL111401 
   this.inv_resize.of_Register (this.tab_qa.tabpage_oppe.uo_oppe.tab_1.tabpage_actions.CONTrol[LL_I]  , ls_scale )
next


//tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_data For resize add by Michael 09.01.2011
FOR LL_I =  1 TO UPPERBOUND(this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_data.CONTrol)
   this.inv_resize.of_Register (this.tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.tabpage_data.CONTrol[LL_I]  , ls_scale )
next
...
if AppeonGetClientType() = "WEB" then w_mdi.SetRedraw(false) //Added by Evan on 04.27.2012
...
IF oldindex = 1 THEN
	...
	IF tab_qa.tabpage_search.uo_search.dw_search.RowCount() > 0 THEN
		...
		//iw_parent.SetRedraw( False) //Commented by Evan on 04.27.2012
	...
	ELSE
		tab_qa.selecttab( 1)
		if AppeonGetClientType() = "WEB" then w_mdi.SetRedraw(true) //Added by Evan on 04.27.2012
...
CHOOSE CASE newindex
	...
	CASE 2
		...
		tab_qa.tabpage_quality_profile.uo_qa.tab_measurments.selecttab(1) //(Appeon)Stephen 01.24.2014 - Bug 3855 : Quality Module issues
	...
	CASE 6 //Added by  Nova 12.05.2009 --v10.1 Profiles Build out
		...
		tab_qa.tabpage_profile.uo_profile.il_appt_rec = il_appt_rec
	CASE ELSE
		IF w_mdi.MenuName	 <> 'm_pfe_cst_mdi_menu' THEN
...
//iw_parent.SetRedraw( True) //Commented by Evan on 04.27.2012
if AppeonGetClientType() = "WEB" then w_mdi.SetRedraw(true) //Added by Evan on 04.27.2012
