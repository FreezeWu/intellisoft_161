...
type st_pnotes from statictext within tabpage_notes
end type
...
type gb_filter_1 from groupbox within pfc_ia_cst_nv_net_dev_new
end type
type cb_ma from commandbutton within pfc_ia_cst_nv_net_dev_new
end type
...
type cbx_compl from checkbox within pfc_ia_cst_nv_net_dev_new
end type
type cb_change from commandbutton within pfc_ia_cst_nv_net_dev_new
end type
...
type cb_3 from commandbutton within pfc_ia_cst_nv_net_dev_new
end type
end forward
...
Boolean ib_new_add = false //(Appeon)Stephen 07.17.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
...
//Boolean ib_setifi = false //add harry 05.31.2016
//DateTime idt_due_date //add harry 05.31.2016
u_dw idw_curr  //maha 07.12.2016
...
public function integer of_app_view_load (long al_row, datawindow adw_net_dev, string as_type, string as_file)
public function integer of_hide_fields_no_esign ()
public function integer of_check_missing_trackids (long al_prac)
public function integer of_retrieve_facilities ()
public function integer of_reset_trackids (long al_prac)
public function integer of_set_active_status (integer ai_active_status)
public function integer of_delete ()
public subroutine of_after_receive_signed_doc (datawindow adw, long al_row, long al_rec_id)
public subroutine of_zoom ()
public subroutine of_reset_tracking ()
public function integer of_edit_workflow_actions ()
end prototypes
...
event pfc_cst_regen_trackids();//Start Code Change ----06.29.2016 #V152 maha
of_reset_trackids( il_prac_id )
end event
...
//---------Begin Added by (Appeon)Stephen 07.12.2016 for Bug id 5226 - Initiate Re-Appointment for Payors--------
dw_orgs_facilities.SetRedraw( False)
dw_orgs_facilities.Retrieve( il_prac_id, gs_user_id )
dw_orgs_facilities.SetRedraw( True)
//---------End Added ------------------------------------------------------
...
//NOT used if single level (facility) only

Integer li_rc
integer li_res //maha 08.03.2015
String ls_sql, ls_rtn
string ls_id, ls_action_status  //add by Stephen 06.14.2012 
Boolean lb_reset_tracking = False //Added by Appeon long.zhang 09.08.2017
...
IF li_rc = 0 THEN
	...
	If il_track_id > 0  Then lb_reset_tracking = True //Reset Tracking when no action item records, Added by Appeon long.zhang 09.08.2017 ((Bug 5794) Appl Tracking record not deleted when Net Dev Action Item record is deleted)
...
if tab_1.tabpage_ai.dw_net_dev.DataObject = "d_net_dev_action_items_ai_sk" and (isvalid(w_action_items) or isvalid(w_action_items_ia)) then //(Appeon)Stephen 07.27.2017 - Applications not showing in Action Items Case#71665 Bug_id#5733
else
	datawindowchild dwchild
	tab_1.tabpage_ai.dw_net_dev.getchild( "gp_id", dwchild)
	dwchild.settransobject(sqlca)
...
IF NOT ib_cbx_compl_changed THEN //if not called by cbx_compl - alfee 10.20.2010
	...
	IF lb_from_prac_folder THEN 
		...
		dwchild.retrieve( il_prac_id)
		dwchild.insertrow(1)
		//Start Code Change ----12.15.2016 #V153 maha - reversed logic
//		if ib_show_ids_addr then
//			tab_1.tabpage_ids.dw_ids.modify("address_id.visible = True") 
		if not ib_show_ids_addr then
			tab_1.tabpage_ids.dw_ids.modify("address_id.visible = False")
...
//of_report_selected_ver9( "NONE")  //(Appeon)Stephen 07.19.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
...
//Use action_status_wf for workflow, Comment by Appeon long.zhang 06.12.2017 (V15.3 Bug #5674 - Workflow: When Changing the status of one action type, 1 or more actions statuses change to codes)
////---------Begin Added by (Appeon)Stephen 01.05.2017 for Case # 00067900: Workflow not working properly  --------
//tab_1.tabpage_ai.dw_net_dev.GetChild( "action_status", dwchild )
//dwchild.SetTransObject( SQLCA )
//if tab_1.tabpage_ai.dw_net_dev.dataobject = "d_net_dev_action_items_ai_sk" or tab_1.tabpage_ai.dw_net_dev.dataobject = "d_net_dev_action_items" then
//	dwchild.Retrieve("Action Status")
//else
//	dwchild.Retrieve()
//end if
////(Appeon)Stephen 01.10.2017 - Case # 00067900: Workflow not working properly
//dwchild.setfilter("")
//dwchild.filter()
////---------End Added ------------------------------------------------------
...
tab_1.tabpage_ai.dw_net_dev.SetRedraw(true)
tab_1.tabpage_ai.dw_net_dev.setfocus() //(Appeon)Stephen 01.05.2017 - Case # 00067900: Workflow not working properly


//debugbreak()
If lb_reset_tracking Then //Reset dw_tracking when no action items, Added by Appeon long.zhang 09.08.2017 ((Bug 5794) Appl Tracking record not deleted when Net Dev Action Item record is deleted)
	of_reset_tracking()
Else
...
if idw_tracking.rowcount() = 0 then
	if il_prac_id > 0 and  il_app_id > 0 and ii_facility_id > 0 then
		...
		*/
		...
		idw_tracking.setitem(1,"track_id",ll_id)
		inv_data_entry.of_field_audit( idw_tracking ) //(Appeon)Stephen 06.20.2016 - V15.2-Application Tracking full Auditing
		idw_tracking.update()
		idw_tracking.retrieve(il_prac_id, il_app_id, ii_facility_id, il_loc_id )
		//need to calculate
	end if
	...
	If lb_from_prac_folder Then //Added by Appeon long.zhang 08.08.2017 (Application Tracking not generating dates automatically Case#72116 Bug_id# 5749)
		of_set_par_status() 
	End If
end if
...
//if ll_map_type = 1 then  //Start Code Change ----10.15.2013 #V14 maha - changed from 2//Start Code Change ----07.11.2016 #V152 maha - removed restriction
...
if li_skip = 0 then
	...
	//set tracking ids, Added by Appeon long.zhang 11.16.2016 (V15.2 Bug # 5410 - Email Payor is not generating the next app tracking step)
	if tab_1.tabpage_ai.dw_tracking.rowcount() > 0 then
...
for r = 1 to lds_data.rowcount()
	...
	li_hide = lds_data.getitemnumber(r,"hide_field") //Bug #5327 - alfee 09.18.2016
...
integer r
long ll_n //maha 06.29.2016

if il_track_id = 0 then return 1
//Start Code Change ----06.29.2016 #V152 maha - don't set if not at the application level
if ii_facility_id = 0 then return 1
if il_app_id = 0 then return 1
setnull(ll_n)
//End Code Change ----06.29.2016 


for r = 1 to tab_1.tabpage_ai.dw_net_dev.rowcount( )
	if  isnull(tab_1.tabpage_ai.dw_net_dev.getitemnumber(r, "track_id")) then
		//Start Code Change ----06.29.2016 #V152 maha - trap for record manually added
		if tab_1.tabpage_ai.dw_net_dev.getitemnumber(r, "facility_id") = ii_facility_id and tab_1.tabpage_ai.dw_net_dev.getitemnumber(r, "app_id") = il_app_id and  tab_1.tabpage_ai.dw_net_dev.getitemnumber(r, "gp_id") = il_loc_id then
			 tab_1.tabpage_ai.dw_net_dev.setitem(r, "track_id", il_track_id)
		else
			 tab_1.tabpage_ai.dw_net_dev.setitem(r, "track_id", ll_n)
		end if
		//End Code Change ----06.29.2016
	end if
next
...
public function integer of_app_view_load (long al_row, datawindow adw_net_dev, string as_type, string as_file);//Start Code Change ----05.18.2016 #V152 maha - created function copied from of_imp_appfile
...
if ll_map_type = 1 then 
	...
	lstr_img.s_module = as_file  //Start Code Change ----05.18.2016 #V152 maha
	
	
	openwithparm(w_app_imgfile_pop, lstr_img)  //Start Code Change ----06.06.2016 #V152 maha
...
public function integer of_hide_fields_no_esign ();//====================================================================
//$<Function>: of_hide_fields_no_esign
//$<Arguments>:
// 	N/M
//$<Return>:  integer
//$<Description>: 
//$<Author>: (Appeon) Harry 06.21.2016 (Application Electronic Signature - BugL061703)
//--------------------------------------------------------------------
//$<Modify History>:
//====================================================================
...
dw_actions.modify("b_esign_r.visible = false")
dw_actions.modify("b_esign_s.visible = false")
dw_actions.modify("b_esign_ns.visible = false")
dw_actions.modify("b_esign_nr.visible = false")  //Add by Harry 07.15.2016 for BugA071502
if of_get_app_setting("set_40","I") = 4 then
	xpos = long(dw_actions.describe("b_esign_ns.x"))
	dw_actions.modify("b_enpreview.x = " + string(xpos)) 
	dw_actions.modify("b_dispreview.x = " + string(xpos)) 
	dw_actions.modify("b_display.x = " + string(xpos)) 
	
	ll_width = long(dw_actions.describe("notes.width")) + long(dw_actions.describe("b_enpreview.width")) 
	dw_actions.modify("notes.width = " + string(ll_width)) 
	if gi_intelliapp_type = 0 or gi_intelliapp_type = 3 then of_hide_fields()
else
	xpos = long(dw_actions.describe("b_enpreview.x")) + long(dw_actions.describe("b_enpreview.width")) + 25
	dw_actions.modify("bcontact.x = " + string(xpos)) 
	xpos = xpos + long((dw_actions.describe("bcontact.width"))) + 25
	dw_actions.modify("b_notes.x = " + string(xpos))
	
	xpos = xpos + long((dw_actions.describe("b_notes.width"))) + 25
	
	dw_actions.modify("notes.x = " + string(xpos))
	dw_actions.modify("notes_t.x = " + string(xpos))
	xpos = xpos + long((dw_actions.describe("notes.width"))) + 15
	
	dw_actions.modify("app_id.x = " + string(xpos))
	dw_actions.modify("app_id_t.x = " + string(xpos))
	xpos = xpos + long((dw_actions.describe("app_id.width"))) + 15
	
	if gi_intelliapp_type = 0 or gi_intelliapp_type = 3 then
		of_hide_fields()
	else
		dw_actions.modify("gp_id.x = " + string(xpos))
		dw_actions.modify("gp_id_t.x = " + string(xpos))
		xpos = xpos + long((dw_actions.describe("gp_id.width"))) + 15
...
public function integer of_check_missing_trackids (long al_prac);//Start Code Change ----06.28.2016 #V152 maha - added to set track ids in existing records where null called from of_retrieve_facilities
 //Start Code Change ----03.23.2017 #V153 maha - moved to application function
of_check_missing_trackids(al_prac)
...
public function integer of_retrieve_facilities ();//Start Code Change ----06.28.2016 #V152 maha - copied from ancester and modified
dw_orgs_facilities.Reset( )
dw_orgs_facilities.setredraw( false)

of_check_missing_trackids( il_prac_id )   //Start Code Change ----06.28.2016 #V152 maha
dw_orgs_facilities.Retrieve( il_prac_id ,gs_user_id)
...
dw_orgs_facilities.setredraw( true)  
//messagebox("", "of_retrieve_facilities")


Return 0
end function

public function integer of_reset_trackids (long al_prac);//Start Code Change ----06.29.2016 #V152 maha - added
update net_dev_action_items set track_id = null where prac_id = :al_prac and active_status = 1;
commit using sqlca;
...
public function integer of_set_active_status (integer ai_active_status);//Start Code Change ----07.12.2016 #V152 maha - replaced original function
integer res
integer li_choice
long li_row
long ll_track  //maha 10.17.2016
string ls_active
u_dw dw_net_dev


if ai_active_status = 1 then
	ls_active = "Active"
else
	ai_active_status = 0 //Start Code Change ----09.13.2017 #V154 maha
	ls_active = "Inactive"
end if

openwithparm(w_appl_make_active_choice, ls_active)

li_choice = message.doubleparm

if li_choice = 0 then return 0  // cancel

dw_net_dev = tab_1.tabpage_ai.dw_net_dev 

if li_choice = 1 then  //single action
	res = messagebox("Make " + ls_active,"Are you sure you want to make the selected Action " + ls_active + " ?", question!,yesno!, 2)
	if res = 2 then return 0
	li_row = dw_net_dev.getrow()
	dw_net_dev.setitem(li_row, "active_status", ai_active_status)

else //tracking and all actions
	res = messagebox("Make " + ls_active,"Are you sure you want to make the Tracking record and all connected Actions " + ls_active + " ?", question!,yesno!, 2)
	if res = 2 then return 0
	debugbreak()
	ll_track = dw_net_dev.getitemnumber(dw_net_dev.getrow(), "track_id")
	for li_row = 1 to dw_net_dev.rowcount()
		if dw_net_dev.getitemnumber(li_row, "track_id") = ll_track then //Start Code Change ----10.17.2016 #V152 maha - trap for higher level node
			dw_net_dev.setitem(li_row, "active_status", ai_active_status)
		end if
	next
	
	//Start Code Change ----10.17.2016 #V152 maha - added trapping if treeview at higher level node
	if tab_1.tabpage_ai.dw_tracking.rowcount() > 0 then
		tab_1.tabpage_ai.dw_tracking.setitem(1, "active_status", ai_active_status)
	else
		update pd_appl_tracking set active_status = :ai_active_status where track_id = :ll_track;
		commit using sqlca;
	end if
	//End Code Change ----10.17.2016
end if
...
public function integer of_delete ();//Start Code Change ----07.12.2016 #V152 maha - added for delete protection of tracking records

choose case tab_1.selectedtab
	case 1
		if idw_curr = tab_1.tabpage_ai.dw_net_dev then
			idw_curr.triggerevent("pfc_deleterow")
		elseif idw_curr = tab_1.tabpage_ai.dw_appoint_stat then
			idw_curr.triggerevent("pfc_deleterow")
		end if
	case 2
		tab_1.tabpage_ids.dw_ids.triggerevent("pfc_deleterow")
	case 3
		tab_1.tabpage_notes.dw_notes.triggerevent("pfc_deleterow")
	case else
		//skip
end choose
...
public subroutine of_after_receive_signed_doc (datawindow adw, long al_row, long al_rec_id);//////////////////////////////////////////////////////////////////////
// Function: of_after_receive_signed_doc()
// Arguments:
...
ldt_currdate = DateTime(Today(),Now())
UPDATE net_dev_action_items SET sig_status = 3, sig_in_date = :ldt_currdate WHERE rec_id = :al_rec_id;

adw.SetItem(al_row,"sig_status", 3)
adw.SetItemstatus(al_row, "sig_status", Primary!, NotModified!)
messagebox("Receive Document", "The signed document is received!")

//Add by Harry 07.18.2016 
//complete the action after receiving/uploading the signed document, and create the next step.
select lookup_code into :ll_complete_action_status from code_lookup where lookup_name = 'Action Status' and code = 'Complete';
ll_action_status = adw.GetItemNumber(al_row, 'action_status')
if ll_action_status <> ll_complete_action_status and ll_complete_action_status > 0 then
	adw.SetItem(al_row,"action_status", ll_complete_action_status)
	adw.SetItem(al_row,"action_date", ldt_currdate)
	//This.event itemchanged( row, this.object.action_status, string(ll_complete_action_status))
...
dw_orgs_facilities.Modify("DataWindow.Zoom=" + string(gi_zoom))  
tab_1.tabpage_ai.dw_appoint_stat.Modify("DataWindow.Zoom=" + string(gi_zoom))
tab_1.tabpage_ai.dw_net_dev.Modify("DataWindow.Zoom=" + string(gi_zoom))
tab_1.tabpage_ai.dw_tracking.Modify("DataWindow.Zoom=" + string(gi_zoom))
tab_1.tabpage_ids.dw_ids.Modify("DataWindow.Zoom=" + string(gi_zoom))
tab_1.tabpage_report.dw_report_data.Modify("DataWindow.Zoom=" + string(gi_zoom))
tab_1.tabpage_notes.dw_notes.Modify("DataWindow.Zoom=" + string(gi_zoom))

end subroutine

public subroutine of_reset_tracking ();//====================================================================
//$<Function>: of_reset_tracking
//$<Arguments>:
//$<Return>:  (None)
//$<Description>: Reset dw_tracking,  called by of_retrieve_detail() .
//$<Author>: (Appeon) long.zhang 09.08.2017 ((Bug 5794) Appl Tracking record not deleted when Net Dev Action Item record is deleted)
//--------------------------------------------------------------------
//$<Modify History>:
//====================================================================
datawindow ldw_tracking

ldw_tracking = tab_1.tabpage_ai.dw_tracking
...
public function integer of_edit_workflow_actions ();//Start Code Change ----12.04.2017 #V16 maha - added
integer li_row
integer res
integer f
integer rc
...
gs_pass_ids lst_ids
debugbreak()
li_row = tab_1.tabpage_ai.dw_net_dev.getrow()
ll_prac = tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "prac_id")
ll_facility = tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "facility_id")
ll_gpid =  tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "gp_id")
ll_app =  tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "app_id")
ll_wf = tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "net_dev_action_items_wf_id")
ll_track =  tab_1.tabpage_ai.dw_net_dev.getitemnumber(li_row, "track_id")

if ll_wf < 1 or isnull(ll_wf) then
	messagebox("Workflow Action Edit","The selected Action is not part of a WorkFlow.")
	return 0
end if

lst_ids.facility_id = ll_facility
lst_ids.prac_id = ll_prac
lst_ids.l_app_id = ll_app
lst_ids.l_ids[1] = ll_gpid
lst_ids.l_ids[2] = ll_wf
lst_ids.l_ids[3] = ll_track

res = openwithparm(w_appl_actions_wf_edit, lst_ids)
integer ss
if res = 1 then
//	of_refresh_data( )
	rc = dw_orgs_facilities.retrieve(ll_prac, gs_user_id)
	f = dw_orgs_facilities.find("pd_appl_tracking_track_id = " + string(ll_track)  , 1, rc)
//	ss = dw_orgs_facilities.selectrow(dw_orgs_facilities.getrow(), false)
//	messagebox(string(dw_orgs_facilities.getrow()), ss)
	if f > 0 then		
		dw_orgs_facilities.expand(f, 1)
		dw_orgs_facilities.expand(f, 2)
		dw_orgs_facilities.setrow(f)
		dw_orgs_facilities.scrolltorow(f)
		dw_orgs_facilities.SelectTreeNode(f,3,true)

		il_app_id = ll_app
		il_loc_id = ll_gpid
		ii_facility_id = ll_facility
		il_track_id = ll_track
		of_retrieve_detail( )
		tab_1.tabpage_ai.dw_appoint_stat.Retrieve( il_prac_id, ii_facility_id )
	end if
...
idw_curr = create u_dw  //Start Code Change ----07.12.2016 #V152 maha
...
if of_get_app_setting("set_40","I") = 4 then
 ...
 //Modified by (Appeon) Harry 06.21.2016 (Application Electronic Signature - BugL061703)
 /*
//else
...
if (gnv_data.of_getitem( 'icred_settings', 'esign', False) ='0' or gs_esigntype = 'echosign') and &
	(tab_1.tabpage_ai.dw_net_dev.dataobject = "d_net_dev_action_items_no_group" or &
		tab_1.tabpage_ai.dw_net_dev.dataobject = "d_net_dev_action_items_sk"	) Then //only work for Provider Folder-Application tab,  Modified by Appeon long.zhang 02.14.2017
	of_hide_fields_no_esign()		
else
...
event type integer ue_sertify ( )
event type integer ue_docusign ( )
integer x = 0
integer y = 248
integer width = 3387
integer height = 816
...
event type integer dw_net_dev::ue_sertify();//////////////////////////////////////////////////////////////////////
// Event: ue_sertify()
// Arguments:
//--------------------------------------------------------------------
// Return:  (none)
//--------------------------------------------------------------------
// Author:	 Harry 05.18.2016
//--------------------------------------------------------------------
...
Datetime ldt_now
str_app_email lstr_app_email
n_ds lds_sertifi_update
...
lstr_app_email.faci_id_list[1] = this.GetItemNumber(ll_row,  'facility_id')
lstr_app_email.action_type =  String(this.GetItemNumber(ll_row,  'action_type'))
...
If lpo_pass.classname( ) <> 'str_app_email' Then Return 0
...
ll_rowcount = lds_sertifi_update.Retrieve(lstr_app_email.rec_id_list[])
If ll_rowcount <= 0 Then Return 0

ldt_now = datetime(today() , Now())

Long i,ll_find
For i = 1 To UpperBound(lstr_app_email.rec_id_list[])
	If Len(lstr_app_email.as_sertifi_docid[i]) > 0 Then
		ll_find = lds_sertifi_update.Find('rec_id = ' + String(lstr_app_email.rec_id_list[i]), 1, lds_sertifi_update.RowCount() )
		If ll_find > 0 Then
			lds_sertifi_update.SetItem(ll_find, 'sig_status',1)
			lds_sertifi_update.SetItem(ll_find, 'sig_out_date',ldt_now)
			lds_sertifi_update.SetItem(ll_find, 'sig_doc_link', lstr_app_email.as_sertifi_link)  
			lds_sertifi_update.SetItem(ll_find, 'es_apicode', gnv_sertifi.is_apicode)
			lds_sertifi_update.SetItem(ll_find, 'es_fileid', lstr_app_email.as_sertifi_fileid )
			lds_sertifi_update.SetItem(ll_find, 'es_docid', lstr_app_email.as_sertifi_docid[i] )
			lds_sertifi_update.SetItem(ll_find, 'es_sender', lstr_app_email.as_sender )
			lds_sertifi_update.SetItem(ll_find, 'es_sender_email', lstr_app_email.as_mail_from )
			lds_sertifi_update.SetItem(ll_find, 'esign_type',gs_esigntype)  //(Appeon) Add by Harry 07.13.2016
		Else
			gnv_sertifi.of_removesignaturerequest(lstr_sertifi_doc)
			Messagebox('Error','Failed to send the document ' + lstr_app_email.fullfilename_list[i] + '.' )
...
//	//Insert Action Item
//	If lstr_app_email.ai_create_ai = 1 Then
//		ll_find = this.Find('rec_id = ' + String(lstr_app_email.rec_id_list[i]), 1, this.RowCount() )
//		If ll_find > 0 Then
//			ib_setifi = true
//			idt_due_date = lstr_app_email.adt_duedate
//			of_email_payor_complete(this, ll_find)
//			ib_setifi = false
//		End If
//	End If
Next
...
If lds_sertifi_update.Update() <> 1 Then
	...
	Return -1
Else
	 this.SetItem(ll_row,  'sig_status', 1)
	 This.SetItemstatus(ll_row, "sig_status", Primary!, NotModified!)  //(Appeon) Modified by Harry 06.27.2016 for BugL062301
End If
...
event type integer dw_net_dev::ue_docusign();//////////////////////////////////////////////////////////////////////
// Event: ue_docusign()
// Arguments:
//--------------------------------------------------------------------
// Return:  (none)
//--------------------------------------------------------------------
// Author:	 Harry 2016-07-13.
//--------------------------------------------------------------------
// Description: Application Electronic Signature - Docusign
//--------------------------------------------------------------------
//////////////////////////////////////////////////////////////////////
...
OpenWithParm(w_app_docusign_request,lstr_app_email)
...
For i = 1 To UpperBound(lstr_app_email.rec_id_list[])
	If Len(lstr_app_email.as_sertifi_docid[i]) > 0 Then
		...
		If ll_find > 0 Then
			...
			lds_sertifi_update.SetItem(ll_find, 'es_license_user_id',is_license_user_id) 
		Else
			gnv_docusign.of_removesignaturerequest(lstr_sertifi_doc)
			Messagebox('Error','Failed to check out the document ' + lstr_app_email.fullfilename_list[i] + '.' )
			If isvalid(lds_sertifi_update) Then Destroy lds_sertifi_update
			Return -1
		End If
	End If
...
Else
	 this.SetItem(ll_row,  'sig_status', 1)
	 This.SetItemstatus(ll_row, "sig_status", Primary!, NotModified!)  
End If
...
event dw_net_dev::clicked;call super::clicked;integer res, li_count, li_ret
long r, ll_rowid, ll_row 
long ll_rec_id,ll_prac_id,ll_app_id
long ll_facid, ll_Pos
long ll_map_type, ll_action_type, ll_action_status 
long ll_incomplete, ll_type_code
Long ll_sig_status, ll_complete_action_status
string s, ls_action_type, ls_action_status, ls_file_ext
string ls_note_temp = ''
String	ls_PicName,ls_PicDir, ls_params, ls_null 
datetime ldt_currdate
str_parm lstr_img
gs_pass_ids lstr_ids
...
ELSEIF dwo.name = "b_enpreview" then  //display image
	...
	//Start Code Change ----05.18.2016 #V152 maha - modified to allow editing of app
	ll_app_id = This.GetItemNumber(row,"app_id")
debugbreak()	
	select mapping_type into :ll_map_type from app_hdr where app_id = :ll_app_id;		
	//if ll_map_type = 1  then	   //Start Code Change ----07.11.2016 #V152 maha - removed type restriction
		of_imp_appfile(row, this, 'view')
		if message.doubleparm = 1 then
			this.setitem(row, "data_len", 100)
			this.setitemstatus(row, "data_len", primary!, notmodified!)
		elseif message.doubleparm = 5 then

		//Start Code Change ----05.18.2016 #V152 maha - moved inside the else
			ll_rec_id = This.GetItemNumber(row,"rec_id")
			ll_prac_id = This.GetItemNumber(row,"prac_id")
			ll_app_id = This.GetItemNumber(row,"app_id")
			ll_facid = This.GetItemNumber( row, "facility_id")  //Start Code Change ----06.17.2016 #V152 maha
		//End Code Change ----05.18.2016
			...
			if FileExists(ls_PicName) then
				if ls_file_ext = ".tif" then
					if w_mdi.of_security_access(7822) = 0 then  // edit saved app
						IF Isvalid(w_image_display) THEN Close (w_image_display) 
						OpenWithParm(w_image_display, ls_PicName)
					else
						ls_params = ls_PicName + "||" + "" + "||" + "" + "||" + "" +  "||" + string(ll_prac_id) + "||" + string(ll_rec_id) + "||" + string(ll_app_id)  + "||" + string(ll_facid) 
					...
					setnull(ls_null)
					ShellExecuteA ( Handle( This ), "open",  ls_PicName, ls_Null, ls_Null, 1)	
				end if
			else
				MessageBox(gnv_app.iapp_object.DisplayName,ls_PicName + " does not exist!")
			end if
		elseif message.doubleparm = 7 then
			this.setitem(row, "data_len", 0)
			this.setitemstatus(row, "data_len", primary!, notmodified!)
			messagebox("Clear Document","Document Deleted")
		//end if
	end if
	//End Code Change ----06.06.2016 #V152 maha
...
ELSEIF dwo.name = "b_dispreview" then //(Appeon)Stephen 08.13.2013 - V141 Email/Fax Applications to Payors	
	...
	ll_app_id = This.GetItemNumber(row,"app_id")
	select mapping_type into :ll_map_type from app_hdr where app_id = :ll_app_id;		
	//	if ll_map_type = 1  then	 //Start Code Change ----10.15.2013 #V14 maha - changed from 2  //Start Code Change ----07.11.2016 #V152 maha - removed type restriction
		res = of_imp_appfile(row, this, 'add')
		//res = message.doubleparm
		if res = 1 then
			this.setitem(row, "data_len", 100)
			this.setitemstatus(row, "data_len", primary!, notmodified!)
		elseif res = 7 then  //delete image
			messagebox("Clear Document","Document Deleted")
			this.setitem(row, "data_len", 0)
			this.setitemstatus(row, "data_len", primary!, notmodified!)
		end if
		
//5. b_esign_xx  - (Application Electronic Signature - Sertifi) Harry 05.19.2016 
ELSEIF dwo.name = "b_esign_ns" or dwo.name = "b_esign_s" or dwo.name = "b_esign_nr" or dwo.name = "b_esign_r"  THEN 
		ll_rec_id = This.GetItemNumber(row,"rec_id")
		ll_prac_id = This.GetItemNumber(row,"prac_id")
		ll_app_id = This.GetItemNumber(row,"app_id")
		il_flag = 1 		
		...
		If dwo.name = "b_esign_ns" Then //not sent
			SELECT isnull(COUNT(*),0) INTO :li_count FROM net_dev_action_images WHERE rec_id = :ll_rec_id and prac_id = :ll_prac_id;
			If li_count <= 0 Then
				Messagebox('Check Document','Please upload a document first!')
				Return
			End If
			Choose Case gs_esigntype  
				Case 'sertifi'
					If This.Event ue_sertify() <> 1 Then 
						Return
					End If
				Case 'docusign'
					...
					If This.Event ue_docusign() <> 1 Then 
						Return
					End If
			End Choose
		End If
		
		If dwo.name = "b_esign_nr" Then //not received
			Choose Case gs_esigntype  
				Case 'sertifi'
					li_ret =  gnv_sertifi.of_download_from_sertifi(ll_rec_id) 
				Case 'docusign'
					li_ret =  gnv_docusign.of_download_from_sertifi(ll_rec_id) 
			End Choose
			If li_ret = 1 Then
				of_after_receive_signed_doc(this, row, ll_rec_id)
			End If
		End If
		
		If dwo.name = "b_esign_s" or dwo.name = "b_esign_r" Then //sent or received
			If dwo.name = "b_esign_s" Then //sent
				ll_sig_status = This.GetItemNumber(row,"sig_status")
				If IsNull(ll_sig_status) or ll_sig_status = 1 then 
					//refresh & download
					Choose Case gs_esigntype  
						Case 'sertifi'
							li_ret = gnv_sertifi.of_refresh_sertifi_status(ll_prac_id, ll_rec_id, ll_app_id)
						Case 'docusign'
							li_ret = gnv_docusign.of_refresh_sertifi_status(ll_prac_id, ll_rec_id, ll_app_id)
					End Choose		
					If li_ret =  1 Then 
						of_after_receive_signed_doc(this, row, ll_rec_id)
					End If
				End If
			End If
			openwithparm(w_app_sertifi_view, String(ll_rec_id) + "#" + String(ll_prac_id))
		End If
END IF
...
event dw_net_dev::itemchanged;call super::itemchanged;integer res //maha 02.23.2017
integer li_null  //maha 03.14.2017 
integer li_final
long ll_complete
long ll_act
datetime ld_null  //maha 03.14.2017 

If AncestorReturnValue <> 0 Then Return AncestorReturnValue //Added by Appeon long.zhang 06.15.2017 (V15.3 Bug #5674 - Workflow: When Changing the status of one action type, 1 or more actions statuses change to codes)
...
//Start Code Change ----02.23.2017 #V153 maha - clear par status if completed 
IF Upper(dwo.name) = "ACTION_STATUS" THEN //setting action date
	select lookup_code into :ll_complete from code_lookup where lookup_name = 'Action Status' and code = 'Complete';
	if this.getitemnumber(row, "action_status", primary!, true) = ll_complete then  //changed from complete
		if tab_1.tabpage_ai.dw_tracking.rowcount() > 0 then
			li_final = integer(gnv_data.of_getitem("app_steps","last step",""))
		//	ll_act = long(gnv_data.of_getitem("app_steps","next step", string(li_final)))
		//	ls_field = "step_" + string(li_step) + "_type"
			ll_act =  gnv_data.ids_app_step_cache.getitemnumber(1, "step_" + string(li_final) + "_type")
			if  ll_act =  this.getitemnumber(row, "action_type") then
				if tab_1.tabpage_ai.dw_tracking.getitemnumber(1,"par_status") = 1 then
					res = messagebox("Status change",  "You have changed the last tracking step Status from Complete.  Do you wish to clear the Tracking Record Par Status and the Last Step  and Action dates?", question!,yesno!,1)
					if res = 1 then
						setnull(li_null)
						dw_tracking.setitem(1,"par_status", li_null)
						 //Start Code Change ----03.14.2017 #V153 maha
						setnull(ld_null)
						dw_tracking.setitem(1, "step_" + string(li_final), ld_null)
						this.setitem(row, "action_date", ld_null)
						 //End Code Change ----03.14.2017
					end if
				end if
			end if
		end if
...
END IF
//End Code Change ----02.23.2017 
 
//Start Code Change ----06.29.2016 #V152 maha
if dwo.name =  "app_id" or dwo.name =  "gp_id" then 
	long n
	setnull(n)
	this.setitem(row, "track_id", n)
	ib_new_add = true
end if
//End Code Change ----06.29.2016
...
/*
// (Appeon)Harry 06.07.2016 - Application Electronic Signature - Sertifi
// If have no Sertifi right, set the visible of esign button to false
Int i
If This.dataobject = "d_net_dev_action_items_sk" Then
	If Not (gnv_data.of_getitem( 'icred_settings', 'esign', False) = '1' and gs_esigntype = 'sertifi') Then
		If rowcount > 0 Then
			For i = 1 to rowcount
				This.SetItem(i, 'sig_status', -1)
			Next
		End If
	End If
End If
...
if ib_new_add = true then//(Appeon)Stephen 07.17.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
	if lb_from_prac_folder then
	...
	ib_new_add = false
else//(Appeon)Stephen 07.17.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
	post of_retrieve_detail()
end if
//---End Modified --------------------------
...
//tab_1.tabpage_ai.dw_tracking.retrieve( il_prac_id, il_app_id, ii_facility_id , il_loc_id  ) //Commented by (Appeon)Stephen 07.19.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
...
//---------Begin Added by (Appeon)Stephen 07.17.2017 for Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017--------
for r = 1 to this.rowcount()
	IF This.GetItemStatus( r, 0, Primary! ) = NewModified! then 
		ib_new_add = true
		exit
	end if
next
...
//------------------- APPEON BEGIN -------------------
//<$>added:long.zhang 12.05.2016
//<$>reason:Bug id 5427 - Workflow not working properly
//				Set track id before call supper pfc_preupdate(), by Appeon long.zhang 12.05.2016 (v15.3 AD Authentication)
 if tab_1.tabpage_ai.dw_tracking.rowcount() > 0 then
	of_check_tracking_id()
	ll_track = This.GetItemNumber(r,'track_id')
end if
...
//Moved above by Appeon long.zhang 12.05.2016 (Bug id 5427 - Workflow not working properly)
...
if il_track_id > 0 then this.setitem(this.getrow(),"track_id", il_track_id)  //Start Code Change ----12.19.2016 #V15.2 maha
if ii_facility_id > 0 then this.setitem(this.getrow(),"facility_id",ii_facility_id)  //Added by Harry 2017-10-18

//Start Code Change ----06.15.2016 #V152 maha
if il_app_id > 0 then
	if this.rowcount() > 1 then
		r = this.getrow()
		this.setitem(r ,"app_type", this.getitemstring(r - 1, "app_type"))
		this.setitem(r ,"action_dept", this.getitemnumber(r - 1, "action_dept"))
	end if
end if
...
//------------------- APPEON END ---------------------
ib_new_add = true //(Appeon)Stephen 07.17.2017 - Bug id 5719 - Workflow Step Execution Lag Issue V15.3 SaaS Build 06232017
return 1
...
if this.rowcount() = 1 then
	inv_data_entry.of_field_audit_delete_last( tab_1.tabpage_ai.dw_tracking, 1) //(Appeon)Stephen 06.15.2016 - V15.2-Application Tracking full Auditing
...
event dw_net_dev::getfocus;call super::getfocus;idw_curr = this  //Start Code Change ----07.12.2016 #V152 maha
end event
...
this.st_pnotes=create st_pnotes
int iCurrent
call super::create
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_pnotes
end on

on tabpage_notes.destroy
call super::destroy
destroy(this.st_pnotes)
end on
...
event dw_orgs_facilities::rowfocuschanged;//not used except for single level (facility) 
...
//if il_app_id = 0 then //Comment by Appeon long.zhang 06.28.2016
if il_app_id = 0  or (Not lb_from_prac_folder) then //Modified by Appeon long.zhang 06.28.2016 (Bug ID# 5216 for Case # 00064932: Error - Action Items - Application Status)
...
//of_check_tracking_id( )  //Start Code Change ----09.04.2015 #V15 maha - maha removed 09.29.2016
end event
...
event dw_orgs_facilities::pfc_predeleterow;call super::pfc_predeleterow;return PREVENT_ACTION  //Start Code Change ----07.12.2016 #V152 maha
end event

event dw_orgs_facilities::getfocus;call super::getfocus;idw_curr = this  //Start Code Change ----07.12.2016 #V152 maha
end event
...
event getfocus;call super::getfocus;idw_curr = this  //Start Code Change ----07.12.2016 #V152 maha
end event
...
for r = 1 to tab_1.tabpage_ai.dw_net_dev.rowcount()
	If Not ( tab_1.tabpage_ai.dw_net_dev.getitemNumber(r,"gp_id") = ll_post ) Then //Add if condition, Added by Appeon long.zhang 04.20.2017 (Bug #5611 for Case #00070133 Defaults back up to top of payer list)
...
//Added by Harry 10.16.2017
long ll_null
SetNull (ll_null)
long r
if dwo.name = "gp_id" then
	for r = 1 to tab_1.tabpage_ai.dw_net_dev.rowcount()
		tab_1.tabpage_ai.dw_net_dev.SetItem(r, 'track_id', ll_null)  
	next
	il_loc_id = Long(data)
	ib_new_add = true
end if
...
inv_data_entry.of_field_audit(this) //(Appeon)Stephen 06.03.2016 - V15.2-Application Tracking full Auditing
return 1
end event
...
event pfc_deleterow;//Added by Appeon long.zhang 10.28.2014
ib_save = True
inv_data_entry.of_field_audit_delete_last( this, this.getrow())
call super::pfc_deleterow  //(Appeon)Stephen 07.08.2016 - BugH062802
...
long backcolor = 16777215
string text = "These are general notes for the Provider about all Applications"
...
event clicked;dw_orgs_facilities.selectrow(0,false)
...
long textcolor = 33554432
boolean vscrollbar = true
string item[] = {"140%","130%","120%","110%","100%","90%","80%","70%","50%"}
borderstyle borderstyle = stylelowered!
end type
