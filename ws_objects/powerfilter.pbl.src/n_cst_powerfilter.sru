...
type prototypes
Function long  GetCurrentThemeName( ref string ThemeFileName, int MaxNameChars , ref  string ColorBuff, int maxcolorchars ,  ref string sizebuff, int maxsizechars ) library "uxtheme.dll" //alias for "GetcurrentThemeName;ansi" 
...
//Design-time options
PUBLIC:
Integer	MaxItems = 500 //10000 //default number of Items to allow in the dropdown //Modified By Ken.Guo 11/14/2012. Consider performance for Web
...
Boolean	KeepNewRows = TRUE //if true, Newly added rows will not be filtered.
datawindow	idw_dw 
Window	iw_parent
...
public function integer of_getcurrenttheme ()
public subroutine of_reset_originalfilter ()
end prototypes
...
String	ls_mod,ls_colname,ls_visible, ls_mod2
Long	li_x,li_colwidth,li_ColNum,li_Visible,li_width, li_ColHeight
Long		ll_xpos,ll_ypos, li_y, ll_substract, ll_Label_Height
...
If of_getcurrenttheme( ) = 0 Then//modified by gavins 20121128
	ll_substract = 20
Else
	ll_substract = 0
End If
//Added By Ken.Guo 11/27/2012
String ls_Zoom,ls_height ,ls_width ,ls_objects[], ls_header_height, ls_Label_Visible
ls_zoom = idw_dw.describe( "datawindow.zoom" )
ls_header_height = idw_dw.Describe("DataWindow.Header.Height")

If Integer( ls_Zoom ) > 100 Then  //modified by gavins 20121128
	ls_width =  string(  round( (ButtonWidth - ll_substract)  * 100.0 /  double( ls_zoom  ),2) )	
	ls_height = string(  round( (ButtonHeight - ll_substract) * 100.0 /  double( ls_zoom  ),2 ) )
ElseIf Integer( ls_Zoom ) < 100 and  Integer( ls_Zoom ) > 80 Then
	ls_width =  string( ButtonWidth - ll_substract   + ButtonWidth * (1 - double( ls_zoom) / 100)  )	
	ls_height = string( ButtonHeight - ll_substract + ButtonHeight * (1 - double( ls_zoom) / 100)  )		
Else
	ls_width =  string( ButtonWidth - ll_substract  )	
	ls_height = string( ButtonHeight - ll_substract )	
End If	
...
FOR li_colnum = 1 to ii_maxcol
	...
	li_x=Long(idw_dw.describe(ls_colname+"_t.x"))
//	li_y=Long(idw_dw.describe(ls_colname+".y")) //Added By Ken.Guo 11/27/2012
	li_colwidth=Long(idw_dw.describe(ls_colname+"_t.width"))
//	li_colHeight = Long(idw_dw.describe(ls_colname+".height"))
//	li_x = li_x + buttonXoffset * idec_unitXfactor
	ll_Label_Height = Long(idw_dw.describe(ls_colname+"_t.height"))
	li_y=Long(idw_dw.describe(ls_colname+"_t.y")) 
	ls_Label_Visible = idw_dw.describe(ls_colname+"_t.visible")
	
	IF isnull(li_colwidth ) or  li_colwidth = 0 Or ll_Label_Height = 0 Or li_y > Long( ls_header_height )  Or ls_Label_Visible = '0' Or ls_Label_Visible = '?' Or ls_Label_Visible = '!' THEN //modified by gavins 20121217
	...
	//Modified By Ken.Guo 11/27/2012
	ll_xpos = li_x +li_colwidth - Long(ls_width) - 4
	ll_yPos = ( li_y + ( ll_Label_Height  - Long(ls_height) ) / 2   )  
	
	If ll_xpos < 0 Then ll_xpos = li_x
...
//	idw_dw.Modify("b_powerfilter"+string(li_ColNum)+".width='"+String(li_width)+"'")
	...
	END IF
	IF long(idw_dw.describe("b_powerfilter"+string(li_ColNum)+".y")) <> ll_ypos THEN //modified by gavins 20121128
		//only modify if position has changed
		ls_mod = ls_mod + " " + "b_powerfilter"+string(li_ColNum)+".y='"+String(ll_ypos)+"'"
	END IF	
	IF idw_dw.describe("b_powerfilter"+string(li_ColNum)+".width") <> ls_width THEN
		ls_mod2 = ls_mod2 + " " +  "b_powerfilter"+string(li_ColNum)+".width='"+ string( ButtonWidth - ll_substract  )+"'"
		//only modify if width has changed
		ls_mod = ls_mod + " " + "b_powerfilter"+string(li_ColNum)+".width='"+ls_width+"'" //Modified By Ken.Guo 11/27/2012 Use ls_width
	END IF
	IF idw_dw.describe("b_powerfilter"+string(li_ColNum)+".height") <> ls_height THEN
		ls_mod2 = ls_mod2 + " " +  "b_powerfilter"+string(li_ColNum)+".height='"+ string( ButtonHeight - ll_substract  )+"'"
		//only modify if width has changed
		ls_mod = ls_mod + " " + "b_powerfilter"+string(li_ColNum)+".height='"+ls_height+"'" //Modified By Ken.Guo 11/27/2012. Use ls_height
	END IF	
...
If len( ls_mod2 ) > 0 Then idw_dw.Modify(ls_mod2)
...
.Date:				08/03/2010
.
.Argument(s):	 string dwotype, string dwoname
.
.Returns:			 none
.
.Description:	
...
FOR li_colnum = 1 to ii_maxcol
	...
	ELSE
		//Modified By Ken.Guo 11/06/2012. Support PowerFilter. Appeon only suppert destroy one object each time.
...
IF this.checked THEN //Added By Ken.Guo 11/06/2012. Support PowerFilter
	ls_return = idw_dw.modify(ls_modstring)
End If
...
ELSE
	...
	FOR li_colnum = 1 to ii_maxcol
		...
		If  UpperBound(is_LookUp) > 0  And UpperBound(is_LookUp)  <=ii_maxcol Then					//Added By Mark Lee 11/09/2012  bug
			IF  NOT isnull(is_lookup[li_colnum]) THEN
				//Modified By Ken.Guo 11/06/2012
				//ls_modstring = ls_modstring + " " + "destroy " + is_lookup[li_colnum]
				ls_modstring = "destroy " + is_lookup[li_colnum]
				ls_return = idw_dw.Modify(ls_modstring)
			END IF
		End IF
	NEXT
...
of_Replace(as_Title,'"',' ')
as_Title = trim(as_Title,TRUE) //TRUE = remove all types of leading/trailing whitespace characters
Return 0
end function
...
//"background.mode='2' background.color='67108864' background.transparency='0' background.gradient.color='8421504' background.gradient.transparency='0' "  +  &
...
//"background.gradient.repetition.length='100' background.gradient.focus='0' background.gradient.scale='100' background.gradient.spread='100' "  +  &
//"tooltip.backcolor='134217752' tooltip.delay.initial='900' tooltip.delay.visible='6000' tooltip.enabled='1' tooltip.hasclosebutton='0' tooltip.icon='0' "  +  &
//"tooltip.isbubble='"+ ls_BubbleStyle +"' tooltip.maxwidth='0' tooltip.textcolor='134217751' "  +  &
//"tooltip.title='"+ as_Title +":'  "  +  &
//"tooltip.tip='"+ DefaultTipText +"'  "  +  &
...
.Returns:			 integer
.
.Description:	Set the parent window to open the custom visual objects on
.					
.PowerFilter Datawindow Filter Service
.Copyright © 2010 James R. Reese.  All Rights Reserved.
.============================================================
...
// Function: of_reset
//--------------------------------------------------------------------
// Description: Reset Filter/Sort/Header ICO
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:         Ken.Guo           Date: 10/30/2012
//--------------------------------------------------------------------
...
If Checked Then
//	IF Len(is_OriginalFilter) > 0 Then //Added By Ken.Guo 04/29/2013
		is_OriginalSort = '' //for performance. prevent sort data.
		For i = 1 To UpperBound(is_ColumnFilter[])
			is_ColumnFilter[i] = ''
			iu_powerfilter_dropdown.ib_picturevisible[i]=false
			iu_powerfilter_dropdown.ib_SelectAll[i]=true
			iu_powerfilter_dropdown.ii_buttonsrow3[i] = 0
			iu_powerfilter_dropdown.ii_buttonsrow4[i] = 0
			iu_powerfilter_monthopts.ii_selectedrow[i] = 0
			iu_powerfilter_predeffilters.ii_selectedrow[i] = 0
			of_setbuttonpics()
		Next		
...
public function integer of_getcurrenttheme ();string 		ls_name, s2, s3
long   		ll_return
...
try
	ll_return = GetCurrentThemeName( ls_name, 512, s2, 0, s3 , 0 ) 
catch( exception e )
...
public subroutine of_reset_originalfilter ();is_OriginalFilter = ""
end subroutine
