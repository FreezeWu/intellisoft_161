...
IF is_ColType = 'date' THEN
	dw_powerfilter.dataobject = 'd_powerfilter_datetree'
	//MULTILANG modify expressions for month names and (Blank) here
	//<DW Control Name>.Modify("<ComputedFieldname>.Expression='<expression>'")
	ls_mod = "case(month(dateitem) when 1 then '"+is_January+"' when 2 then '"+is_February+"' when 3 then '"+is_March+"' when 4 then '"+is_April+"' when 5 then '"+is_May+"' when 6 then '"+is_June+ &
	"' when 7 then '"+is_July+"' when 8 then '"+is_August+"' when 9 then '"+is_September+"' when 10 then '"+is_October+"' when 11 then '"+is_November+"' when 12 then '"+is_December+"' else '"+is_Blank+"')"
	ls_return = dw_powerfilter.modify( 'compute_month.Expression="'+ls_mod+'"')

	ls_mod = "IF(isdate(string(dateitem)),string(year( dateitem )),'"+is_Blank+"')"
	ls_return = dw_powerfilter.modify( 'compute_year.Expression="'+ls_mod+'"')
ELSEIF is_ColType = 'datet' THEN
	dw_powerfilter.dataobject = 'd_powerfilter_datetimetree'
	//MULTILANG modify expressions for month names and (Blank) here
	ls_mod = "case(month(datetimeitem) when 1 then '"+is_January+"' when 2 then '"+is_February+"' when 3 then '"+is_March+"' when 4 then '"+is_April+"' when 5 then '"+is_May+"' when 6 then '"+is_June+ &
	"' when 7 then '"+is_July+"' when 8 then '"+is_August+"' when 9 then '"+is_September+"' when 10 then '"+is_October+"' when 11 then '"+is_November+"' when 12 then '"+is_December+"' else '"+is_Blank+"')"
	ls_return = dw_powerfilter.modify( 'compute_month.Expression="'+ls_mod+'"')

	ls_mod = "[General];"+is_Blank
	ls_return = dw_powerfilter.modify( 'compute_year.format="'+ls_mod+'"')
ELSE
	...
	of_replace( ls_mod,'(Blank)', is_blank)
	//Commented By Ken.Guo 10/30/2012. Appeon Not Support.
...
FOR ll_row = 1 to ll_rowcount
	...
	of_Replace(ls_value,'"','~~~"')
	If is_ColType = 'datet' Then
		If  ids_powerfilter[ii_colnum].Find('Left(Item,7) = "' + Left(ls_value,7) + '"',1,ll_dropdownrows) > 0 THEN
			Continue
		End If
	ElseIF ids_powerfilter[ii_colnum].Find('Item = "' + ls_value + '"',1,ll_dropdownrows) > 0 THEN
		CONTINUE
	END IF
	// Insert new row.
	of_Replace(ls_value,'~~~"','"')
...
IF li_height <> this.height or li_width <> this.width THEN
	...
	CHOOSE CASE dw_powerfilter.dataobject
		...
		CASE 'd_powerfilter_datetimetree'
			If dw_powerfilter.describe( "datetimeitem.width" ) <> '?' Then
...
//Begin - Modified By Ken.Guo 03/07/2013
/* The power filter dropdown is getting cut off if the screen is not big enough
...
END IF
//End - Modified By Ken.Guo 03/07/2013
...
If ib_allowquicksort Then //added by gavins 20130523
	dw_buttons.height = cbx_1.y - dw_buttons.y + 4
End If
RETURN 0
...
iu_powerfilter_checkbox.of_buildfilter(is_ColumnFilter,ii_ColNum)
idw_dw.TriggerEvent("ue_AfterPowerFilter")
...
String ls_item,ls_delimfront,ls_delimback,ls_equal,ls_tipItem
...
ELSE
	CHOOSE CASE is_ColType
		...
		CASE 'date'
			...
			ls_delimback = '")'
		CASE 'datet','times'
			//Modified By Ken.Guo 04/08/2013
			ls_delimfront = 'Left("'
			ls_delimback = '",7)'
			/*
			ls_delimfront = 'datetime("'
		...
		CHOOSE CASE ls_item
			CASE is_blank //MULTILANG
				...
				IF ll_rows > 1 THEN
					...
					of_Replace(ls_Item,'"','~~~"')
					If is_ColType = 'datet' Then  //Added By Ken.Guo 04/08/2013  For performance in the Web.
						is_ColumnFilter = is_ColumnFilter + ' or '  + ' String(' + ls_colname + ',"yyyy-mm")' + ' in (' + ls_delimfront + ls_item + ls_delimback 
					Else
			...
			CASE ELSE
				...
				If is_ColType = 'datet' Then //Added By Ken.Guo 04/08/2013. For performance in the Web.
					is_ColumnFilter = ' String(' + ls_colname + ',"yyyy-mm")' + ' in (' + ls_delimfront + ls_item + ls_delimback 
				Else
...
gnv_debug.of_output( False, 'PowerFilter: '+ is_ColumnFilter)
...
IF this.dataobject = 'd_powerfilter_datetree' THEN
	ls_band = this.GetBandAtPointer()
	li_tab = Pos(ls_band, "~t", 1)
	ls_level = Left(ls_band, li_tab - 1)
	ll_row = long(Mid(ls_band, li_tab + 1))
	if ll_row > 0 THEN
		//
	ELSE
		IF row > 0 THEN
			ll_row = row //might be using keyboard instead of mouse?
		ELSE
			ll_row = 1
		END IF
	END IF
	li_year = year(this.GetItemDate(ll_row,'dateitem')) //reference year for row/band selected
	li_month = month(this.GetItemDate(ll_row,'dateitem')) //reference month for row/band selected
	IF isnull(li_year) THEN //(Blank) row, so set that one row and be done
		ls_checked = this.GetItemString(ll_row,'checkyear') //if 0 or 2, set all year to 1, if 1, set all year to 0
		CHOOSE CASE ls_checked
			CASE '0','2'
				ls_setvalue = '1'
			CASE '1'
				ls_setvalue = '0'
		END CHOOSE
		this.SetItem(ll_row,'checkyear',ls_setvalue)
		this.SetItem(ll_row,'checkmonth',ls_setvalue)
		this.SetItem(ll_row,'check',ls_setvalue)				
	ELSE
		CHOOSE CASE ls_level
			CASE 'tree.level.1' // = Year clicked 
				ls_checked = this.GetItemString(ll_row,'checkyear') //if 0 or 2, set all year to 1, if 1, set all year to 0
				CHOOSE CASE ls_checked
					CASE '0','2'
						ls_setvalue = '1'
					CASE '1'
						ls_setvalue = '0'
				END CHOOSE
				DO While year(this.GetItemDate(ll_row,'dateitem')) = li_year
					this.SetItem(ll_row,'checkyear',ls_setvalue)
					this.SetItem(ll_row,'checkmonth',ls_setvalue)
					this.SetItem(ll_row,'check',ls_setvalue)
					ll_row ++
					IF ll_row > this.rowcount() then exit
				LOOP
			CASE 'tree.level.2' // = Month clicked
				ls_checked = this.GetItemString(ll_row,'checkmonth') //if 0 or 2, set all month to 1, if 1, set all month to 0
				CHOOSE CASE ls_checked
					CASE '0','2'
						ls_setvalue = '1'
					CASE '1'
						ls_setvalue = '0'
				END CHOOSE
				DO While month(this.GetItemDate(ll_row,'dateitem')) = li_month and year(this.GetItemDate(ll_row,'dateitem')) = li_year
					this.SetItem(ll_row,'checkmonth',ls_setvalue)
					this.SetItem(ll_row,'check',ls_setvalue)
					ll_row ++
					IF ll_row > this.rowcount() then exit
				LOOP
			CASE ELSE
				//detail band, process like non-date tree
	...
	//find 1st row with year = reference year, find last row with year = reference year
	//if all checked, then checkyear for all = 1.  if none checked, then checkyear for all = 0. if mix, then checkyear = 2
	if isnull( li_year ) Then li_year = 0//added by gavins 20130523
	if isnull( li_month ) Then li_month = 0
	ll_firstrow = this.Find ( "year(dateitem) = "+string(li_year),1,this.rowcount())
	ll_lastrow = this.Find ( "year(dateitem) = "+string(li_year),this.rowcount(),ll_firstrow)
	
	if ll_lastrow = 0 then ll_lastrow = ll_firstrow 
	ib_partial = FALSE
	FOR ll_row = ll_firstrow +1 TO ll_lastrow
		IF this.getitemstring(ll_row,'check') <> this.getitemstring(ll_row - 1,'check') THEN
			ib_partial = TRUE
			EXIT
		END IF
	NEXT
	IF ib_partial THEN 
		this.setitem(ll_firstrow,'checkyear','2')
	ELSE
		this.setitem(ll_firstrow,'checkyear',this.getitemstring(ll_firstrow,'check'))
	END IF
	
	ll_firstrow = this.Find ( "year(dateitem) = "+string(li_year)+" and month(dateitem) = "+string(li_month),1,this.rowcount())
	ll_lastrow = this.Find ( "year(dateitem) = "+string(li_year)+" and month(dateitem) = "+string(li_month),this.rowcount(),ll_firstrow)
	if ll_lastrow = 0 then ll_lastrow = ll_firstrow //last row was = to reference month
	ib_partial = FALSE
	FOR ll_row = ll_firstrow +1 TO ll_lastrow
		IF this.getitemstring(ll_row,'check') <> this.getitemstring(ll_row - 1,'check') THEN
			ib_partial = TRUE
	...
	NEXT
	IF ib_partial THEN 
		this.setitem(ll_firstrow,'checkmonth','2')
	ELSE
		this.setitem(ll_firstrow,'checkmonth',this.getitemstring(ll_firstrow,'check'))
	END IF
ELSEIF this.dataobject = 'd_powerfilter_datetimetree' THEN
	//same logic block as datetree, except references to datetime instead of date
	ls_band = this.GetBandAtPointer()
	li_tab = Pos(ls_band, "~t", 1)
	ls_level = Left(ls_band, li_tab - 1)
	ll_row = long(Mid(ls_band, li_tab + 1))
	...
	ELSE
		...
		END IF
	END IF
	li_year = year(date(this.GetItemDatetime(ll_row,'datetimeitem'))) //reference year for row/band selected
	li_month = month(date(this.GetItemDatetime(ll_row,'datetimeitem'))) //reference month for row/band selected
	IF isnull(li_year) THEN //(Blank) row, so set that one row and be done
		ls_checked = this.GetItemString(ll_row,'checkyear') //if 0 or 2, set all year to 1, if 1, set all year to 0
		CHOOSE CASE ls_checked
			CASE '0','2'
			...
			CASE 'tree.level.1' // = Year clicked 
				...
				CHOOSE CASE ls_checked
					...
					CASE '1'
						ls_setvalue = '0'
				END CHOOSE
				DO While year(date(this.GetItemDatetime(ll_row,'datetimeitem'))) = li_year
					this.SetItem(ll_row,'checkyear',ls_setvalue)
					this.SetItem(ll_row,'checkmonth',ls_setvalue)
					this.SetItem(ll_row,'check',ls_setvalue)
					ll_row ++
			...
			CASE 'tree.level.2' // = Month clicked
				...
				CHOOSE CASE ls_checked
					...
					CASE '1'
						ls_setvalue = '0'
				END CHOOSE
				DO While month(date(this.GetItemDatetime(ll_row,'datetimeitem'))) = li_month and year(date(this.GetItemDatetime(ll_row,'datetimeitem'))) = li_year
					this.SetItem(ll_row,'checkmonth',ls_setvalue)
					this.SetItem(ll_row,'check',ls_setvalue)
					ll_row ++
					IF ll_row > this.rowcount() then exit
	...
	if isnull( li_year ) Then li_year = 0//added by gavins 20130523
	if isnull( li_month ) Then li_month = 0
	ll_firstrow = this.Find ( "year(datetimeitem) = "+string(li_year),1,this.rowcount())
	ll_lastrow = this.Find ( "year(datetimeitem) = "+string(li_year),this.rowcount(),ll_firstrow)
	if ll_lastrow = 0 then ll_lastrow = ll_firstrow 
	ib_partial = FALSE
	FOR ll_row = ll_firstrow +1 TO ll_lastrow
		IF this.getitemstring(ll_row,'check') <> this.getitemstring(ll_row - 1,'check') THEN
	...
	ll_firstrow = this.Find ( "year(datetimeitem) = "+string(li_year)+" and month(datetimeitem) = "+string(li_month),1,this.rowcount())
	ll_lastrow  = this.Find ( "year(datetimeitem) = "+string(li_year)+" and month(datetimeitem) = "+string(li_month),this.rowcount(),ll_firstrow)
	if ll_lastrow = 0 then ll_lastrow = ll_firstrow //last row was = to reference month
	ib_partial = FALSE
	FOR ll_row = ll_firstrow +1 TO ll_lastrow
		IF this.getitemstring(ll_row,'check') <> this.getitemstring(ll_row - 1,'check') THEN
...
ELSE
	...
	//not a date tree, just check or clear
...
CHOOSE CASE row
	...
	CASE this.rowcount() - 1
		...
		of_close()
		of_savestate()	
		idw_dw.TriggerEvent("ue_AfterPowerFilter") //Added By Ken.Guo 10/30/2012. Support PowerFilter
...
event resize;If ib_allowquicksort Then //added by gavins 20130523
	this.y = 4 
	this.height = cbx_1.y - dw_buttons.y + 4
End If
end event
...
*/
Long ll_row,ll_rows
String ls_value0[], ls_value1[]

ll_rows = dw_powerfilter.rowcount()

//Modified By Ken.Guo 04/08/2013. for performance
FOR ll_row = 1 to ll_rows
	ls_value0[ll_row] = '0'
	ls_value1[ll_row] = '1'
Next
...
IF this.checked THEN
	//select all items
	dw_powerfilter.Object.check[1, ll_rows] = ls_value1[]
	dw_powerfilter.Object.checkmonth[1, ll_rows] = ls_value1[]
	dw_powerfilter.Object.checkyear[1, ll_rows] = ls_value1[]
//	FOR ll_row = 1 to ll_rows
...
ELSE
	//unselect all items
	dw_powerfilter.Object.check[1, ll_rows] = ls_value0[]
	dw_powerfilter.Object.checkmonth[1, ll_rows] = ls_value0[]
	dw_powerfilter.Object.checkyear[1, ll_rows] = ls_value0[]	
//	FOR ll_row = 1 to ll_rows
