...
type dp_1 from datepicker within w_powerfilter_custom
end type
type dp_2 from datepicker within w_powerfilter_custom
end type
...
CHOOSE CASE lower(is_openparms.Parm10)
	CASE 'char(','char'
		...
		if lower( ds_1.Describe("item.BitmapName") ) = 'yes' then
			...
			dw_pic1.height = long(ls_height) 
			dw_pic1.y = dw_parm1.y + 4
			...
			dw_pic2.height = long(ls_height) 
			dw_pic2.y = dw_parm2.y + 4
...
datetime value = DateTime(Date("2010-12-29"), Time("20:04:02.000000"))
integer textsize = -8
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
string calendarfontname = "Tahoma"
...
Date ld_date
Time lt_time
this.getvalue(ld_date,lt_time) 
dw_parm1.SetItem(1,'Item',String(ld_date))//,is_colformat))
...
Time lt_time
this.getvalue(ld_date,lt_time) 
dw_parm2.SetItem(1,'Item',String(ld_date))//,is_colformat))
